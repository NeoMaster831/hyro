 ...
 *           Intel Manual
 *
 * @remarks All references are based on <b>Intel(R) 64 and IA-32 architectures
 * software developer's manual combined volumes: 1, 2A, 2B, 2C, 2D, 3A, 3B, 3C,
 * 3D, and 4</b> (June 2021) and <b>Intel(R) Virtualization Technology for
 * Directed I/O</b> (April 2021)
 * @{
 */
/**
 ...
 *           Control registers
 *
 * Control registers (CR0, CR1, CR2, CR3, and CR4) determine operating mode of
 * the processor and the characteristics of the currently executing task. These
 * registers are 32 bits in all 32-bit modes and compatibility mode. In 64-bit
 * mode, control registers are expanded to 64 bits. The MOV CRn instructions are
 * used to manipulate the register bits. Operand-size prefixes for these
 * instructions are ignored. The following is also true:
 * - The control registers can be read and loaded (or modified) using the
 * move-to-or-from-control-registers forms of the MOV instruction. In protected
 * mode, the MOV instructions allow the control registers to be read or loaded
 * (at privilege level 0 only). This restriction means that application programs
 * or operating-system procedures (running at privilege levels 1, 2, or 3) are
 ...
 * are within the linear-address or physical-address limitations of the
 * implementation.
 * - Register CR8 is available in 64-bit mode only. The control registers are
 * summarized below, and each architecturally defined control field in these
 * control registers is described individually.
 ...
 * of that paging structure in the processor's internal data caches (they do not
 * control TLB caching of page-directory information). When using the physical
 * address extension, the CR3 register contains the base address of the
 * page-directorypointer table. In IA-32e mode, the CR3 register contains the
 ...
 * processor. This register is available only in 64-bit mode. However, interrupt
 * filtering continues to apply in compatibility mode.
 *
...
typedef union {
  struct {
     ...
     *
     * [Bit 3] Controls the memory type used to access the first paging
     * structure of the current paging-structure hierarchy. This bit is not used
     * if paging is disabled, with PAE paging, or with 4-level paging if
     * CR4.PCIDE=1.
     *
...
#define CR3_PAGE_LEVEL_WRITE_THROUGH(_) (((_) >> 3) & 0x01)
     ...
     *
     * [Bit 4] Controls the memory type used to access the first paging
     * structure of the current paging-structure hierarchy. This bit is not used
     * if paging is disabled, with PAE paging, or with 4-level paging2 if
     * CR4.PCIDE=1.
     *
     * @see Vol3A[4.9(PAGING AND MEMORY TYPING)]
     */
...
#define CR3_PAGE_LEVEL_CACHE_DISABLE(_) (((_) >> 4) & 0x01)
     ...
     * (32-bit paging) or PML4 table (64-bit paging) used for linear-address
     * translation.
     *
...
#define CR4_USERMODE_INSTRUCTION_PREVENTION(_) (((_) >> 11) & 0x01)
     ...
     *
     * [Bit 12] When set in IA-32e mode, the processor uses 5-level paging to
     * translate 57-bit linear addresses. When clear in IA-32e mode, the
     * processor uses 4-level paging to translate 48-bit linear addresses. This
     * bit cannot be modified in IA-32e mode.
     *
     * @see Vol3C[4(PAGING)]
     */
...
#define CR4_OS_XSAVE(_) (((_) >> 18) & 0x01)
     ...
     *
     * [Bit 19] When set, the LOADIWKEY instruction is enabled; in addition, if
     * support for the AES Key Locker instructions has been activated by system
     * firmware, CPUID.19H:EBX.AESKLE[bit 0] is enumerated as 1 and the AES Key
     * Locker instructions are enabled. When clear, CPUID.19H:EBX.AESKLE[bit 0]
...
#define CR4_PROTECTION_KEY_ENABLE(_) (((_) >> 22) & 0x01)
     ...
     *
     * [Bit 23] Enables control-flow enforcement technology when set. This flag
     * can be set only if CR0.WP is set, and it must be clear before CR0.WP can
     * be cleared.
     *
...
#define CR4_CONTROL_FLOW_ENFORCEMENT_ENABLE(_) (((_) >> 23) & 0x01)
     ...
     *
     * [Bit 24] 4-level paging and 5-level paging associate each supervisor-mode
     * linear address with a protection key. When set, this flag allows use of
     * the IA32_PKRS MSR to specify, for each protection key, whether
...
typedef union {
  struct {
     ...
     *
     * [Bits 3:0] This sets the threshold value corresponding to the
     * highestpriority interrupt to be blocked. A value of 0 means all
     * interrupts are enabled. This field is available in 64- bit mode. A value
...
#define CR8_TASK_PRIORITY_LEVEL(_) (((_) >> 0) & 0x0F)
     ...
     * @brief Reserved
     *
     * [Bits 63:4] Reserved and must be written with zeros. Failure to do this
 ...
 *           Debug registers
 *
 * Eight debug registers control the debug operation of the processor. These
 * registers can be written to and read using the move to/from debug register
 * form of the MOV instruction. A debug register may be the source or
 * destination operand for one of these instructions. Debug registers are
 * privileged resources; a MOV instruction that accesses these registers can
 * only be executed in real-address mode, in SMM or in protected mode at a CPL
 * of 0. An attempt to read or write the debug registers from any other
 * privilege level generates a general-protection exception (\#GP). The primary
 * function of the debug registers is to set up and monitor from 1 to 4
 * breakpoints, numbered 0 though 3. For each breakpoint, the following
 * information can be specified:
 * - The linear address where the breakpoint is to occur.
 * - The length of the breakpoint location: 1, 2, 4, or 8 bytes.
 * - The operation that must be performed at the address for a debug exception
 * to be generated.
...
typedef union {
  struct {
     ...
     *
     * [Bits 3:0] Indicates (when set) that its associated breakpoint condition
     * was met when a debug exception was generated. These flags are set if the
     ...
     * breakpoint is not enabled by the Ln or the Gn flags in register DR7.
     * Therefore on a \#DB, a debug handler should check only those B0-B3 bits
     * which correspond to an enabled breakpoint.
     */
...
#define DR6_BREAKPOINT_CONDITION(_) (((_) >> 0) & 0x0F)
     ...
     * accesses one of the debug registers (DR0 through DR7). This flag is
     * enabled when the GD (general detect) flag in debug control register DR7
     * is set.
     *
     * @see Vol3B[17.2.4(Debug Control Register (DR7))]
     */
...
#define DR6_DEBUG_REGISTER_ACCESS_DETECTED(_) (((_) >> 13) & 0x01)
     ...
     * the singlestep execution mode (enabled with the TF flag in the EFLAGS
     * register). The single-step mode is the highestpriority debug exception.
     * When the BS flag is set, any of the other debug status bits also may be
     * set.
     */
...
#define DR6_SINGLE_INSTRUCTION(_) (((_) >> 14) & 0x01)
     ...
     *
     * [Bit 15] Indicates (when set) that the debug exception was triggered by
     * the singlestep execution mode (enabled with the TF flag in the EFLAGS
     * register). The single-step mode is the highestpriority debug exception.
     * When the BS flag is set, any of the other debug status bits also may be
     * set.
...
#define DR6_TASK_SWITCH(_) (((_) >> 15) & 0x01)
     ...
     *
     * [Bit 16] Indicates (when clear) that a debug exception (\#DB) or
     * breakpoint exception (\#BP) occurred inside an RTM region while advanced
     * debugging of RTM transactional regions was enabled. This bit is set for
     * any other debug exception (including all those that occur when advanced
     * debugging of RTM transactional regions is not enabled). This bit is
     * always 1 if the processor does not support RTM.
     *
...
typedef union {
  struct {
    /**
     * @brief L0 through L3 (local breakpoint enable) flags (bits 0, 2, 4, and
     * 6)
     *
     * [Bit 0] Enables (when set) the breakpoint condition for the associated
     ...
     * processor automatically clears these flags on every task switch to avoid
...
#define DR7_LOCAL_BREAKPOINT_0(_) (((_) >> 0) & 0x01)
    ...
    /**
     * @brief G0 through G3 (global breakpoint enable) flags (bits 1, 3, 5, and
     * 7)
     *
     * [Bit 1] Enables (when set) the breakpoint condition for the associated
     * breakpoint for all tasks. When a breakpoint condition is detected and its
     * associated Gn flag is set, a debug exception is generated. The processor
     * does not clear these flags on a task switch, allowing a breakpoint to be
...
#define DR7_GLOBAL_BREAKPOINT_3(_) (((_) >> 7) & 0x01)
     ...
     *
     * [Bit 8] This feature is not supported in the P6 family processors, later
     * IA-32 processors, and Intel 64 processors. When set, these flags cause
     * the processor to detect the exact instruction that caused a data
     * breakpoint condition. For backward and forward compatibility with other
     * Intel processors, we recommend that the LE and GE flags be set to 1 if
...
#define DR7_GLOBAL_EXACT_BREAKPOINT(_) (((_) >> 9) & 0x01)
     ...
     * regions. This advanced debugging is enabled only if IA32_DEBUGCTL.RTM is
     * also set.
     *
     * @see Vol3B[17.3.3(Debug Exceptions, Breakpoint Exceptions, and Restricted
     * Transactional Memory (RTM))]
...
#define DR7_RESTRICTED_TRANSACTIONAL_MEMORY(_) (((_) >> 11) & 0x01)
     ...
     *
     * [Bit 13] Enables (when set) debug-register protection, which causes a
     * debug exception to be generated prior to any MOV instruction that
     * accesses a debug register. When such a condition is detected, the BD flag
     * in debug status register DR6 is set prior to generating the exception.
     * This condition is provided to support in-circuit emulators. When the
     * emulator needs to access the debug registers, emulator software can set
     * the GD flag to prevent interference from the program currently executing
     * on the processor. The processor clears the GD flag upon entering to the
     * debug exception handler, to allow the handler access to the debug
     * registers.
     */
...
#define DR7_GENERAL_DETECT(_) (((_) >> 13) & 0x01)
    ...
    /**
     * @brief R/W0 through R/W3 (read/write) fields (bits 16, 17, 20, 21, 24,
     * 25, 28, and 29)
     *
     * [Bits 17:16] Specifies the breakpoint condition for the corresponding
     * breakpoint. The DE (debug extensions) flag in control register CR4
     * determines how the bits in the R/Wn fields are interpreted. When the DE
     * flag is set, the processor interprets bits as follows:
     * - 00 - Break on instruction execution only.
     * - 01 - Break on data writes only.
     * - 10 - Break on I/O reads or writes.
     * - 11 - Break on data reads or writes but not instruction fetches.
     * When the DE flag is clear, the processor interprets the R/Wn bits the
     * same as for the Intel386(TM) and Intel486(TM) processors, which is as
     * follows:
     * - 00 - Break on instruction execution only.
     * - 01 - Break on data writes only.
     * - 10 - Undefined.
...
#define DR7_READ_WRITE_0(_) (((_) >> 16) & 0x03)
    ...
    /**
     * @brief LEN0 through LEN3 (Length) fields (bits 18, 19, 22, 23, 26, 27,
     * 30, and 31)
     *
     * [Bits 19:18] Specify the size of the memory location at the address
     * specified in the corresponding breakpoint address register (DR0 through
     ...
     * execution), then the LENn field should also be 00. The effect of using
     * other lengths is undefined.
     *
 ...
 */
/**
 * @brief Returns CPUID's Highest Value for Basic Processor Information and the
 ...
 * value is returned in the EAX register and is processor specific. A vendor
 * identification string is also returned in EBX, EDX, and ECX. For Intel
...
/**
 * @brief Returns Model, Family, Stepping Information, Additional Information
...
typedef struct {
  /**
   * @brief When CPUID executes with EAX set to 01H, version information is
   * returned in EAX
   */
  union {
    struct {
...
#define CPUID_VERSION_INFORMATION_EXTENDED_FAMILY_ID(_) (((_) >> 20) & 0xFF)
  ...
  /**
   * @brief When CPUID executes with EAX set to 01H, additional information is
  ...
  union {
    struct {
       ...
       * contains brand strings for IA-32 processors. More information about
...
#define CPUID_ADDITIONAL_INFORMATION_BRAND_INDEX(_) (((_) >> 0) & 0xFF)
       ...
       * the CLFLUSH and CLFLUSHOPT instructions in 8-byte increments. This
...
#define CPUID_ADDITIONAL_INFORMATION_CLFLUSH_LINE_SIZE(_) (((_) >> 8) & 0xFF)
       ...
       * addressing different logical processors in a physical package. This
       * field is only valid if CPUID.1.EDX.HTT[bit 28] = 1.
       */
...
#define CPUID_ADDITIONAL_INFORMATION_INITIAL_APIC_ID(_) (((_) >> 24) & 0xFF)
  ...
  /**
   * @brief When CPUID executes with EAX set to 01H, feature information is
   * returned in ECX and EDX
   */
  union {
    struct {
      /**
...
#define CPUID_FEATURE_INFORMATION_ECX_MONITOR_MWAIT_INSTRUCTION(_)             \
  (((_) >> 3) & 0x01)
       ...
       * the Debug Store feature to allow for branch message storage qualified
       * by CPL.
       */
...
#define CPUID_FEATURE_INFORMATION_ECX_THERMAL_MONITOR_2(_) (((_) >> 8) & 0x01)
       ...
       *
       * [Bit 9] A value of 1 indicates the presence of the Supplemental
...
#define CPUID_FEATURE_INFORMATION_ECX_SUPPLEMENTAL_STREAMING_SIMD_EXTENSIONS_3( \
  ...
  (((_) >> 9) & 0x01)
       ...
       * either adaptive mode or shared mode. A value of 0 indicates this
       * feature is not supported. See definition of the IA32_MISC_ENABLE MSR
...
#define CPUID_FEATURE_INFORMATION_EDX_FLOATING_POINT_UNIT_ON_CHIP(_)           \
  (((_) >> 0) & 0x01)
       ...
       *
       * [Bit 1] Virtual 8086 mode enhancements, including CR4.VME for
       * controlling the feature, CR4.PVI for protected mode virtual interrupts,
       * software interrupt indirection, expansion of the TSS with the software
...
#define CPUID_FEATURE_INFORMATION_EDX_VIRTUAL_8086_MODE_ENHANCEMENTS(_)        \
  (((_) >> 1) & 0x01)
       ...
       * @brief Debugging Extensions
       *
       * [Bit 2] Support for I/O breakpoints, including CR4.DE for controlling
...
#define CPUID_FEATURE_INFORMATION_EDX_DEBUGGING_EXTENSIONS(_)                  \
  (((_) >> 2) & 0x01)
       ...
       *
       * [Bit 3] Large pages of size 4 MByte are supported, including CR4.PSE
       * for controlling the feature, the defined dirty bit in PDE (Page
       * Directory Entries), optional reserved bit trapping in CR3, PDEs, and
       * PTEs.
       */
...
#define CPUID_FEATURE_INFORMATION_EDX_TIMESTAMP_COUNTER(_) (((_) >> 4) & 0x01)
       ...
       *
       * [Bit 5] The RDMSR and WRMSR instructions are supported. Some of the
...
#define CPUID_FEATURE_INFORMATION_EDX_PHYSICAL_ADDRESS_EXTENSION(_)            \
  (((_) >> 6) & 0x01)
       ...
       *
       * [Bit 7] Exception 18 is defined for Machine Checks, including CR4.MCE
       * for controlling the feature. This feature does not define the
       * model-specific implementations of machine-check error logging,
       * reporting, and processor shutdowns. Machine Check exception handlers
       * may have to depend on processor version to do model specific processing
       * of the exception, or test for the presence of the Machine Check
       * feature.
       */
...
#define CPUID_FEATURE_INFORMATION_EDX_SYSENTER_SYSEXIT_INSTRUCTIONS(_)         \
  (((_) >> 11) & 0x01)
       ...
       *
       * [Bit 12] MTRRs are supported. The MTRRcap MSR contains feature bits
       * that describe what memory types are supported, how many variable MTRRs
       * are supported, and whether fixed MTRRs are supported.
       */
...
#define CPUID_FEATURE_INFORMATION_EDX_MEMORY_TYPE_RANGE_REGISTERS(_)           \
  (((_) >> 12) & 0x01)
       ...
       *
       * [Bit 13] The global bit is supported in paging-structure entries that
       * map a page, indicating TLB entries that are common to different
       * processes and need not be flushed. The CR4.PGE bit controls this
       * feature.
       */
...
#define CPUID_FEATURE_INFORMATION_EDX_MACHINE_CHECK_ARCHITECTURE(_)            \
  (((_) >> 14) & 0x01)
       ...
       *
       * [Bit 15] The conditional move instruction CMOV is supported. In
       * addition, if x87 FPU is present as indicated by the CPUID.FPU feature
       * bit, then the FCOMI and FCMOV instructions are supported
       */
...
#define CPUID_FEATURE_INFORMATION_EDX_MMX_SUPPORT(_) (((_) >> 23) & 0x01)
       ...
       *
       * [Bit 24] The FXSAVE and FXRSTOR instructions are supported for fast
       * save and restore of the floating point context. Presence of this bit
       * also indicates that CR4.OSFXSR is available for an operating system to
...
#define CPUID_FEATURE_INFORMATION_EDX_SELF_SNOOP(_) (((_) >> 27) & 0x01)
       ...
       * processor in the package and software should assume only a single APIC
       * ID is reserved. A value of 1 for HTT indicates the value in
...
#define CPUID_FEATURE_INFORMATION_EDX_THERMAL_MONITOR(_) (((_) >> 29) & 0x01)
       ...
       *
       * [Bit 31] The processor supports the use of the FERR\#/PBE\# pin when
       * the processor is in the stop-clock state (STPCLK\# is asserted) to
       * signal the processor that an interrupt is pending and that the
       * processor should return to normal operation to handle the interrupt.
       * Bit 10 (PBE enable) in the IA32_MISC_ENABLE MSR enables this
       * capability.
       */
 ...
 * processor returns encoded data that describe a set of deterministic cache
 * parameters (for the cache level associated with the input in ECX). Valid
 * index values start from 0. Software can enumerate the deterministic cache
 * parameters for each level of the cache hierarchy starting with an index value
 * of 0, until the parameters report the value associated with the cache type
 * field is 0. The architecturally defined fields reported by deterministic
 * cache parameters are documented in Table 3-8. This Cache Size in Bytes
 * - = (Ways + 1) * (Partitions + 1) * (Line_Size + 1) * (Sets + 1)
 * - = (EBX[31:22] + 1) * (EBX[21:12] + 1) * (EBX[11:0] + 1) * (ECX + 1)
 * The CPUID leaf 04H also reports data that can be used to derive the topology
 * of processor cores in a physical package. This information is constant for
 * all valid index values. Software can query the raw data reported by executing
 * CPUID with EAX=04H and ECX=0 and use it as part of the topology enumeration
 * algorithm.
 *
...
#define CPUID_EAX_CACHE_LEVEL(_) (((_) >> 5) & 0x07)
      ...
      /**
       * [Bit 8] Self Initializing cache level (does not need SW
       * initialization).
       */
...
#define CPUID_ECX_NUMBER_OF_SETS(_) (((_) >> 0) & 0xFFFFFFFF)
  ...
  union {
    struct {
       ...
       *
       * [Bit 0] - 0 = WBINVD/INVD from threads sharing this cache acts upon
 ...
 * instruction. The MWAIT instruction optionally provides additional extensions
...
#define CPUID_ECX_ENUMERATION_OF_MONITOR_MWAIT_EXTENSIONS(_) (((_) >> 0) & 0x01)
      ...
      /**
       * [Bit 1] Supports treating interrupts as break-event for MWAIT, even
 ...
 * about thermal and power management features.
 */
...
#define CPUID_EAX_POWER_LIMIT_NOTIFICATION(_) (((_) >> 4) & 0x01)
      ...
      /**
       * [Bit 5] ECMD. Clock modulation duty cycle extension is supported if
       * set.
       */
...
#define CPUID_EAX_HWP_NOTIFICATION(_) (((_) >> 8) & 0x01)
      ...
      /**
       * [Bit 9] HWP_Activity_Window. IA32_HWP_REQUEST[bits 41:32] is supported
       * if set.
       */
...
#define CPUID_EAX_HWP_ACTIVITY_WINDOW(_) (((_) >> 9) & 0x01)
      ...
      /**
       * [Bit 10] HWP_Energy_Performance_Preference. IA32_HWP_REQUEST[bits
       * 31:24] is supported if set.
       */
...
#define CPUID_EAX_HWP_ENERGY_PERFORMANCE_PREFERENCE(_) (((_) >> 10) & 0x01)
      ...
      /**
       * [Bit 11] HWP_Package_Level_Request. IA32_HWP_REQUEST_PKG MSR is
       * supported if set.
       */
...
#define CPUID_EBX_NUMBER_OF_INTERRUPT_THRESHOLDS_IN_THERMAL_SENSOR(_)          \
  ...
  union {
    struct {
      /**
       * [Bit 0] Hardware Coordination Feedback Capability (Presence of
       ...
       * a percentage of the expected processor performance when running at the
       * TSC frequency.
       */
...
#define CPUID_ECX_NUMBER_OF_INTEL_THREAD_DIRECTOR_CLASSES(_) (((_) >> 3) & 0x01)
      ...
      /**
       * [Bits 15:8] The processor supports performance-energy bias preference
       * if CPUID.06H:ECX.SETBH[bit 3] is set and it also implies the presence
       * of a new architectural MSR called IA32_ENERGY_PERF_BIAS (1B0H).
       */
...
/**
 * @brief Structured Extended Feature Flags Enumeration Leaf (Output depends on
 * ECX input value)
 *
 ...
 * extended feature flags. When CPUID executes with EAX set to 07H and the input
 * value of ECX is invalid (see leaf 07H entry in Table 3-8), the processor
 * returns 0 in EAX/EBX/ECX/EDX. In subleaf 0, EAX returns the maximum input
 * value of the highest leaf 7 sub-leaf, and EBX, ECX & EDX contain information
...
#define CPUID_ECX_AVX512_VBMI2(_) (((_) >> 6) & 0x01)
      ...
      /**
       * [Bit 7] Supports CET shadow stack features if 1. Processors that set
       * this bit define bits 1:0 of the IA32_U_CET and IA32_S_CET MSRs.
...
#define CPUID_EDX_PCONFIG(_) (((_) >> 18) & 0x01)
      ...
      /**
       * [Bit 20] Supports CET indirect branch tracking features if 1.
       * Processors that set this bit define bits 5:2 and bits 63:10 of the
...
#define CPUID_EDX_IA32_CORE_CAPABILITIES(_) (((_) >> 30) & 0x01)
       ...
       * (SSBD). Processors that set this bit support the IA32_SPEC_CTRL MSR.
 ...
 * about Direct Cache Access capabilities.
 */
 ...
 * of CPUID leaf 0BH by verifying
 * - the highest leaf index supported by CPUID is >= 0BH, and
 * - CPUID.0BH:EBX[15:0] reports a non-zero value.
 *
 * @note Most of Leaf 0BH output depends on the initial value in ECX. The EDX
 * output of leaf 0BH is always valid and does not vary with input value in ECX.
 * Output value in ECX[7:0] always equals input value in ECX[7:0]. Sub-leaf
 * index 0 enumerates SMT level. Each subsequent higher sub-leaf index
 * enumerates a higherlevel topological entity in hierarchical order. For
 * sub-leaves that return an invalid level-type of 0 in ECX[15:8]; EAX and EBX
 * will return 0. If an input value n in ECX returns the invalid level-type of 0
...
#define CPUID_EAX_X2APIC_ID_TO_UNIQUE_TOPOLOGY_ID_SHIFT(_) (((_) >> 0) & 0x1F)
  ...
  union {
    struct {
       ...
       *
       * @note Software must not use EBX[15:0] to enumerate processor topology
       * of the system. This value in this field (EBX[15:0]) is only intended
       * for display/diagnostic purposes. The actual number of logical
       * processors available to BIOS/OS/Applications may be different from the
...
#define CPUID_ECX_LEVEL_NUMBER(_) (((_) >> 0) & 0xFF)
       ...
       *
       * @note The value of the "level type" field is not related to level
 ...
 * extensions that are supported in the processor and storage size requirements
 * of the XSAVE/XRSTOR area. When CPUID executes with EAX set to 0DH and ECX = n
 * (n > 1, and is a valid sub-leaf index), the processor returns information
 * about the size and offset of each processor extended state save area within
 * the XSAVE/XRSTOR area. Software can use the forward-extendable technique
 ...
 * <pre> For i = 2 to 62 // sub-leaf 1 is reserved IF (CPUID.(EAX=0DH,
...
typedef struct {
  /**
   * @brief Reports the supported bits of the lower 32 bits of XCR0. XCR0[n] can
   * be set to 1 only if EAX[n] is 1
   */
  union {
    struct {
      /**
...
/**
 * @brief Processor Extended State Enumeration Sub-leaves (EAX = 0DH, ECX = n, n
 * > 1)
 *
 * @note Leaf 0DH output depends on the initial value in ECX. Each sub-leaf
 * index (starting at position 2) is supported if it corresponds to a supported
 * bit in either the XCR0 register or the IA32_XSS MSR. If ECX contains an
 * invalid sub-leaf index, EAX/EBX/ECX/EDX return 0. Sub-leaf n (0 <= n <= 31)
 * is invalid if sub-leaf 0 returns 0 in EAX[n] and sub-leaf 1 returns 0 in
 * ECX[n]. Sub-leaf n (32 <= n <= 63) is invalid if sub-leaf 0 returns 0 in
...
typedef struct {
  union {
    struct {
       ...
       * save area for an extended state feature associated with a valid
       * sub-leaf index, n.
       */
      UINT32 Ia32PlatformDcaCap : 32;
#define CPUID_EAX_IA32_PLATFORM_DCA_CAP_BIT 0
...
#define CPUID_EAX_IA32_PLATFORM_DCA_CAP(_) (((_) >> 0) & 0xFFFFFFFF)
  ...
  union {
    struct {
       ...
       * area from the beginning of the XSAVE/XRSTOR area. This field reports 0
       * if the sub-leaf index, n, does not map to a valid bit in the XCR0
       * register.
       */
      UINT32 Reserved : 32;
#define CPUID_EBX_RESERVED_BIT 0
 ...
 * the processor can use to monitor of any supported resource types. Each bit,
 * starting from bit 1, corresponds to a specific resource type if the bit is
 * set. The bit position corresponds to the sub-leaf index (or ResID) that
 * software must use to query QoS monitoring capability available for that type.
 * See Table 3-8. When CPUID executes with EAX set to 0FH and ECX = n (n >= 1,
 * and is a valid ResID), the processor returns information software can use to
 * program IA32_PQR_ASSOC, IA32_QM_EVTSEL MSRs before reading QoS data from the
 * IA32_QM_CTR MSR.
 * @{
 */
#define CPUID_INTEL_RESOURCE_DIRECTOR_TECHNOLOGY_MONITORING_INFORMATION        \
  0x0000000F
/**
 * @brief Intel Resource Director Technology (Intel RDT) Monitoring Enumeration
 * Sub-leaf (EAX = 0FH, ECX = 0)
 *
 * @note Leaf 0FH output depends on the initial value in ECX. Sub-leaf index 0
...
/**
 * @brief L3 Cache Intel RDT Monitoring Capability Enumeration Sub-leaf (EAX =
 * 0FH, ECX = 1)
 *
 ...
 * information about the bit-vector representation of QoS Enforcement resource
 * types that are supported in the processor. Each bit, starting from bit 1,
 * corresponds to a specific resource type if the bit is set. The bit position
 * corresponds to the sub-leaf index (or ResID) that software must use to query
 * QoS enforcement capability available for that type. When CPUID executes with
 * EAX set to 10H and ECX = n (n >= 1, and is a valid ResID), the processor
 * returns information about available classes of service and range of QoS mask
 * MSRs that software can use to configure each class of services using
 * capability bit masks in the QoS Mask registers, IA32_resourceType_Mask_n.
 * @{
 */
#define CPUID_INTEL_RESOURCE_DIRECTOR_TECHNOLOGY_ALLOCATION_INFORMATION        \
  0x00000010
/**
 * @brief Intel Resource Director Technology (Intel RDT) Allocation Enumeration
 * Sub-leaf (EAX = 10H, ECX = 0)
 *
 * @note Leaf 10H output depends on the initial value in ECX. Sub-leaf index 0
 * reports valid resource identification (ResID) starting at bit position 1 of
 * EBX.
 */
typedef struct {
  union {
...
/**
 * @brief L3 Cache Allocation Technology Enumeration Sub-leaf (EAX = 10H, ECX =
 * ResID = 1)
 *
...
#define CPUID_EAX_LENGTH_OF_CAPACITY_BIT_MASK(_) (((_) >> 0) & 0x1F)
  ...
  union {
    struct {
      /**
       * [Bits 31:0] Bit-granular map of isolation/contention of allocation
       * units.
       */
      UINT32 Ebx0 : 32;
...
/**
 * @brief L2 Cache Allocation Technology Enumeration Sub-leaf (EAX = 10H, ECX =
 * ResID = 2)
 *
 * @note Leaf 10H output depends on the initial value in ECX.
 */
...
/**
 * @brief Memory Bandwidth Allocation Enumeration Sub-leaf (EAX = 10H, ECX =
 * ResID = 3)
 *
 * @note Leaf 10H output depends on the initial value in ECX.
 */
 ...
 *           EAX = 0x12
 *
 * When CPUID executes with EAX set to 12H and ECX = 0H, the processor returns
 * information about Intel SGX capabilities. When CPUID executes with EAX set to
 * 12H and ECX = 1H, the processor returns information about Intel SGX
 * attributes. When CPUID executes with EAX set to 12H and ECX = n (n > 1), the
 * processor returns information about Intel SGX Enclave Page Cache.
 * @{
 */
 ...
 *
 * @note Leaf 12H sub-leaf 0 (ECX = 0) is supported if CPUID.(EAX=07H,
 * ECX=0H):EBX[SGX] = 1.
 */
typedef struct {
  union {
 ...
 *
 * @note Leaf 12H sub-leaf 1 (ECX = 1) is supported if CPUID.(EAX=07H,
 * ECX=0H):EBX[SGX] = 1.
 */
typedef struct {
  union {
    struct {
      /**
       * [Bits 31:0] Reports the valid bits of SECS.ATTRIBUTES[31:0] that
...
/**
 * @brief Intel SGX EPC Enumeration Leaf, sub-leaves (EAX = 12H, ECX = 2 or
 * higher)
 *
 * @note Leaf 12H sub-leaf 2 or higher (ECX >= 2) is supported if
 * CPUID.(EAX=07H, ECX=0H):EBX[SGX] = 1. This structure describes sub-leaf type
 * 0.
 */
typedef struct {
  union {
 ...
 *
 * @note Leaf 12H sub-leaf 2 or higher (ECX >= 2) is supported if
 * CPUID.(EAX=07H, ECX=0H):EBX[SGX] = 1. This structure describes sub-leaf
 * type 1.
 */
typedef struct {
  union {
...
#define CPUID_EBX_EPC_BASE_PHYSICAL_ADDRESS_2(_) (((_) >> 0) & 0xFFFFF)
  ...
  union {
    struct {
       ...
       * - If EAX[3:0] 0000b, then all bits of the EDX:ECX pair are enumerated
       * as 0.
       * - If EAX[3:0] 0001b, then this section has confidentiality and
       * integrity protection. All other encodings are reserved.
       */
 ...
 *           EAX = 0x14
 *
 * When CPUID executes with EAX set to 14H and ECX = 0H, the processor returns
 * information about Intel Processor Trace extensions. When CPUID executes with
 * EAX set to 14H and ECX = n (n > 0 and less than the number of non-zero bits
 * in CPUID.(EAX=14H, ECX= 0H).EAX), the processor returns information about
 * packet generation in Intel Processor Trace.
 * @{
 */
...
#define CPUID_EBX_FLAG5(_) (((_) >> 5) & 0x01)
       ...
       * set IA32_RTIT CTL[56] (InjectPsb-PmiOnEnable), enabling the processor
       * to setIA32_12TIT STATUS[7] (PendTopaPMI) and/or IA32_RTIT_STATUS[6]
...
#define CPUID_EBX_FLAG6(_) (((_) >> 6) & 0x01)
      ...
      /**
       * [Bit 7] If 1, writes can set IA32_RTIT_CTL[31] (EventEn), enabling
 ...
 *
 * When CPUID executes with EAX set to 15H and ECX = 0H, the processor returns
 * information about Time Stamp Counter and Core Crystal Clock.
 *
 * @note If EBX[31:0] is 0, the TSC/"core crystal clock" ratio is not
 * enumerated. EBX[31:0]/EAX[31:0] indicates the ratio of the TSC frequency and
 * the core crystal clock frequency. If ECX is 0, the nominal core crystal clock
 * frequency is not enumerated. "TSC frequency" = "core crystal clock frequency"
 * * EBX/EAX.
 */
...
typedef struct {
  union {
    struct {
      /**
       * [Bits 31:0] An unsigned integer which is the denominator of the
 ...
 * about Processor Frequency Information.
 *
 ...
 *       accurately correlate to information / counters returned by other
 * processor interfaces. While a processor may support the Processor Frequency
 * Information leaf, fields that return a value of zero are not supported.
 */
 ...
 * about the System-On-Chip Vendor Attribute Enumeration.
 * @{
 */
#define CPUID_SOC_VENDOR_INFORMATION 0x00000017
/**
 * @brief System-On-Chip Vendor Attribute Enumeration Main Leaf (EAX = 17H, ECX
 * = 0)
 *
 * @note Leaf 17H main leaf (ECX = 0). Leaf 17H output depends on the initial
 * value in ECX. Leaf 17H sub-leaves 1 through 3 reports SOC Vendor Brand
 * String. Leaf 17H is valid if MaxSOCID_Index >= 3. Leaf 17H sub-leaves 4 and
 * above are reserved.
 */
typedef struct {
  union {
    struct {
      /**
       * [Bits 31:0] Reports the maximum input value of supported sub-leaf in
       * leaf 17H.
       */
...
/**
 * @brief System-On-Chip Vendor Attribute Enumeration Sub-leaf (EAX = 17H, ECX
 * = 1..3)
 *
 * @note Leaf 17H output depends on the initial value in ECX. SOC Vendor Brand
 * String is a UTF-8 encoded string padded with trailing bytes of 00H. The
 * complete SOC Vendor Brand String is constructed by concatenating in ascending
 * order of EAX:EBX:ECX:EDX and from the sub-leaf 1 fragment towards sub-leaf 3.
 */
typedef struct {
  union {
    struct {
...
/**
 * @brief System-On-Chip Vendor Attribute Enumeration Sub-leaves (EAX = 17H, ECX
 * > MaxSOCID_Index)
 *
 ...
 * about the Deterministic Address Translation Parameters.
 * @{
 */
#define CPUID_DETERMINISTIC_ADDRESS_TRANSLATION_PARAMETERS 0x00000018
/**
 * @brief Deterministic Address Translation Parameters Main Leaf (EAX = 18H, ECX
 * = 0)
 *
 ...
 * Sub-leaf index n is invalid if n exceeds the value that sub-leaf 0 returns in
 * EAX. A sub-leaf index is also invalid if EDX[4:0] returns 0. Valid sub-leaves
 * do not need to be contiguous or in any particular order. A valid sub-leaf may
 * be in a higher input ECX value than an invalid sub-leaf or than a valid
...
#define CPUID_ECX_NUMBER_OF_SETS(_) (((_) >> 0) & 0xFFFFFFFF)
  ...
  union {
    struct {
       ...
       * read/write and instruction fetches. Others will require separate
       * entries (e.g., one loaded on data read/write and another loaded on an
       * instruction fetch) . Please see the Intel(R) 64 and IA-32 Architectures
...
/**
 * @brief Deterministic Address Translation Parameters Sub-leaf (EAX = 18H, ECX
 * >= 1)
 *
 * @note Each sub-leaf enumerates a different address translation structure.
 *       If ECX contains an invalid sub-leaf index, EAX/EBX/ECX/EDX return 0.
...
typedef struct {
  union {
    struct {
      /**
       * [Bits 31:0] Maximum Input Value for Extended Function CPUID
       * Information.
       */
 ...
 *           IA32_P5_MC_(x)
 *
 * When machine-check exceptions are enabled for the Pentium processor (MCE flag
 * is set in control register CR4), the machine-check exception handler uses the
 * RDMSR instruction to read the error type from the P5_MC_TYPE register and the
 * machine check address from the P5_MC_ADDR register. The handler then normally
 * reports these register values to the system console before aborting
 * execution.
 *
 * @see Vol3B[15.10.2(Pentium Processor Machine-Check Exception Handling)]
 * (reference)
 * @{
 */
/**
...
typedef union {
  struct {
     ...
     *
     * [Bits 52:50] Contains information concerning the intended platform for
     * the processor.
     *
     *
     * 52 | 51 | 50 | _
...
/**
 * This register holds the APIC base address, permitting the relocation of the
 * APIC memory map.
 *
 * @remarks 06_01H
...
typedef union {
  struct {
     ...
     *
     * [Bit 0] When set, locks this MSR from being written; writes to this bit
     * will result in GP(0).
     *
     * @note Once the Lock bit is set, the contents of this register cannot be
     * modified. Therefore the lock bit must be set after configuring support
     * for Intel Virtualization Technology and prior to transferring control to
     * an option ROM or the OS. Hence, once the Lock bit is set, the entire
     * IA32_FEATURE_CONTROL contents are preserved across RESET when PWRGOOD is
     *       not deasserted.
     * @remarks If any one enumeration condition for defined bit field position
...
#define IA32_FEATURE_CONTROL_LOCK_BIT(_) (((_) >> 0) & 0x01)
     ...
     *
     * [Bit 1] This bit enables a system executive to use VMX in conjunction
     * with SMX to support Intel(R) Trusted Execution Technology. BIOS must set
     * this bit only when the CPUID function 1 returns VMX feature flag and SMX
     * feature flag set (ECX bits 5 and 6 respectively).
     *
...
#define IA32_FEATURE_CONTROL_ENABLE_VMX_INSIDE_SMX(_) (((_) >> 1) & 0x01)
     ...
     *
     * [Bit 2] This bit enables VMX for a system executive that does not require
     * SMX. BIOS must set this bit only when the CPUID function 1 returns the
     * VMX feature flag set (ECX bit 5).
     *
...
#define IA32_FEATURE_CONTROL_ENABLE_VMX_OUTSIDE_SMX(_) (((_) >> 2) & 0x01)
     ...
     *
     * [Bits 14:8] When set, each bit in the field represents an enable control
     * for a corresponding SENTER function. This field is supported only if
...
#define IA32_FEATURE_CONTROL_SENTER_LOCAL_FUNCTION_ENABLES(_)                  \
  (((_) >> 8) & 0x7F)
     ...
     *
     * [Bit 15] This bit must be set to enable SENTER leaf functions. This bit
...
#define IA32_FEATURE_CONTROL_SENTER_GLOBAL_ENABLE(_) (((_) >> 15) & 0x01)
     ...
     *
     * [Bit 17] This bit must be set to enable runtime reconfiguration of SGX
     * Launch Control via the IA32_SGXLEPUBKEYHASHn MSR.
     *
...
#define IA32_FEATURE_CONTROL_SGX_GLOBAL_ENABLE(_) (((_) >> 18) & 0x01)
     ...
     *
     * [Bit 20] When set, system software can program the MSRs associated with
     * LMCE to configure delivery of some machine check exceptions to a single
     * logical processor.
     *
...
/**
 * Speculation Control. The MSR bits are defined as logical processor scope. On
 * some core implementations, the bits may impact sibling logical processors on
 * the same core. This MSR has a value of 0 after reset and is unaffected by
 * INIT\# or SIPI\#.
 *
 * @remarks If any one of the enumeration conditions for defined bit field
 * positions holds.
 */
...
#define IA32_SPEC_CTRL_IBRS(_) (((_) >> 0) & 0x01)
     ...
     * indirect branch predictions on all logical processors on the core from
     * being controlled by any sibling logical processor in the same core.
     *
...
/**
 * Prediction Command. Gives software a way to issue commands that affect the
 * state of predictors.
 *
 * @remarks If any one of the enumeration conditions for defined bit field
 * positions holds.
 ...
 *
 * Executing a WRMSR instruction to this MSR causes a microcode update to be
 * loaded into the processor. A processor may prevent writing to this MSR when
 ...
 * A processor may prevent writing to this MSR when loading guest states on VM
...
#define IA32_BIOS_UPDATE_SIGNATURE_RESERVED(_) (((_) >> 0) & 0xFFFFFFFF)
     ...
     *
     * [Bits 63:32] This field contains the signature of the currently loaded
     * microcode update when read following the execution of the CPUID
     * instruction, function 1. It is required that this register field be
     * pre-loaded with zero prior to executing the CPUID, function 1. If the
     * field remains equal to zero, then there is no microcode update loaded.
     * Another nonzero value will be the signature.
     *
 ...
 *
 * Bits (64*n+63):(64*n) of the SHA256 digest of the SIGSTRUCT.MODULUS for SGX
 * Launch Enclave. On reset, the default value is the digest of Intel's signing
 * key.
 *
 * @remarks Read permitted If CPUID.(EAX=12H,ECX=0H): EAX[0]=1 &&
 * CPUID.(EAX=07H,ECX=0H):ECX[30]=1. Write permitted if CPUID.(EAX=12H,ECX=0H):
...
typedef union {
  struct {
     ...
     * @brief Valid <b>(R/W)</b>
     *
     * [Bit 0] The STM may be invoked using VMCALL only if this bit is 1.
     * Because VMCALL is used to activate the dual-monitor treatment, the
     * dual-monitor treatment cannot be activated if the bit is 0. This bit is
     * cleared when the logical processor is reset.
     *
...
#define IA32_SMM_MONITOR_CTL_SMI_UNBLOCKING_BY_VMXOFF(_) (((_) >> 2) & 0x01)
     ...
     *
     * [Bits 31:12] Value that, when shifted left 12 bits, is the physical
     * address of MSEG (the MSEG base address).
     *
     * @see Vol3C[34.15.5(Enabling the Dual-Monitor Treatment)] (reference)
     */
    UINT64 MsegBase : 20;
...
typedef struct {
   ...
   * identifiers enable software to avoid using an MSEG header formatted for one
   * processor on a processor that uses a different format. Software can
   * discover the MSEG revision identifier that a processor uses by reading the
   * VMX capability MSR IA32_VMX_MISC.
   *
...
#define IA32_STM_FEATURES_IA32E 0x00000001
  ...
  /**
   * Fields that determine how processor state is loaded when the STM is
   * activated. SMM code should establish these fields so that activating of the
   * STM invokes the STM's initialization code.
   *
...
typedef union {
  struct {
     ...
     *
     * [Bits 7:0] Indicates the number of variable ranges implemented on the
     * processor.
     */
...
#define IA32_MTRR_CAPABILITIES_VARIABLE_RANGE_COUNT(_) (((_) >> 0) & 0xFF)
     ...
     *
     * [Bit 8] Fixed range MTRRs (IA32_MTRR_FIX64K_00000 through
     * IA32_MTRR_FIX4K_0F8000) are supported when set; no fixed range registers
     * are supported when clear.
     */
...
#define IA32_MTRR_CAPABILITIES_FIXED_RANGE_SUPPORTED(_) (((_) >> 8) & 0x01)
     ...
     *
     * [Bit 10] The write-combining (WC) memory type is supported when set; the
...
#define IA32_MTRR_CAPABILITIES_WC_SUPPORTED(_) (((_) >> 10) & 0x01)
     ...
     *
     * [Bit 11] The system-management range register (SMRR) interface is
     * supported when bit 11 is set; the SMRR interface is not supported when
     * clear.
     */
...
typedef union {
  struct {
    /**
     * [Bit 0] RDCL_NO: The processor is not susceptible to Rogue Data Cache
     * Load (RDCL).
     */
    UINT64 RdclNo : 1;
...
#define IA32_ARCH_CAPABILITIES_SSB_NO(_) (((_) >> 4) & 0x01)
    ...
    /**
     * [Bit 5] MDS_NO: Processor is not susceptible to Microarchitectural Data
     * Sampling (MDS).
     */
    UINT64 MdsNo : 1;
...
#define IA32_ARCH_CAPABILITIES_MDS_NO(_) (((_) >> 5) & 0x01)
    ...
    /**
     * [Bit 6] IF_PSCHANGE_MC_NO: The processor is not susceptible to a machine
...
/**
 * Flush Command. Gives software a way to invalidate structures with finer
 * granularity than other architectural methods.
 *
 * @remarks If any one of the enumeration conditions for defined bit field
 * positions holds.
 */
 ...
 *          Available when CPUID.ARCH_CAP(EAX=7H,ECX = 0):EDX[29] = 1 and
 * IA32_ARCH_CAPABILITIES.bit 7 = 1.
 */
#define IA32_TSX_CTRL 0x00000122
typedef union {
  struct {
    /**
     * [Bit 0] RTM_DISABLE: When set to 1, XBEGIN will always abort with EAX
     * code 0.
     */
    UINT64 RtmDisable : 1;
...
#define IA32_TSX_CTRL_RTM_DISABLE(_) (((_) >> 0) & 0x01)
     ...
     * CPUID.07H.EBX.HLE [bit 4] report 0. When set to 0 and the SKU supports
     * TSX, these bits will return 1.
     */
 ...
 *
 * The lower 16 bits of this MSR are the segment selector for the privilege
 * level 0 code segment. This value is also used to determine the segment
 * selector of the privilege level 0 stack segment. This value cannot indicate a
 * null selector.
 *
 * @remarks 06_01H
 ...
 *
 * The value of this MSR is loaded into RSP (thus, this value contains the stack
 * pointer for the privilege level 0 stack). This value cannot represent a
 ...
 *
 * The value of this MSR is loaded into RIP (thus, this value references the
 * first instruction of the selected operating procedure or routine). In
...
#define IA32_MCG_CAP_MCG_CTL_P(_) (((_) >> 8) & 0x01)
    ...
    /**
     * [Bit 9] Extended machine check state registers are present if this bit is
     * set.
     */
    UINT64 McgExtP : 1;
...
#define IA32_MCG_CAP_MCP_CMCI_P(_) (((_) >> 10) & 0x01)
    ...
    /**
     * [Bit 11] Threshold-based error status register are present if this bit is
     * set.
     */
    UINT64 McgTesP : 1;
...
#define IA32_MCG_CAP_MCG_EXT_CNT(_) (((_) >> 16) & 0xFF)
    ...
    /**
     * [Bit 24] The processor supports software error recovery if this bit is
     * set.
     */
    UINT64 McgSerP : 1;
...
#define IA32_MCG_CAP_MCG_SER_P(_) (((_) >> 24) & 0x01)
    ...
    /**
     * [Bit 26] Indicates that the processor allows platform firmware to be
     * invoked when an error is detected so that it may provide additional
...
#define IA32_MCG_CAP_MCG_ELOG_P(_) (((_) >> 26) & 0x01)
    ...
    /**
     * [Bit 27] Indicates that the processor supports extended state in
...
#define IA32_PERFEVTSEL_EVENT_SELECT(_) (((_) >> 0) & 0xFF)
    ...
    /**
     * [Bits 15:8] Qualifies the microarchitectural condition to detect on the
...
#define IA32_PERFEVTSEL_INTR(_) (((_) >> 20) & 0x01)
    ...
    /**
     * [Bit 21] When set to 1, it enables counting the associated event
...
#define IA32_PERFEVTSEL_ANY_THREAD(_) (((_) >> 21) & 0x01)
    ...
    /**
     * [Bit 22] Enables the corresponding performance counter to commence
...
#define IA32_PERFEVTSEL_INV(_) (((_) >> 23) & 0x01)
    ...
    /**
     * [Bits 31:24] When CMASK is not zero, the corresponding performance
 ...
 *
 * @remarks 0F_03H
 * @see Vol3B[14.1.1(Software Interface For Initiating Performance State
 * Transitions)]
 */
 ...
 *
 * Performance Control. Software makes a request for a new Performance state
 * (P-State) by writing this MSR.
 *
 * @remarks 0F_03H
 * @see Vol3B[14.1.1(Software Interface For Initiating Performance State
 * Transitions)]
...
#define IA32_CLOCK_MODULATION_EXTENDED_ON_DEMAND_CLOCK_MODULATION_DUTY_CYCLE(  \
  ...
  (((_) >> 0) & 0x01)
     ...
     *
     * [Bits 3:1] On-Demand Clock Modulation Duty Cycle: Specific encoded values
     * for target duty cycle modulation.
     *
     * @remarks If CPUID.01H:EDX[22] = 1
     */
 ...
 *
 * Thermal Status Information. Contains status information about the processor's
...
typedef union {
  struct {
     ...
     *
     * [Bit 0] When set, the fast-strings feature (for REP MOVS and REP STORS)
     * is enabled (default). When clear, fast-strings are disabled.
     *
     * @remarks 0F_0H
     */
...
#define IA32_MISC_ENABLE_FAST_STRINGS_ENABLE(_) (((_) >> 0) & 0x01)
     ...
     *
     * [Bit 3] - 1 = Setting this bit enables the thermal control circuit (TCC)
     * portion of the Intel Thermal Monitor feature. This allows the processor
     * to automatically reduce power consumption in response to TCC activation.
     * - 0 = Disabled.
     *
     ...
     * still be activated. The default value of this field varies with product.
     * @remarks 0F_0H
     */
...
#define IA32_MISC_ENABLE_ENHANCED_INTEL_SPEEDSTEP_TECHNOLOGY_ENABLE(_)         \
  (((_) >> 16) & 0x01)
     ...
     * (CPUID.01H:ECX[bit3] = 0). This indicates that MONITOR/MWAIT are not
     * supported. Software attempts to execute MONITOR/MWAIT will cause \#UD
     * when this bit is 0. When this bit is set to 1 (default), MONITOR/MWAIT
     * are supported (CPUID.01H:ECX[bit 3] = 1). If the SSE3 feature flag ECX[0]
     * is not set (CPUID.01H:ECX[bit 0] = 0), the OS must not attempt to alter
     * this bit. BIOS must leave it in the default state. Writing this bit when
     * the SSE3 feature flag is set to 0 may generate a \#GP exception.
     *
     * @remarks 0F_03H
     */
...
#define IA32_MISC_ENABLE_ENABLE_MONITOR_FSM(_) (((_) >> 18) & 0x01)
     ...
     *
     * [Bit 22] When this bit is set to 1, CPUID.00H returns a maximum value in
     * EAX[7:0] of 2. BIOS should contain a setup question that allows users to
     * specify when the installed OS does not support CPUID functions greater
     * than 2. Before setting this bit, BIOS must execute the CPUID.0H and
     * examine the maximum value returned in EAX[7:0]. If the maximum value is
     * greater than 2, this bit is supported. Otherwise, this bit is not
     * supported. Setting this bit when the maximum value is not greater than 2
     * may generate a \#GP exception. Setting this bit may cause unexpected
     * behavior in software that depends on the availability of CPUID leaves
     * greater than 2.
     *
     * @remarks 0F_03H
     */
...
#define IA32_MISC_ENABLE_LIMIT_CPUID_MAXVAL(_) (((_) >> 22) & 0x01)
     ...
     *
     * [Bit 23] When set to 1, xTPR messages are disabled. xTPR messages are
     * optional messages that allow the processor to inform the chipset of its
     * priority.
     *
...
#define IA32_MISC_ENABLE_XTPR_MESSAGE_DISABLE(_) (((_) >> 23) & 0x01)
     ...
     *
     * [Bit 34] When set to 1, the Execute Disable Bit feature (XD Bit) is
     * disabled and the XD Bit extended feature flag will be clear
     * (CPUID.80000001H: EDX[20]=0). When set to a 0 (default), the Execute
     * Disable Bit feature (if available) allows the OS to enable PAE paging and
     * take advantage of data only pages. BIOS must not alter the contents of
     * this bit location, if XD bit is not supported. Writing this bit to 1 when
     * the XD Bit extended feature flag is set to 0 may generate a \#GP
     * exception.
     *
 ...
 * transitions detected with the package's thermal sensor.
 *
 * @remarks If CPUID.06H: EAX[6] = 1
 * @see Vol3B[14.8(PACKAGE LEVEL THERMAL MANAGEMENT)]
 */
...
typedef union {
  struct {
    /**
     * [Bit 0] Setting this bit to 1 enables the processor to record a running
     * trace of the most recent branches taken by the processor in the LBR
     * stack.
     *
     * @remarks 06_01H
     */
...
#define IA32_DEBUGCTL_TR(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 7] Setting this bit enables branch trace messages (BTMs) to be
     * logged in a BTS buffer.
     *
     * @remarks 06_0EH
     */
    UINT64 Bts : 1;
...
#define IA32_DEBUGCTL_BTS(_) (((_) >> 7) & 0x01)
    ...
    /**
     * [Bit 8] When clear, BTMs are logged in a BTS buffer in circular fashion.
...
#define IA32_DEBUGCTL_FREEZE_LBRS_ON_PMI(_) (((_) >> 11) & 0x01)
    ...
    /**
     * [Bit 12] When set, each ENABLE bit of the global counter control MSR are
     * frozen (address 38FH) on a PMI request.
     *
     * @remarks If CPUID.01H: ECX[15] = 1 && CPUID.0AH: EAX[7:0] > 1
     */
...
#define IA32_DEBUGCTL_FREEZE_PERFMON_ON_PMI(_) (((_) >> 12) & 0x01)
    ...
    /**
     * [Bit 13] When set, enables the logical processor to receive and generate
     * PMI on behalf of the uncore.
     *
     * @remarks 06_1AH
     */
...
#define IA32_MTRR_PHYSBASE_TYPE(_) (((_) >> 0) & 0xFF)
    ...
    /**
     * [Bits 47:12] Specifies the base address of the address range. This 24-bit
     * value, in the case where MAXPHYADDR is 36 bits, is extended by 12 bits at
     * the low end to form the base address (this automatically aligns the
...
typedef union {
  struct {
    ...
    /**
     * [Bit 11] Enables the register pair when set; disables register pair when
     * clear.
     */
    UINT64 Valid : 1;
...
#define IA32_MTRR_PHYSMASK_VALID(_) (((_) >> 11) & 0x01)
    ...
    /**
     * [Bits 47:12] Specifies a mask (24 bits if the maximum physical address
     * size is 36 bits, 28 bits if the maximum physical address size is 40
     * bits). The mask determines the range of the region being mapped,
     * according to the following relationships:
     * - Address_Within_Range AND PhysMask = PhysBase AND PhysMask
     * - This value is extended by 12 bits at the low end to form the mask
     * value.
     * - The width of the PhysMask field depends on the maximum physical address
     * size supported by the processor. CPUID.80000008H reports the maximum
     * physical address size supported by the processor. If CPUID.80000008H is
     * not available, software may assume that the processor supports a 36-bit
...
/**
 * Architecture defined number of fixed range MTRRs (1 for 64k, 2 for 16k, 8 for
 * 4k).
 */
#define IA32_MTRR_FIX_COUNT ((1 + 2 + 8) * 8)

/**
 * Architecture defined number of variable range MTRRs. See: Table 2-2. IA-32
 * Architectural MSRs
 *
...
/**
 * Read Only MSR that enumerates the existence of performance monitoring
 * features.
 *
 ...
 *
 * Fixed-Function Performance Counter Control. Counter increments while the
 * results of ANDing respective enable bit in IA32_PERF_GLOBAL_CTRL with the
...
#define IA32_FIXED_CTR_CTRL_EN0_USR(_) (((_) >> 1) & 0x01)
     ...
     * processor core. When set to 0, the counter only increments the associated
     * event conditions occurring in the logical processor which programmed the
     * MSR.
     */
    UINT64 AnyThread0 : 1;
...
#define IA32_PERF_GLOBAL_STATUS_OVF_FIXEDCTR2(_) (((_) >> 34) & 0x01)
    ...
    /**
     * [Bit 55] Trace_ToPA_PMI: A PMI occurred due to a ToPA entry memory buffer
     * that was completely filled.
     *
     * @remarks If (CPUID.(EAX=07H, ECX=0):EBX[25] = 1) && IA32_RTIT_CTL.ToPA =
     * 1
     */
...
#define IA32_PERF_GLOBAL_STATUS_CTR_FRZ(_) (((_) >> 59) & 0x01)
     ...
     * include contributions from the direct or indirect operation Intel SGX to
     * protect an enclave.
     *
...
typedef union {
  struct {
    /**
     * [Bits 31:0] EN_PMC(n). Enable bitmask. Only the first n-1 bits are valid.
...
#define IA32_PERF_GLOBAL_CTRL_EN_PMCN(_) (((_) >> 0) & 0xFFFFFFFF)
    ...
    /**
     * [Bits 63:32] EN_FIXED_CTR(n). Enable bitmask. Only the first n-1 bits are
...
typedef union {
  struct {
    /**
     * [Bits 31:0] Set 1 to clear Ovf_PMC(n) bit. Clear bitmask. Only the first
...
#define IA32_PERF_GLOBAL_STATUS_RESET_CLEAR_OVF_PMCN(_)                        \
  (((_) >> 0) & 0xFFFFFFFF)
    ...
    /**
     * [Bits 34:32] Set 1 to clear Ovf_FIXED_CTR(n) bit. Clear bitmask. Only the
     * first n-1 bits are valid. Bits 31:n are reserved.
     *
     * @remarks If CPUID.0AH: EDX[4:0] > n
     */
...
typedef union {
  struct {
    /**
     * [Bits 31:0] Set 1 to cause Ovf_PMC(n) = 1. Set bitmask. Only the first
     * n-1 bits are valid. Bits 31:n are reserved.
     *
     * @remarks If CPUID.0AH: EAX[15:8] > n
     */
...
#define IA32_PERF_GLOBAL_STATUS_SET_OVF_PMCN(_) (((_) >> 0) & 0xFFFFFFFF)
    ...
    /**
     * [Bits 34:32] Set 1 to cause Ovf_FIXED_CTR(n) = 1. Set bitmask. Only the
     * first n-1 bits are valid. Bits 31:n are reserved.
     *
     * @remarks If CPUID.0AH: EDX[4:0] > n
     */
...
typedef union {
  struct {
    /**
     * [Bits 31:0] IA32_PERFEVTSEL(n) in use. Status bitmask. Only the first n-1
...
#define IA32_PERF_GLOBAL_INUSE_IA32_PERFEVTSELN_IN_USE(_)                      \
  (((_) >> 0) & 0xFFFFFFFF)
    ...
    /**
     * [Bits 34:32] IA32_FIXED_CTR(n) in use. Status bitmask. Only the first n-1
     * bits are valid. Bits 31:n are reserved.
     */
...
typedef union {
  struct {
     ...
     *
     * [Bits 30:0] 31-bit VMCS revision identifier used by the processor.
     * Processors that use the same VMCS revision identifier use the same size
     * for VMCS regions.
     */
...
#define IA32_VMX_BASIC_MUST_BE_ZERO(_) (((_) >> 31) & 0x01)
     ...
     *
     * [Bits 44:32] Report the number of bytes that software should allocate for
     * the VMXON region and any VMCS region. It is a value greater than 0 and at
     * most 4096 (bit 44 is set if and only if bits 43:32 are clear).
     */
...
#define IA32_VMX_BASIC_VMCS_SIZE_IN_BYTES(_) (((_) >> 32) & 0x1FFF)
     ...
     * processor's physical-address width.2 If the bit is 1, these addresses are
     * limited to 32 bits. This bit is always 0 for processors that support
...
#define IA32_VMX_BASIC_VMCS_PHYSICAL_ADDRESS_WIDTH(_) (((_) >> 48) & 0x01)
     ...
     *
     * [Bit 49] Read as 1, the logical processor supports the dual-monitor
     * treatment of system-management interrupts and system-management mode.
     *
...
#define IA32_VMX_BASIC_DUAL_MONITOR_SUPPORT(_) (((_) >> 49) & 0x01)
     ...
     * structures to map them into the linear-address space. If it does so, it
     * should establish mappings that use the memory type reported bits 53:50 in
     * this MSR. As of this writing, all processors that support VMX operation
     * indicate the write-back type.
     */
    UINT64 MemoryType : 4;
...
#define IA32_VMX_BASIC_MEMORY_TYPE(_) (((_) >> 50) & 0x0F)
    ...
    /**
     * @brief Whether the processor provides additional information for exits
     * due to INS/OUTS
     *
     * [Bit 54] When set to 1, the processor reports information in the VM-exit
     * instruction-information field on VM exits due to execution of the INS and
     * OUTS instructions. This reporting is done only if this bit is read as 1.
     *
...
#define IA32_VMX_BASIC_INS_OUTS_REPORTING(_) (((_) >> 54) & 0x01)
    ...
    /**
     * @brief Whether default 1 bits in control MSRs (pin/proc/exit/entry) may
     * be cleared to 0 and that 'true' control MSRs are supported
     *
     * [Bit 55] Is read as 1 if any VMX controls that default to 1 may be
     * cleared to 0. It also reports support for the VMX capability MSRs
...
typedef union {
  struct {
    /**
     * @brief External interrupts cause VM-exits if set; otherwise dispatched
     * through the guest's IDT
     *
     * [Bit 0] If this control is 1, external interrupts cause VM exits.
     * Otherwise, they are delivered normally through the guest
     * interrupt-descriptor table (IDT). If this control is 1, the value of
...
#define IA32_VMX_PINBASED_CTLS_EXTERNAL_INTERRUPT_EXITING(_) (((_) >> 0) & 0x01)
    ...
    /**
     * @brief Non-maskable interrupts cause VM-exits if set; otherwise
     * dispatched through the guest's IDT
     *
     * [Bit 3] If this control is 1, non-maskable interrupts (NMIs) cause VM
     * exits. Otherwise, they are delivered normally using descriptor 2 of the
     * IDT. This control also determines interactions between IRET and blocking
     * by NMI.
     *
     * @see Vol3C[25.3(CHANGES TO INSTRUCTION BEHAVIOR IN VMX NON-ROOT
     * OPERATION)]
     */
    UINT64 NmiExiting : 1;
...
#define IA32_VMX_PINBASED_CTLS_NMI_EXITING(_) (((_) >> 3) & 0x01)
     ...
     * @brief Virtual NMIs
     *
     * [Bit 5] If this control is 1, NMIs are never blocked and the "blocking by
     * NMI" bit (bit 3) in the interruptibility-state field indicates
     * "virtual-NMI blocking". This control also interacts with the "NMI-window
...
#define IA32_VMX_PINBASED_CTLS_ACTIVATE_VMX_PREEMPTION_TIMER(_)                \
  (((_) >> 6) & 0x01)
     ...
     * posted-interrupt notification vector specially, updating the virtual-APIC
     * page with posted-interrupt requests.
     *
...
#define IA32_VMX_PROCBASED_CTLS_INTERRUPT_WINDOW_EXITING(_) (((_) >> 2) & 0x01)
     ...
     *
     * [Bit 3] This control determines whether executions of RDTSC, executions
     * of RDTSCP, and executions of RDMSR that read from the
     * IA32_TIME_STAMP_COUNTER MSR return a value modified by the TSC offset
     * field.
     *
...
#define IA32_VMX_PROCBASED_CTLS_HLT_EXITING(_) (((_) >> 7) & 0x01)
     ...
     *
     * [Bit 9] This control determines whether executions of INVLPG cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS_INVLPG_EXITING(_) (((_) >> 9) & 0x01)
     ...
     *
     * [Bit 10] This control determines whether executions of MWAIT cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS_MWAIT_EXITING(_) (((_) >> 10) & 0x01)
     ...
     *
     * [Bit 11] This control determines whether executions of RDPMC cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS_RDPMC_EXITING(_) (((_) >> 11) & 0x01)
     ...
     *
     * [Bit 12] This control determines whether executions of RDTSC and RDTSCP
     * cause VM exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS_RDTSC_EXITING(_) (((_) >> 12) & 0x01)
    ...
    /**
     * @brief VM-exit when executing the MOV to CR3 instruction (forced to 1 on
     * the 'first' VT-x capable CPUs; this actually includes the newest Nehalem
     * CPUs)
     *
     * [Bit 15] In conjunction with the CR3-target controls, this control
     * determines whether executions of MOV to CR3 cause VM exits. The first
     * processors to support the virtual-machine extensions supported only the
     * 1-setting of this control.
     *
...
#define IA32_VMX_PROCBASED_CTLS_CR3_LOAD_EXITING(_) (((_) >> 15) & 0x01)
    ...
    /**
     * @brief VM-exit when executing the MOV from CR3 instruction (forced to 1
     * on the 'first' VT-x capable CPUs; this actually includes the newest
     * Nehalem CPUs)
     *
     * [Bit 16] This control determines whether executions of MOV from CR3 cause
     * VM exits. The first processors to support the virtual-machine extensions
...
#define IA32_VMX_PROCBASED_CTLS_CR3_STORE_EXITING(_) (((_) >> 16) & 0x01)
    ...
    /**
     * @brief Determines whether the tertiary processor based VM-execution
     * controls are used
     *
     ...
     * processor operates as if all the tertiary processor-based VM-execution
     * controls were also 0.
     */
...
#define IA32_VMX_PROCBASED_CTLS_ACTIVATE_TERTIARY_CONTROLS(_)                  \
  (((_) >> 17) & 0x01)
    UINT64 Reserved5 : 1;
     ...
     *
     * [Bit 19] This control determines whether executions of MOV to CR8 cause
     * VM exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS_CR8_LOAD_EXITING(_) (((_) >> 19) & 0x01)
     ...
     *
     * [Bit 20] This control determines whether executions of MOV from CR8 cause
     * VM exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS_NMI_WINDOW_EXITING(_) (((_) >> 22) & 0x01)
     ...
     *
     * [Bit 23] This control determines whether executions of MOV DR cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS_MOV_DR_EXITING(_) (((_) >> 23) & 0x01)
     ...
     *
     * [Bit 24] This control determines whether executions of I/O instructions
...
#define IA32_VMX_PROCBASED_CTLS_UNCONDITIONAL_IO_EXITING(_) (((_) >> 24) & 0x01)
     ...
     * executions of I/O instructions For this control, "0" means "do not use
     * I/O bitmaps" and "1" means "use I/O bitmaps." If the I/O bitmaps are
     * used, the setting of the "unconditional I/O exiting" control is ignored.
     *
...
#define IA32_VMX_PROCBASED_CTLS_MONITOR_TRAP_FLAG(_) (((_) >> 27) & 0x01)
     ...
     * execution of the RDMSR and WRMSR instructions. For this control, "0"
     * means "do not use MSR bitmaps" and "1" means "use MSR bitmaps." If the
     * MSR bitmaps are not used, all executions of the RDMSR and WRMSR
...
#define IA32_VMX_PROCBASED_CTLS_USE_MSR_BITMAPS(_) (((_) >> 28) & 0x01)
     ...
     *
     * [Bit 29] This control determines whether executions of MONITOR cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS_MONITOR_EXITING(_) (((_) >> 29) & 0x01)
     ...
     *
     * [Bit 30] This control determines whether executions of PAUSE cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS_PAUSE_EXITING(_) (((_) >> 30) & 0x01)
    ...
    /**
     * @brief Determines whether the secondary processor based VM-execution
     * controls are used
     *
...
typedef union {
  struct {
    ...
    /**
     * @brief Save guest debug controls (dr7 & IA32_DEBUGCTL_MSR) (forced to 1
     * on the 'first' VT-x capable CPUs; this actually includes the newest
     * Nehalem CPUs)
     *
     * [Bit 2] This control determines whether DR7 and the IA32_DEBUGCTL MSR are
     * saved on VM exit. The first processors to support the virtual-machine
...
#define IA32_VMX_EXIT_CTLS_SAVE_DEBUG_CONTROLS(_) (((_) >> 2) & 0x01)
     ...
     *
     * [Bit 9] On processors that support Intel 64 architecture, this control
     * determines whether a logical processor is in 64-bit mode after the next
     * VM exit. Its value is loaded into CS.L, IA32_EFER.LME, and IA32_EFER.LMA
     * on every VM exit.1 This control must be 0 on processors that do not
...
#define IA32_VMX_EXIT_CTLS_HOST_ADDRESS_SPACE_SIZE(_) (((_) >> 9) & 0x01)
     ...
     *
     * [Bit 12] This control determines whether the IA32_PERF_GLOBAL_CTRL MSR is
     * loaded on VM exit.
     */
...
#define IA32_VMX_EXIT_CTLS_LOAD_IA32_PERF_GLOBAL_CTRL(_) (((_) >> 12) & 0x01)
    ...
    /**
     * @brief Acknowledge external interrupts with the irq controller if one
     * caused a VM-exit
     *
     ...
     * acknowledges the interrupt controller, acquiring the interrupt's vector.
     * The vector is stored in the VM-exit interruption-information field, which
     * is marked valid.
     * - If such a VM exit occurs and this control is 0, the interrupt is not
     * acknowledged and the VM-exit interruption-information field is marked
     * invalid.
     */
...
#define IA32_VMX_EXIT_CTLS_ACKNOWLEDGE_INTERRUPT_ON_EXIT(_) (((_) >> 15) & 0x01)
     ...
     *
     * [Bit 18] This control determines whether the IA32_PAT MSR is saved on VM
     * exit.
     */
...
#define IA32_VMX_EXIT_CTLS_SAVE_IA32_PAT(_) (((_) >> 18) & 0x01)
     ...
     *
     * [Bit 19] This control determines whether the IA32_PAT MSR is loaded on VM
     * exit.
     */
...
#define IA32_VMX_EXIT_CTLS_LOAD_IA32_PAT(_) (((_) >> 19) & 0x01)
     ...
     *
     * [Bit 20] This control determines whether the IA32_EFER MSR is saved on VM
     * exit.
     */
...
#define IA32_VMX_EXIT_CTLS_SAVE_IA32_EFER(_) (((_) >> 20) & 0x01)
     ...
     *
     * [Bit 21] This control determines whether the IA32_EFER MSR is loaded on
     * VM exit.
     */
...
#define IA32_VMX_EXIT_CTLS_LOAD_IA32_EFER(_) (((_) >> 21) & 0x01)
    ...
    /**
     * @brief Whether the value of the VMX preemption timer is saved on every
     * VM-exit
     *
     * [Bit 22] This control determines whether the value of the VMX-preemption
...
#define IA32_VMX_EXIT_CTLS_SAVE_VMX_PREEMPTION_TIMER_VALUE(_)                  \
  (((_) >> 22) & 0x01)
    ...
    /**
     * [Bit 23] This control determines whether the IA32_BNDCFGS MSR is cleared
     * on VM exit.
     */
...
#define IA32_VMX_EXIT_CTLS_CLEAR_IA32_BNDCFGS(_) (((_) >> 23) & 0x01)
    ...
    /**
     * [Bit 24] If this control is 1, Intel Processor Trace does not produce a
     * paging information packet (PIP) on a VM exit or a VMCS packet on an SMM
     * VM exit.
     *
...
#define IA32_VMX_EXIT_CTLS_CONCEAL_VMX_FROM_PT(_) (((_) >> 24) & 0x01)
    ...
    /**
     * [Bit 25] This control determines whether the IA32_RTIT_CTL MSR is cleared
     * on VM exit.
     *
     * @see Vol3C[35(INTEL(R) PROCESSOR TRACE)]
     */
...
#define IA32_VMX_EXIT_CTLS_CLEAR_IA32_RTIT_CTL(_) (((_) >> 25) & 0x01)
    ...
    /**
     * [Bit 26] This control determines whether the IA32_LBR_CTL MSR is cleared
     * on VM exit.
     */
...
#define IA32_VMX_EXIT_CTLS_CLEAR_IA32_LBR_CTL(_) (((_) >> 26) & 0x01)
    ...
    /**
     * [Bit 28] This control determines whether CET-related MSRs and SPP are
     * loaded on VM exit.
     *
     * @see Vol1[18(CONTROL-FLOW ENFORCEMENT TECHNOLOGY (CET))]
     */
...
#define IA32_VMX_EXIT_CTLS_LOAD_IA32_CET_STATE(_) (((_) >> 28) & 0x01)
    ...
    /**
     * [Bit 29] This control determines whether the IA32_PKRS MSR is loaded on
     * VM exit.
     */
...
#define IA32_VMX_EXIT_CTLS_LOAD_IA32_PKRS(_) (((_) >> 29) & 0x01)
    ...
    /**
     * [Bit 31] This control determines whether the secondary VM-exit controls
     * are used. If this control is 0, the logical processor operates as if all
     * the secondary VM-exit controls were also 0.
     */
    UINT64 ActivateSecondaryControls : 1;
...
typedef union {
  struct {
    ...
    /**
     * @brief Load guest debug controls (dr7 & IA32_DEBUGCTL_MSR) (forced to 1
     * on the 'first' VT-x capable CPUs; this actually includes the newest
     * Nehalem CPUs)
     *
     * [Bit 2] This control determines whether DR7 and the IA32_DEBUGCTL MSR are
     * loaded on VM entry. The first processors to support the virtual-machine
     * extensions supported only the 1-setting of this control.
     */
...
#define IA32_VMX_ENTRY_CTLS_LOAD_DEBUG_CONTROLS(_) (((_) >> 2) & 0x01)
     ...
     *
     * [Bit 9] On processors that support Intel 64 architecture, this control
     * determines whether the logical processor is in IA-32e mode after VM
     * entry. Its value is loaded into IA32_EFER.LMA as part of VM entry. This
...
#define IA32_VMX_ENTRY_CTLS_IA32E_MODE_GUEST(_) (((_) >> 9) & 0x01)
     ...
     * system-management mode (SMM) after VM entry. This control must be 0 for
...
#define IA32_VMX_ENTRY_CTLS_ENTRY_TO_SMM(_) (((_) >> 10) & 0x01)
    ...
    /**
     * @brief Disable dual treatment of SMI and SMM; must be zero for VM-entry
     * outside of SMM
     *
     * [Bit 11] If set to 1, the default treatment of SMIs and SMM is in effect
     * after the VM entry. This control must be 0 for any VM entry from outside
     * SMM
     *
...
#define IA32_VMX_ENTRY_CTLS_DEACTIVATE_DUAL_MONITOR_TREATMENT(_)               \
  (((_) >> 11) & 0x01)
     ...
     *
     * [Bit 13] This control determines whether the IA32_PERF_GLOBAL_CTRL MSR is
     * loaded on VM entry.
     */
    UINT64 LoadIa32PerfGlobalCtrl : 1;
...
#define IA32_VMX_ENTRY_CTLS_LOAD_IA32_PERF_GLOBAL_CTRL(_) (((_) >> 13) & 0x01)
     ...
     *
     * [Bit 14] This control determines whether the IA32_PAT MSR is loaded on VM
     * entry.
     */
    UINT64 LoadIa32Pat : 1;
...
#define IA32_VMX_ENTRY_CTLS_LOAD_IA32_PAT(_) (((_) >> 14) & 0x01)
     ...
     *
     * [Bit 15] This control determines whether the IA32_EFER MSR is loaded on
     * VM entry.
     */
    UINT64 LoadIa32Efer : 1;
...
#define IA32_VMX_ENTRY_CTLS_LOAD_IA32_EFER(_) (((_) >> 15) & 0x01)
    ...
    /**
     * [Bit 16] This control determines whether the IA32_BNDCFGS MSR is loaded
     * on VM entry.
     */
...
#define IA32_VMX_ENTRY_CTLS_LOAD_IA32_BNDCFGS(_) (((_) >> 16) & 0x01)
    ...
    /**
     * [Bit 17] If this control is 1, Intel Processor Trace does not produce a
     * paging information packet (PIP) on a VM entry or a VMCS packet on a VM
     * entry that returns from SMM.
     *
     * @see Vol3C[35(INTEL(R) PROCESSOR TRACE)]
     */
    UINT64 ConcealVmxFromPt : 1;
...
#define IA32_VMX_ENTRY_CTLS_CONCEAL_VMX_FROM_PT(_) (((_) >> 17) & 0x01)
    ...
    /**
     * [Bit 18] This control determines whether the IA32_RTIT_CTL MSR is loaded
     * on VM entry.
     */
...
#define IA32_VMX_ENTRY_CTLS_LOAD_IA32_RTIT_CTL(_) (((_) >> 18) & 0x01)
    ...
    /**
     * [Bit 20] This control determines whether CET-related MSRs and SPP are
     * loaded on VM entry.
     */
...
#define IA32_VMX_ENTRY_CTLS_LOAD_CET_STATE(_) (((_) >> 20) & 0x01)
    ...
    /**
     * [Bit 21] This control determines whether the IA32_LBR_CTL MSR is loaded
     * on VM entry.
     */
...
#define IA32_VMX_ENTRY_CTLS_LOAD_IA32_LBR_CTL(_) (((_) >> 21) & 0x01)
    ...
    /**
     * [Bit 22] This control determines whether the IA32_PKRS MSR is loaded on
     * VM entry.
     */
    UINT64 LoadIa32Pkrs : 1;
...
typedef union {
  struct {
    /**
     * @brief Relationship between the preemption timer and tsc; count down
     * every time bit x of the tsc changes
     *
     ...
     * Specifically, the VMX-preemption timer (if it is active) counts down by 1
     * every time bit X in the TSC changes due to a TSC increment.
     */
...
#define IA32_VMX_MISC_PREEMPTION_TIMER_TSC_RELATIONSHIP(_) (((_) >> 0) & 0x1F)
     ...
     *
     * [Bit 5] When set to 1, VM exits store the value of IA32_EFER.LMA into the
     * "IA-32e mode guest" VM-entry control. This bit is read as 1 on any
     * logical processor that supports the 1-setting of the "unrestricted guest"
     * VM-execution control.
     *
     * @see Vol3C[27.2(RECORDING VM-EXIT INFORMATION AND UPDATING VM-ENTRY
     * CONTROL FIELDS)]
     */
...
#define IA32_VMX_MISC_STORE_EFER_LMA_ON_VMEXIT(_) (((_) >> 5) & 0x01)
     ...
     * - Bit 8 reports (if set) the support for activity state 3
     * (wait-for-SIPI). If an activity state is not supported, the
     * implementation causes a VM entry to fail if it attempts to establish that
     * activity state. All implementations support VM entry to activity state 0
     * (active).
     */
...
#define IA32_VMX_MISC_ACTIVITY_STATES(_) (((_) >> 6) & 0x07)
     ...
     *
     * [Bit 14] When set to 1, Intel(R) Processor Trace (Intel PT) can be used
     ...
     * IA32_RTIT_CTL.TraceEn; any attempt to write IA32_RTIT_CTL while in VMX
     * operation (including VMX root operation) causes a general-protection
     * exception.
     *
...
#define IA32_VMX_MISC_INTEL_PT_AVAILABLE_IN_VMX(_) (((_) >> 14) & 0x01)
     ...
     *
     * [Bit 15] When set to 1, the RDMSR instruction can be used in
     * system-management mode (SMM) to read the IA32_SMBASE MSR (MSR address
     * 9EH).
     *
...
#define IA32_VMX_MISC_RDMSR_CAN_READ_IA32_SMBASE_MSR_IN_SMM(_)                 \
  (((_) >> 15) & 0x01)
     ...
     * processor. This number is a value between 0 and 256, inclusive (bit 24 is
     * set if and only if bits 23:16 are clear).
     */
...
#define IA32_VMX_MISC_CR3_TARGET_COUNT(_) (((_) >> 16) & 0x1FF)
     ...
     *
     * [Bits 27:25] Used to compute the recommended maximum number of MSRs that
     * should appear in the VM-exit MSR-store list, the VM-exit MSR-load list,
     * or the VM-entry MSR-load list. Specifically, if the value bits 27:25 of
     * IA32_VMX_MISC is N, then 512 * (N + 1) is the recommended maximum number
     * of MSRs to be included in each list. If the limit is exceeded, undefined
     * processor behavior may result (including a machine check during the VMX
     * transition).
     */
...
#define IA32_VMX_MISC_MAX_NUMBER_OF_MSR(_) (((_) >> 25) & 0x07)
     ...
     *
     * [Bit 28] When set to 1, bit 2 of the IA32_SMM_MONITOR_CTL can be set
...
#define IA32_VMX_MISC_SMM_MONITOR_CTL_B2(_) (((_) >> 28) & 0x01)
     ...
     *
     * [Bit 29] When set to 1, software can use VMWRITE to write to any
     * supported field in the VMCS; otherwise, VMWRITE cannot be used to modify
...
#define IA32_VMX_MISC_VMWRITE_VMEXIT_INFO(_) (((_) >> 29) & 0x01)
    ...
    /**
     * [Bit 30] When set to 1, VM entry allows injection of a software
...
#define IA32_VMX_MISC_ZERO_LENGTH_INSTRUCTION_VMENTRY_INJECTION(_)             \
  (((_) >> 30) & 0x01)
     ...
     *
     * [Bits 63:32] Report the 32-bit MSEG revision identifier used by the
     * processor.
     */
    UINT64 MsegId : 32;
...
typedef union {
  struct {
     ...
     *
     * [Bit 0] If this control is 1, the logical processor treats specially
     * accesses to the page with the APICaccess address.
     *
...
#define IA32_VMX_PROCBASED_CTLS2_ENABLE_EPT(_) (((_) >> 1) & 0x01)
     ...
     *
     * [Bit 2] This control determines whether executions of LGDT, LIDT, LLDT,
...
#define IA32_VMX_PROCBASED_CTLS2_DESCRIPTOR_TABLE_EXITING(_) (((_) >> 2) & 0x01)
     ...
     *
     * [Bit 3] If this control is 0, any execution of RDTSCP causes an
     * invalid-opcode exception (\#UD).
     */
...
#define IA32_VMX_PROCBASED_CTLS2_ENABLE_RDTSCP(_) (((_) >> 3) & 0x01)
     ...
     *
     * [Bit 4] If this control is 1, the logical processor treats specially
     * RDMSR and WRMSR to APIC MSRs (in the range 800H-8FFH).
     *
...
#define IA32_VMX_PROCBASED_CTLS2_ENABLE_VPID(_) (((_) >> 5) & 0x01)
     ...
     *
     * [Bit 6] This control determines whether executions of WBINVD cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS2_UNRESTRICTED_GUEST(_) (((_) >> 7) & 0x01)
     ...
     *
     * [Bit 8] If this control is 1, the logical processor virtualizes certain
     * APIC accesses.
     *
     * @see Vol3C[29.4(VIRTUALIZING MEMORY-MAPPED APIC ACCESSES)]
     * @see Vol3C[29.5(VIRTUALIZING MSR-BASED APIC ACCESSES)]
...
#define IA32_VMX_PROCBASED_CTLS2_APIC_REGISTER_VIRTUALIZATION(_)               \
  (((_) >> 8) & 0x01)
     ...
     *
     * [Bit 9] This controls enables the evaluation and delivery of pending
     * virtual interrupts as well as the emulation of writes to the APIC
     * registers that control interrupt prioritization.
     */
...
#define IA32_VMX_PROCBASED_CTLS2_VIRTUAL_INTERRUPT_DELIVERY(_)                 \
  (((_) >> 9) & 0x01)
     ...
     *
     * [Bit 10] This control determines whether a series of executions of PAUSE
     * can cause a VM exit.
     *
...
#define IA32_VMX_PROCBASED_CTLS2_PAUSE_LOOP_EXITING(_) (((_) >> 10) & 0x01)
     ...
     *
     * [Bit 11] This control determines whether executions of RDRAND cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS2_ENABLE_INVPCID(_) (((_) >> 12) & 0x01)
     ...
     *
     * [Bit 13] Setting this control to 1 enables use of the VMFUNC instruction
...
#define IA32_VMX_PROCBASED_CTLS2_ENABLE_VM_FUNCTIONS(_) (((_) >> 13) & 0x01)
     ...
     *
     * [Bit 14] If this control is 1, executions of VMREAD and VMWRITE in VMX
     * non-root operation may access a shadow VMCS (instead of causing VM
     * exits).
     *
...
#define IA32_VMX_PROCBASED_CTLS2_VMCS_SHADOWING(_) (((_) >> 14) & 0x01)
     ...
     *
     * [Bit 15] If this control is 1, executions of ENCLS consult the
     * ENCLS-exiting bitmap to determine whether the instruction causes a VM
     * exit.
     *
...
#define IA32_VMX_PROCBASED_CTLS2_ENABLE_ENCLS_EXITING(_) (((_) >> 15) & 0x01)
     ...
     *
     * [Bit 16] This control determines whether executions of RDSEED cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS2_RDSEED_EXITING(_) (((_) >> 16) & 0x01)
     ...
     *
     * [Bit 17] If this control is 1, an access to a guest-physical address that
     * sets an EPT dirty bit first adds an entry to the page-modification log.
     *
...
#define IA32_VMX_PROCBASED_CTLS2_ENABLE_PML(_) (((_) >> 17) & 0x01)
     ...
     *
     * [Bit 18] If this control is 1, EPT violations may cause virtualization
     * exceptions (\#VE) instead of VM exits.
     *
...
#define IA32_VMX_PROCBASED_CTLS2_EPT_VIOLATION(_) (((_) >> 18) & 0x01)
     ...
     *
     * [Bit 19] If this control is 1, Intel Processor Trace suppresses from PIPs
     * an indication that the processor was in VMX non-root operation and omits
     * a VMCS packet from any PSB+ produced in VMX nonroot operation.
     *
     * @see Vol3C[35(INTEL(R) PROCESSOR TRACE)]
     */
    UINT64 ConcealVmxFromPt : 1;
...
#define IA32_VMX_PROCBASED_CTLS2_CONCEAL_VMX_FROM_PT(_) (((_) >> 19) & 0x01)
     ...
     *
     * [Bit 20] If this control is 0, any execution of XSAVES or XRSTORS causes
     * a \#UD.
     */
...
#define IA32_VMX_PROCBASED_CTLS2_ENABLE_XSAVES(_) (((_) >> 20) & 0x01)
    ...
    /**
     * [Bit 22] If this control is 1, EPT execute permissions are based on
     * whether the linear address being accessed is supervisor mode or user
     * mode.
     *
...
#define IA32_VMX_PROCBASED_CTLS2_MODE_BASED_EXECUTE_CONTROL_FOR_EPT(_)         \
  (((_) >> 22) & 0x01)
    ...
    /**
     * [Bit 23] If this control is 1, EPT write permissions may be specified at
     * the granularity of 128 bytes.
     *
...
#define IA32_VMX_PROCBASED_CTLS2_SUB_PAGE_WRITE_PERMISSIONS_FOR_EPT_FLAG       \
  0x800000
#define IA32_VMX_PROCBASED_CTLS2_SUB_PAGE_WRITE_PERMISSIONS_FOR_EPT_MASK 0x01
#define IA32_VMX_PROCBASED_CTLS2_SUB_PAGE_WRITE_PERMISSIONS_FOR_EPT(_)         \
  (((_) >> 23) & 0x01)

    /**
     * [Bit 24] If this control is 1, all output addresses used by Intel
     * Processor Trace are treated as guestphysical addresses and translated
     * using EPT.
     *
...
#define IA32_VMX_PROCBASED_CTLS2_PT_USES_GUEST_PHYSICAL_ADDRESSES(_)           \
  (((_) >> 24) & 0x01)

    /**
     * @brief Use TSC scaling
     *
     * [Bit 25] This control determines whether executions of RDTSC, executions
     * of RDTSCP, and executions of RDMSR that read from the
     * IA32_TIME_STAMP_COUNTER MSR return a value modified by the TSC multiplier
     * field.
     *
     * @see Vol3C[24.6.5(Time-Stamp Counter Offset and Multiplier)]
     * @see Vol3C[25.3(CHANGES TO INSTRUCTION BEHAVIOR IN VMX NON-ROOT
...
#define IA32_VMX_PROCBASED_CTLS2_USE_TSC_SCALING(_) (((_) >> 25) & 0x01)
     ...
     *
     * [Bit 26] If this control is 0, any execution of TPAUSE, UMONITOR, or
     * UMWAIT causes a \#UD.
     */
...
#define IA32_VMX_PROCBASED_CTLS2_ENABLE_USER_WAIT_PAUSE(_) (((_) >> 26) & 0x01)
     ...
     *
     * [Bit 28] If this control is 1, executions of ENCLV consult the
     * ENCLV-exiting bitmap to determine whether the instruction causes a VM
     * exit.
     *
 ...
 *
 * @remarks If ( CPUID.01H:ECX.[5] && IA32_VMX_PROCBASED_CTLS[63] &&
 * (IA32_VMX_PROCBASED_CTLS2[33] || IA32_VMX_PROCBASED_CTLS2[37]) )
...
typedef union {
  struct {
    /**
     * [Bit 0] When set to 1, the processor supports execute-only translations
     * by EPT. This support allows software to configure EPT paging-structure
     * entries in which bits 1:0 are clear (indicating that data accesses are
     * not allowed) and bit 2 is set (indicating that instruction fetches are
     * allowed).
     */
...
#define IA32_VMX_EPT_VPID_CAP_PAGE_WALK_LENGTH_4(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 8] When set to 1, the logical processor allows software to configure
     * the EPT paging-structure memory type to be uncacheable (UC).
     *
...
#define IA32_VMX_EPT_VPID_CAP_MEMORY_TYPE_UNCACHEABLE(_) (((_) >> 8) & 0x01)
    ...
    /**
     * [Bit 14] When set to 1, the logical processor allows software to
...
#define IA32_VMX_EPT_VPID_CAP_MEMORY_TYPE_WRITE_BACK(_) (((_) >> 14) & 0x01)
    ...
    /**
     * [Bit 16] When set to 1, the logical processor allows software to
     * configure a EPT PDE to map a 2-Mbyte page (by setting bit 7 in the EPT
     * PDE).
     */
...
#define IA32_VMX_EPT_VPID_CAP_PDE_2MB_PAGES(_) (((_) >> 16) & 0x01)
    ...
    /**
     * [Bit 17] When set to 1, the logical processor allows software to
     * configure a EPT PDPTE to map a 1-Gbyte page (by setting bit 7 in the EPT
     * PDPTE).
     */
...
#define IA32_VMX_EPT_VPID_CAP_EPT_ACCESSED_AND_DIRTY_FLAGS(_)                  \
  (((_) >> 21) & 0x01)
    ...
    /**
     * [Bit 22] When set to 1, the processor reports advanced VM-exit
     * information for EPT violations. This reporting is done only if this bit
     * is read as 1.
     *
...
#define IA32_VMX_EPT_VPID_CAP_ADVANCED_VMEXIT_EPT_VIOLATIONS_INFORMATION(_)    \
  (((_) >> 22) & 0x01)
    ...
    /**
     * [Bit 23] If bit 23 is read as 1, supervisor shadow-stack control is
     * supported.
     *
...
#define IA32_VMX_EPT_VPID_CAP_INVVPID_ALL_CONTEXTS(_) (((_) >> 42) & 0x01)
    ...
    /**
     * [Bit 43] When set to 1, the single-context-retaining-globals INVVPID type
     * is supported.
     */
...
#define IA32_VMX_EPT_VPID_CAP_INVVPID_SINGLE_CONTEXT_RETAIN_GLOBALS(_)         \
  (((_) >> 43) & 0x01)
    ...
    /**
     * [Bits 53:48] Enumerate the maximum HLAT prefix size. It is expected that
     * any processor that supports the 1-setting of the "enable HLAT"
     * VM-execution control will enumerate this value as 1.
     *
 ...
 * Primary Processor-Based VM-Execution Flex Controls, VM-Exit Flex Controls and
...
typedef union {
  struct {
     ...
     * allows control X to be 0 if bit X in the MSR is cleared to 0; if bit X in
     * the MSR is set to 1, VM entry fails if control X is 0.
     */
...
#define IA32_VMX_TRUE_CTLS_ALLOWED_0_SETTINGS(_) (((_) >> 0) & 0xFFFFFFFF)
     ...
     * allows control X to be 1 if bit 32+X in the MSR is set to 1; if bit 32+X
     * in the MSR is cleared to 0, VM entry fails if control X is 1.
     */
...
typedef union {
  struct {
    /**
     * [Bit 0] The EPTP-switching VM function changes the EPT pointer to a value
     * chosen from the EPTP list.
     *
...
/**
 * Capability Reporting Register of Tertiary Processor-Based VM-Execution
 * Controls.
 *
...
typedef union {
  struct {
     ...
     *
     * [Bit 0] This control determines whether executions of LOADIWKEY cause VM
     * exits.
     */
...
#define IA32_VMX_PROCBASED_CTLS3_LOADIWKEY_EXITING(_) (((_) >> 0) & 0x01)
     ...
     *
     * [Bit 1] This control enables hypervisor-managed linear-address
     * translation.
     *
     * @see Vol3A[4.5.1(Ordinary Paging and HLAT Paging)]
     */
...
#define IA32_VMX_PROCBASED_CTLS3_ENABLE_HLAT(_) (((_) >> 1) & 0x01)
    ...
    /**
     * @brief If this control is 1, EPT permissions can be specified to allow
     * writes only for paging-related control updates
     *
     * [Bit 2] If this control is 1, EPT permissions can be specified to allow
     * writes only for paging-related control updates.
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
...
#define IA32_VMX_PROCBASED_CTLS3_EPT_PAGING_WRITE(_) (((_) >> 2) & 0x01)
     ...
     * accesses using linear addresses verification whose translation has
     * certain properties.
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
 ...
 *
 * Intel SGX only allows launching ACMs with an Intel SGX SVN that is at the
 * same level or higher than the expected Intel SGX SVN. The expected Intel SGX
 * SVN is specified by BIOS and locked down by the processor on the first
 * successful execution of an Intel SGX instruction that doesn't return an error
 * code. Intel SGX provides interfaces for system software to discover whether a
 * non faulting Intel SGX instruction has been executed, and evaluate the
 * suitability of the Intel SGX SVN value of any ACM that is expected to be
 * launched by the OS or the VMM.
 *
 * @remarks If CPUID.(EAX=07H, ECX=0H): EBX[2] = 1
...
typedef union {
  struct {
    /**
     * [Bit 0] - If 1, indicates that a non-faulting Intel SGX instruction has
     * been executed, consequently, launching a properly signed ACM but with
     * Intel SGX SVN value less than the BIOS specified Intel SGX SVN threshold
...
typedef union {
  struct {
     ...
     *
     * [Bits 47:7] The base physical address. How this address is used depends
     * on the value of IA32_RTIT_CTL.ToPA:
     * - 0: This is the base physical address of a single, contiguous physical
     * output region. This could be mapped to DRAM or to MMIO, depending on the
     * value. The base address should be aligned with the size of the region,
     * such that none of the 1s in the mask value overlap with 1s in the base
     * address. If the base is not aligned, an operational error will result.
     * - 1: The base physical address of the current ToPA table. The address
     * must be 4K aligned. Writing an address in which bits 11:7 are non-zero
     * will not cause a \#GP, but an operational error will be signaled once
     * TraceEn is set.
     *
...
#define IA32_RTIT_OUTPUT_MASK_PTRS_LOWER_MASK(_) (((_) >> 0) & 0x7F)
     ...
     *
     * [Bits 31:7] The use of this field depends on the value of
     * IA32_RTIT_CTL.ToPA:
     * - 0: This field holds bits 31:7 of the mask value for the single,
     * contiguous physical output region. The size of this field indicates that
     * regions can be of size 128B up to 4GB. This value (combined with the
     * lower 7 bits, which are reserved to 1) will be ANDed with the
     * OutputOffset field to determine the next write address. All 1s in this
     * field should be consecutive and starting at bit 7, otherwise the region
     * will not be contiguous, and an operational error will be signaled when
     * TraceEn is set.
     * - 1: This field holds bits 27:3 of the offset pointer into the current
     * ToPA table. This value can be added to the IA32_RTIT_OUTPUT_BASE value to
     * produce a pointer to the current ToPA table entry, which itself is a
     * pointer to the current output region. In this scenario, the lower 7
     * reserved bits are ignored. This field supports tables up to 256 MBytes in
     * size.
     *
     * @see Vol3C[35.3.9(Operational Errors)]
     */
...
#define IA32_RTIT_OUTPUT_MASK_PTRS_MASK_OR_TABLE_OFFSET(_)                     \
  (((_) >> 7) & 0x1FFFFFF)
     ...
     * @brief Output Offset
     *
     * [Bits 63:32] The use of this field depends on the value of
     * IA32_RTIT_CTL.ToPA:
     * - 0: This is bits 31:0 of the offset pointer into the single, contiguous
     * physical output region. This value will be added to the
     * IA32_RTIT_OUTPUT_BASE value to form the physical address at which the
     * next byte of packet output data will be written. This value must be less
     * than or equal to the MaskOrTableOffset field, otherwise an operational
     * error will be signaled when TraceEn is set.
     * - 1: This field holds bits 31:0 of the offset pointer into the current
     * ToPA output region. This value will be added to the output region base
     * field, found in the current ToPA table entry, to form the physical
     * address at which the next byte of trace output data will be written. This
     * value must be less than the ToPA entry size, otherwise an operational
     * error will be signaled when TraceEn is set.
     *
     * @see Vol3C[35.3.9(Operational Errors)]
     */
...
typedef union {
  struct {
     ...
     * When this bit transitions from 1 to 0, all buffered packets are flushed
     * out of internal buffers. A further store, fence, or architecturally
     * serializing instruction may be required to ensure that packet data can be
     * observed at the trace endpoint. Note that the processor will clear this
     * bit on \#SMI (Section) and warm reset. Other MSR bits of IA32_RTIT_CTL
     * (and other trace configuration MSRs) are not impacted by these events.
     *
     * @see Vol3C[35.2.7.3(Enabling and Disabling Packet Generation with
     * TraceEn)]
     */
...
#define IA32_RTIT_CTL_FUP_ON_PTW(_) (((_) >> 5) & 0x01)
     ...
     * @brief FabricEn
     *
     * [Bit 6] - 0: Trace output is directed to the memory subsystem, mechanism
     * depends on IA32_RTIT_CTL.ToPA.
     * - 1: Trace output is directed to the trace transport subsystem,
     * IA32_RTIT_CTL.ToPA is ignored.
     *
     * @remarks If (CPUID.(EAX=07H, ECX=0):ECX[3] = 1) Reserved if
...
#define IA32_RTIT_CTL_CR3_FILTER(_) (((_) >> 7) & 0x01)
     ...
     * IA32_RTIT_CTL.FabricEn=0 1: If CPUID.(EAX=14H, ECX=0):ECX.TOPA[bit 0] =
     * 1, and IA32_RTIT_CTL.FabricEn=0 WRMSR to IA32_RTIT_CTL that sets TraceEn
     * but clears this bit and FabricEn would cause \#GP: If CPUID.(EAX=14H,
     *          ECX=0):ECX.SNGLRGNOUT[bit 2] = 0 WRMSR to IA32_RTIT_CTL that
     * sets this bit causes \#GP: If CPUID.(EAX=14H, ECX=0):ECX.TOPA[bit 0] = 0
     * @see Vol3C[35.2.6.2(Table of Physical Addresses (ToPA))]
     */
    UINT64 Topa : 1;
...
#define IA32_RTIT_CTL_TOPA(_) (((_) >> 8) & 0x01)
     ...
     *
     * @remarks If (CPUID.(EAX=07H, ECX=0):EBX[3] = 1) Reserved if
...
#define IA32_RTIT_CTL_PTW_ENABLED(_) (((_) >> 12) & 0x01)
     ...
     * - 1: Enable COFI-based packets: FUP, TIP, TIP.PGE, TIP.PGD, TNT,
     * MODE.Exec, MODE.TSX.
     *
...
#define IA32_RTIT_CTL_BRANCH_ENABLED(_) (((_) >> 13) & 0x01)
     ...
     * @brief MTCFreq
     *
     * [Bits 17:14] Defines MTC packet Frequency, which is based on the core
     * crystal clock, or Always Running Timer (ART). MTC will be sent each time
     * the selected ART bit toggles. The following Encodings are defined: 0:
     * ART(0), 1: ART(1), 2: ART(2), 3: ART(3), 4: ART(4), 5: ART(5), 6: ART(6),
     * 7: ART(7), 8: ART(8), 9: ART(9), 10: ART(10), 11: ART(11), 12: ART(12),
     * 13: ART(13), 14: ART(14), 15: ART(15)
     *
     * @remarks If (CPUID.(EAX=07H, ECX=0):EBX[3] = 1) Reserved if
     * CPUID.(EAX=14H, ECX=0):EBX.MTC[bit 3] = 0
     * @see Vol3C[35.3.1(Detection of Intel Processor Trace and Capability
     * Enumeration)]
     */
...
#define IA32_RTIT_CTL_MTC_FREQUENCY(_) (((_) >> 14) & 0x0F)
     ...
     * @brief CYCThresh
     *
     * [Bits 22:19] CYC packet threshold. CYC packets will be sent with the
     * first eligible packet after N cycles have passed since the last CYC
     * packet. If CycThresh is 0 then N=0, otherwise N is defined as
     * 2(CycThresh-1). The following Encodings are defined: 0: 0, 1: 1, 2: 2, 3:
     * 4, 4: 8, 5: 16, 6: 32, 7: 64, 8: 128, 9: 256, 10: 512, 11: 1024, 12:
     * 2048, 13: 4096, 14: 8192, 15: 16384
     *
     * @remarks If (CPUID.(EAX=07H, ECX=0):EBX[1] = 1) Reserved if
...
#define IA32_RTIT_CTL_CYC_THRESHOLD(_) (((_) >> 19) & 0x0F)
     ...
     * @brief PSBFreq
     *
     * [Bits 27:24] Indicates the frequency of PSB packets. PSB packet frequency
     * is based on the number of Intel PT packet bytes output, so this field
     * allows the user to determine the increment of
     * IA32_IA32_RTIT_STATUS.PacketByteCnt that should cause a PSB to be
     * generated. Note that PSB insertion is not precise, but the average output
     * bytes per PSB should approximate the SW selected period. The following
     * Encodings are defined: 0: 2K, 1: 4K, 2: 8K, 3: 16K, 4: 32K, 5: 64K, 6:
     * 128K, 7: 256K, 8: 512K, 9: 1M, 10: 2M, 11: 4M, 12: 8M, 13: 16M, 14: 32M,
     * 15: 64M
     *
     * @remarks If (CPUID.(EAX=07H, ECX=0):EBX[1] = 1) Reserved if
     * CPUID.(EAX=14H, ECX=0):EBX.CPSB_CAM[bit 1] = 0
...
#define IA32_RTIT_CTL_PSB_FREQUENCY(_) (((_) >> 24) & 0x0F)
     ...
     * @brief ADDR0_CFG
     *
     * [Bits 35:32] Configures the base/limit register pair IA32_RTIT_ADDR0_A/B
     * based on the following encodings:
     * - 0: ADDR0 range unused.
     * - 1: The [IA32_RTIT_ADDR0_A..IA32_RTIT_ADDR0_B] range defines a FilterEn
     * range. FilterEn will only be set when the IP is within this range, though
     * other FilterEn ranges can additionally be used.
     * - 2: The [IA32_RTIT_ADDR0_A..IA32_RTIT_ADDR0_B] range defines a TraceStop
     * range. TraceStop will be asserted if code branches into this range.
     * - 3..15: Reserved (\#GP).
     *
     * @remarks If (CPUID.(EAX=07H, ECX=1):EAX[2:0] > 0) Reserved if
     * CPUID.(EAX=14H, ECX=1):EBX.RANGECNT[2:0] >= 0
...
#define IA32_RTIT_CTL_ADDR0_CFG(_) (((_) >> 32) & 0x0F)
     ...
     * @brief ADDR1_CFG
     *
     * [Bits 39:36] Configures the base/limit register pair IA32_RTIT_ADDR1_A/B
     * based on the following encodings:
     * - 0: ADDR1 range unused.
     * - 1: The [IA32_RTIT_ADDR1_A..IA32_RTIT_ADDR1_B] range defines a FilterEn
     * range. FilterEn will only be set when the IP is within this range, though
     * other FilterEn ranges can additionally be used.
     * - 2: The [IA32_RTIT_ADDR1_A..IA32_RTIT_ADDR1_B] range defines a TraceStop
     * range. TraceStop will be asserted if code branches into this range.
     * - 3..15: Reserved (\#GP).
     *
     * @remarks If (CPUID.(EAX=07H, ECX=1):EAX[2:0] > 1) Reserved if
     * CPUID.(EAX=14H, ECX=1):EBX.RANGECNT[2:0] < 2
     * @see Vol3C[35.2.4.3(Filtering by IP)]
     * @see Vol3C[35.4.2.10(Core:Bus Ratio (CBR) Packet)]
     */
    UINT64 Addr1Cfg : 4;
...
#define IA32_RTIT_CTL_ADDR1_CFG(_) (((_) >> 36) & 0x0F)
     ...
     * @brief ADDR2_CFG
     *
     * [Bits 43:40] Configures the base/limit register pair IA32_RTIT_ADDR2_A/B
     * based on the following encodings:
     * - 0: ADDR2 range unused.
     * - 1: The [IA32_RTIT_ADDR2_A..IA32_RTIT_ADDR2_B] range defines a FilterEn
     * range. FilterEn will only be set when the IP is within this range, though
     * other FilterEn ranges can additionally be used.
     * - 2: The [IA32_RTIT_ADDR2_A..IA32_RTIT_ADDR2_B] range defines a TraceStop
     * range. TraceStop will be asserted if code branches into this range.
     * - 3..15: Reserved (\#GP).
     *
     * @remarks If (CPUID.(EAX=07H, ECX=1):EAX[2:0] > 2) Reserved if
     * CPUID.(EAX=14H, ECX=1):EBX.RANGECNT[2:0] < 3
     * @see Vol3C[35.2.4.3(Filtering by IP)]
     * @see Vol3C[35.4.2.10(Core:Bus Ratio (CBR) Packet)]
     */
    UINT64 Addr2Cfg : 4;
...
#define IA32_RTIT_CTL_ADDR2_CFG(_) (((_) >> 40) & 0x0F)
     ...
     * @brief ADDR3_CFG
     *
     * [Bits 47:44] Configures the base/limit register pair IA32_RTIT_ADDR3_A/B
     * based on the following encodings:
     * - 0: ADDR3 range unused.
     * - 1: The [IA32_RTIT_ADDR3_A..IA32_RTIT_ADDR3_B] range defines a FilterEn
     * range. FilterEn will only be set when the IP is within this range, though
     * other FilterEn ranges can additionally be used.
     * - 2: The [IA32_RTIT_ADDR3_A..IA32_RTIT_ADDR3_B] range defines a TraceStop
     * range. TraceStop will be asserted if code branches into this range.
     * - 3..15: Reserved (\#GP).
     *
     * @remarks If (CPUID.(EAX=07H, ECX=1):EAX[2:0] > 3) Reserved if
     * CPUID.(EAX=14H, ECX=1):EBX.RANGECNT[2:0] < 4
     * @see Vol3C[35.2.4.3(Filtering by IP)]
     * @see Vol3C[35.4.2.10(Core:Bus Ratio (CBR) Packet)]
     */
    UINT64 Addr3Cfg : 4;
...
#define IA32_RTIT_CTL_ADDR3_CFG(_) (((_) >> 44) & 0x0F)
     ...
     *
     * [Bit 56] - 1: Enables use of IA32_RTIT_STATUS bits PendPSB[6] and
     * PendTopaPMI[7].
...
typedef union {
  struct {
     ...
     *
     * [Bit 0] This bit is written by the processor, and indicates that tracing
     * is allowed for the current IP. Writes are ignored.
     *
     * @remarks If (CPUID.(EAX=07H, ECX=0):EBX[2] = 1)
...
#define IA32_RTIT_STATUS_FILTER_ENABLED(_) (((_) >> 0) & 0x01)
     ...
     *
     * [Bit 1] The processor sets this bit to indicate that tracing is allowed
     * for the current context. Writes are ignored.
     *
...
#define IA32_RTIT_STATUS_CONTEXT_ENABLED(_) (((_) >> 1) & 0x01)
     ...
     *
     * [Bit 2] The processor sets this bit to indicate that tracing is enabled.
     * Writes are ignored.
     *
...
#define IA32_RTIT_STATUS_TRIGGER_ENABLED(_) (((_) >> 2) & 0x01)
     ...
     * @brief Error
     *
     * [Bit 4] The processor sets this bit to indicate that an operational error
     * has been encountered. When this bit is set, TriggerEn is cleared to 0 and
     * packet generation is disabled. When TraceEn is cleared, software can
     * write this bit. Once it is set, only software can clear it. It is not
     * recommended that software ever set this bit, except in cases where it is
     * restoring a prior saved state.
     *
...
#define IA32_RTIT_STATUS_ERROR(_) (((_) >> 4) & 0x01)
     ...
     * @brief Stopped
     *
     * [Bit 5] The processor sets this bit to indicate that a ToPA Stop
     * condition has been encountered. When this bit is set, TriggerEn is
     * cleared to 0 and packet generation is disabled. When TraceEn is cleared,
     * software can write this bit. Once it is set, only software can clear it.
     * It is not recommended that software ever set this bit, except in cases
     * where it is restoring a prior saved state.
     *
...
#define IA32_RTIT_STATUS_STOPPED(_) (((_) >> 5) & 0x01)
     ...
     * @brief Pend PSB
     *
     * [Bit 6] If IA32_RTIT_CTL.InjectPsbPmiOnEnable[56] = 1, the processor sets
     * this bit when the threshold for a PSB+ to be inserted has been reached.
     * The processor will clear this bit when the PSB+ has been inserted into
     * the trace. If PendPSB = 1 and InjectPsbPmiOnEnable = 1 when
     * IA32_RTIT_CTL.TraceEn[0] transitions from 0 to 1, a PSB+ will be inserted
     * into the trace.
     *
...
#define IA32_RTIT_STATUS_PEND_PSB(_) (((_) >> 6) & 0x01)
     ...
     * @brief Pend ToPA PMI
     *
     * [Bit 7] If IA32_RTIT_CTL.InjectPsbPmiOnEnable[56] = 1, the processor sets
     * this bit when the threshold for a ToPA PMI to be inserted has been
     * reached. Software should clear this bit once the ToPA PMI has been
     * handled. If PendTopaPMI = 1 and InjectPsbPmiOnEnable = 1 when
...
#define IA32_RTIT_STATUS_PEND_TOPA_PMI(_) (((_) >> 7) & 0x01)
     ...
     * @brief PacketByteCnt
     *
     * [Bits 48:32] This field is written by the processor, and holds a count of
     * packet bytes that have been sent out. The processor also uses this field
     * to determine when the next PSB packet should be inserted. Note that the
     * processor may clear or modify this field at any time while
     * IA32_RTIT_CTL.TraceEn=1. It will have a stable value when
 ...
 *
 * The IA32_RTIT_CR3_MATCH register is compared against CR3 when
 * IA32_RTIT_CTL.CR3Filter is 1. Bits 63:5 hold the CR3 address value to match,
 * bits 4:0 are reserved to 0.
 *
 * @remarks If (CPUID.(EAX=07H, ECX=0):EBX[25] = 1)
 ...
 *           IA32_RTIT_ADDR(x)
 *
 * The role of the IA32_RTIT_ADDRn_A/B register pairs, for each n, is determined
 * by the corresponding ADDRn_CFG fields in IA32_RTIT_CTL. The number of these
 * register pairs is enumerated by CPUID.(EAX=14H, ECX=1):EAX.RANGECNT[2:0].
 *
 * @remarks If (CPUID.(EAX=07H, ECX=1):EAX[2:0] > n)
 * @see Vol3C[35.2.7.2(IA32_RTIT_CTL MSR)]
 * @see Vol3C[35.2.7.5(IA32_RTIT_ADDRn_A and IA32_RTIT_ADDRn_B MSRs)]
 * (reference)
 * @{
 */
/**
...
/**
 * DS Save Area. Points to the linear address of the first byte of the DS buffer
 * management area, which is used to manage the BTS and PEBS buffers. Returns:
 * - [63:0] The linear address of the first byte of the DS buffer management
 * area, if IA-32e mode is active.
 * - [31:0] The linear address of the first byte of the DS buffer management
 * area, if not in IA-32e mode.
 ...
 *
 * @remarks - Bits 1:0 are defined if CPUID.(EAX=07H,ECX=0H):ECX.CET_SS[07] = 1.
 * - Bits 5:2 and bits 63:10 are defined if
...
#define IA32_U_CET_ENDBR_EN(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] Enable legacy compatibility treatment for indirect branch
     * tracking.
     *
     * @see Vol1[18(CONTROL-FLOW ENFORCEMENT TECHNOLOGY (CET))]
     */
...
#define IA32_U_CET_NO_TRACK_EN(_) (((_) >> 4) & 0x01)
    ...
    /**
     * [Bit 5] When set to 1, disables suppression of CET indirect branch
     * tracking on legacy compatibility.
     *
     * @see Vol1[18(CONTROL-FLOW ENFORCEMENT TECHNOLOGY (CET))]
     */
...
#define IA32_U_CET_SUPPRESS_DIS(_) (((_) >> 5) & 0x01)
    ...
    /**
     * [Bit 10] When set to 1, indirect branch tracking is suppressed. This bit
...
#define IA32_U_CET_SUPPRESS(_) (((_) >> 10) & 0x01)
    ...
    /**
     * [Bit 11] Value of the indirect branch tracking state machine. Values:
...
#define IA32_U_CET_TRACKER(_) (((_) >> 11) & 0x01)
    ...
    /**
     * [Bits 63:12] Linear address bits 63:12 of a legacy code page bitmap used
     * for legacy compatibility when indirect branch tracking is enabled. If the
     * processor does not support Intel 64 architecture, these fields have only
     * 32 bits; bits 63:32 of the MSRs are reserved. On processors that support
     * Intel 64 architecture this value cannot represent a non-canonical
     * address. In protected mode, only 31:0 are loaded. The linear address
     * written must be aligned to 8 bytes and bits 2:0 must be 0 (hardware
     * requires bits 1:0 to be 0).
     *
     * @see Vol1[18(CONTROL-FLOW ENFORCEMENT TECHNOLOGY (CET))]
     */
 ...
 * only 32 bits; bits 63:32 of the MSRs are reserved. On processors that support
 * Intel 64 architecture this value cannot represent a non-canonical address. In
 * protected mode, only 31:0 are loaded. The linear address written must be
 * aligned to 8 bytes and bits 2:0 must be 0 (hardware requires bits 1:0 to be
 * 0).
 *
...
/**
 * Linear address of a table of seven shadow stack pointers that are selected in
 * IA-32e mode using the IST index (when not 0) from the interrupt gate
 * descriptor. This MSR is not present on processors that do not support Intel
 * 64 architecture. This field cannot represent a non-canonical address.
 *
 * @remarks If CPUID.(EAX=07H, ECX=0H):ECX.CET_SS[07] = 1
 */
...
#define IA32_DEBUG_INTERFACE_ENABLE(_) (((_) >> 0) & 0x01)
     ...
     * automatically on the first SMI assertion even if not explicitly set by
     * BIOS. Default is 0.
     *
     * @remarks If CPUID.01H:ECX.[11] = 1
     */
...
#define IA32_DEBUG_INTERFACE_LOCK(_) (((_) >> 30) & 0x01)
     ...
     *
     * [Bit 31] This "sticky bit" is set by hardware to indicate the status of
     * bit 0. Default is 0.
     *
     * @remarks If CPUID.01H:ECX.[11] = 1
     */
...
#define IA32_QM_EVTSEL_EVENT_ID(_) (((_) >> 0) & 0xFF)
     ...
     *
     * [Bits 63:32] ID for monitoring hardware to report monitored data via
     * IA32_QM_CTR.
     *
     * @remarks Bits [N+31:32] N = Ceil (Log2 (CPUID.(EAX= 0FH,ECX=0H).EBX[31:0]
     * + 1))
     */
...
#define IA32_QM_CTR_RESOURCE_MONITORED_DATA(_) (((_) >> 0) & 0x3FFFFFFFFFFFFFFF)
     ...
     * @brief Unavailable
     *
     * [Bit 62] If 1, indicates data for this RMID is not available or not
 ...
 *
 * @remarks If ( (CPUID.(EAX=07H, ECX=0):EBX[12] = 1) or (CPUID.(EAX=07H,
 * ECX=0):EBX[15] = 1 ) )
 */
...
typedef union {
  struct {
     ...
     *
     * [Bits 31:0] ID for monitoring hardware to track internal operation, e.g.,
     * memory access.
     *
     * @remarks Bits [N-1:0] N = Ceil (Log2 (CPUID.(EAX= 0FH, ECX=0H).EBX[31:0]
     * +1)) 31:N Reserved
     */
    UINT64 ResourceMonitoringId : 32;
...
#define IA32_PQR_ASSOC_RESOURCE_MONITORING_ID(_) (((_) >> 0) & 0xFFFFFFFF)
     ...
     * @brief COS <b>(R/W)</b>
     *
     * [Bits 63:32] The class of service (COS) to enforce (on writes); returns
     * the current COS when read.
     *
...
#define IA32_BNDCFGS_ENABLE(_) (((_) >> 0) & 0x01)
    ...
    /**
     * [Bit 1] Preserve the bounds registers for near branch instructions in the
...
typedef union {
  struct {
     ...
     *
     * [Bit 0] Force HDC idling or wake up HDC-idled logical processors in the
     * package.
     *
     * @remarks If CPUID.06H:EAX.[13] = 1
 ...
 * Auxiliary TSC.
 *
 * @remarks If CPUID.80000001H: EDX[27] = 1 or CPUID.(EAX=7,ECX=0):ECX[bit 22] =
 * 1
 */
 ...
 * time. 32-bit paging uses a hierarchy of paging structures to produce a
 * translation for a linear address. CR3 is used to locate the first
 * paging-structure, the page directory. 32-bit paging may map linear addresses
 * to either 4-KByte pages or 4-MByte pages.
 *
...
#define PDE_4MB_32_SUPERVISOR(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] Page-level write-through; indirectly determines the memory type
     * used to access the 4-MByte page referenced by this entry.
     *
     * @see Vol3A[4.9.2(Paging and Memory Typing When the PAT is Supported
...
#define PDE_4MB_32_PAGE_LEVEL_WRITE_THROUGH(_) (((_) >> 3) & 0x01)
    ...
    /**
     * [Bit 4] Page-level cache disable; indirectly determines the memory type
     * used to access the 4-MByte page referenced by this entry.
     *
     * @see Vol3A[4.9.2(Paging and Memory Typing When the PAT is Supported
     * (Pentium III and More Recent Processor Families))]
...
#define PDE_4MB_32_PAGE_LEVEL_CACHE_DISABLE(_) (((_) >> 4) & 0x01)
    ...
    /**
     * [Bit 5] Accessed; indicates whether software has accessed the 4-MByte
...
#define PDE_4MB_32_DIRTY(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 7] Page size; must be 1 (otherwise, this entry references a page
     * table).
     */
    UINT32 LargePage : 1;
...
#define PDE_4MB_32_IGNORED_1(_) (((_) >> 9) & 0x07)
    ...
    /**
     * [Bit 12] Indirectly determines the memory type used to access the 4-MByte
     * page referenced by this entry.
     *
     * @see Vol3A[4.9.2(Paging and Memory Typing When the PAT is Supported
     * (Pentium III and More Recent Processor Families))]
...
#define PDE_4MB_32_PAT(_) (((_) >> 12) & 0x01)
    ...
    /**
     * [Bits 20:13] Bits (M-1):32 of physical address of the 4-MByte page
     * referenced by this entry.
     */
...
#define PDE_4MB_32_PAGE_FRAME_NUMBER_LOW(_) (((_) >> 13) & 0xFF)
    ...
    /**
     * [Bits 31:22] Bits 31:22 of physical address of the 4-MByte page
     * referenced by this entry.
     */
...
#define PDE_32_IGNORED_1(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 7] If CR4.PSE = 1, must be 0 (otherwise, this entry maps a 4-MByte
     * page); otherwise, ignored.
     */
    UINT32 LargePage : 1;
...
#define PDE_32_IGNORED_2(_) (((_) >> 8) & 0x0F)
    ...
    /**
     * [Bits 31:12] Physical address of 4-KByte aligned page table referenced by
     * this entry.
     */
...
#define PTE_32_PAGE_LEVEL_CACHE_DISABLE(_) (((_) >> 4) & 0x01)
    ...
    /**
     * [Bit 5] Accessed; indicates whether software has accessed the 4-KByte
     * page referenced by this entry.
     *
     * @see Vol3A[4.8(Accessed and Dirty Flags)]
     */
...
#define PTE_32_DIRTY(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 7] Indirectly determines the memory type used to access the 4-KByte
     * page referenced by this entry.
     *
     * @see Vol3A[4.9.2(Paging and Memory Typing When the PAT is Supported
     * (Pentium III and More Recent Processor Families))]
...
#define PT_ENTRY_32_IGNORED_1(_) (((_) >> 9) & 0x07)
    ...
    /**
     * [Bits 31:12] Physical address of the 4-KByte page referenced by this
     * entry.
     */
    UINT32 PageFrameNumber : 20;
 ...
 *
 * A logical processor uses 4-level paging if CR0.PG = 1, CR4.PAE = 1, and
 * IA32_EFER.LME = 1. With 4-level paging, linear address are translated using a
 * hierarchy of in-memory paging structures located using the contents of CR3.
 * 4-level paging translates 48-bit linear addresses to 52-bit physical
 * addresses. Although 52 bits corresponds to 4 PBytes, linear addresses are
 * limited to 48 bits; at most 256 TBytes of linear-address space may be
 * accessed at any given time. 4-level paging uses a hierarchy of paging
 * structures to produce a translation for a linear address. CR3 is used to
 * locate the first paging-structure, the PML4 table. Use of CR3 with 4-level
 * paging depends on whether processcontext identifiers (PCIDs) have been
 * enabled by setting CR4.PCIDE.
 *
 * @see Vol3A[4.5(4-LEVEL PAGING)] (reference)
 * @{
 */
/**
 * @brief Format of a 4-Level PML4 Entry (PML4E) that References a
...
#define PML4E_64_PRESENT(_) (((_) >> 0) & 0x01)
    ...
    /**
     * [Bit 1] Read/write; if 0, writes may not be allowed to the 512-GByte
...
#define PML4E_64_SUPERVISOR(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] Page-level write-through; indirectly determines the memory type
     * used to access the page-directory-pointer table referenced by this entry.
     *
     * @see Vol3A[4.9.2(Paging and Memory Typing When the PAT is Supported
     * (Pentium III and More Recent Processor Families))]
     */
...
#define PML4E_64_RESTART(_) (((_) >> 11) & 0x01)
    ...
    /**
     * [Bits 47:12] Physical address of 4-KByte aligned page-directory-pointer
...
#define PML4E_64_IGNORED_2(_) (((_) >> 52) & 0x7FF)
    ...
    /**
     * [Bit 63] If IA32_EFER.NXE = 1, execute-disable (if 1, instruction fetches
     * are not allowed from the 512-GByte region controlled by this entry);
...
/**
 * @brief Format of a 4-Level Page-Directory-Pointer-Table Entry (PDPTE) that
 * Maps a 1-GByte Page
 */
typedef union {
  struct {
...
#define PDPTE_1GB_64_SUPERVISOR(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] Page-level write-through; indirectly determines the memory type
     * used to access the 1-GByte page referenced by this entry.
     *
     * @see Vol3A[4.9.2(Paging and Memory Typing When the PAT is Supported
     * (Pentium III and More Recent Processor Families))]
     */
...
#define PDPTE_1GB_64_PAGE_LEVEL_CACHE_DISABLE(_) (((_) >> 4) & 0x01)
    ...
    /**
     * [Bit 5] Accessed; indicates whether software has accessed the 1-GByte
     * page referenced by this entry.
     *
     * @see Vol3A[4.8(Accessed and Dirty Flags)]
     */
...
#define PDPTE_1GB_64_RESTART(_) (((_) >> 11) & 0x01)
    ...
    /**
     * [Bit 12] Indirectly determines the memory type used to access the 1-GByte
     * page referenced by this entry.
     *
...
#define PDPTE_1GB_64_PAT(_) (((_) >> 12) & 0x01)
    ...
    /**
     * [Bits 47:30] Physical address of the 1-GByte page referenced by this
     * entry.
     */
...
#define PDPTE_1GB_64_IGNORED_2(_) (((_) >> 52) & 0x7F)
    ...
    /**
     * [Bits 62:59] Protection key; if CR4.PKE = 1, determines the protection
     * key of the page; ignored otherwise.
     *
...
#define PDPTE_1GB_64_PROTECTION_KEY(_) (((_) >> 59) & 0x0F)
    ...
    /**
     * [Bit 63] If IA32_EFER.NXE = 1, execute-disable (if 1, instruction fetches
     * are not allowed from the 1-GByte page controlled by this entry);
     * otherwise, reserved (must be 0).
     *
     * @see Vol3A[4.6(Access Rights)]
     */
...
#define PDPTE_64_WRITE(_) (((_) >> 1) & 0x01)
    ...
    /**
     * [Bit 2] User/supervisor; if 0, user-mode accesses are not allowed to the
     * 1-GByte region controlled by this entry.
     *
     * @see Vol3A[4.6(Access Rights)]
     */
    UINT64 Supervisor : 1;
...
#define PDPTE_64_SUPERVISOR(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] Page-level write-through; indirectly determines the memory type
     * used to access the page directory referenced by this entry.
     *
     * @see Vol3A[4.9.2(Paging and Memory Typing When the PAT is Supported
     * (Pentium III and More Recent Processor Families))]
     */
...
#define PDPTE_64_RESTART(_) (((_) >> 11) & 0x01)
    ...
    /**
     * [Bits 47:12] Physical address of 4-KByte aligned page directory
     * referenced by this entry.
     */
    UINT64 PageFrameNumber : 36;
...
#define PDE_2MB_64_SUPERVISOR(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] Page-level write-through; indirectly determines the memory type
     * used to access the 2-MByte page referenced by this entry.
     *
     * @see Vol3A[4.9.2(Paging and Memory Typing When the PAT is Supported
     * (Pentium III and More Recent Processor Families))]
     */
...
#define PDE_2MB_64_PAGE_LEVEL_CACHE_DISABLE(_) (((_) >> 4) & 0x01)
    ...
    /**
     * [Bit 5] Accessed; indicates whether software has accessed the 2-MByte
     * page referenced by this entry.
     *
     * @see Vol3A[4.8(Accessed and Dirty Flags)]
     */
...
#define PDE_2MB_64_RESTART(_) (((_) >> 11) & 0x01)
    ...
    /**
     * [Bit 12] Indirectly determines the memory type used to access the 2-MByte
     * page referenced by this entry.
     *
     * @note The PAT is supported on all processors that support 4-level paging.
     * @see Vol3A[4.9.2(Paging and Memory Typing When the PAT is Supported
...
#define PDE_2MB_64_PAT(_) (((_) >> 12) & 0x01)
    ...
    /**
     * [Bits 47:21] Physical address of the 2-MByte page referenced by this
     * entry.
     */
...
#define PDE_2MB_64_PROTECTION_KEY(_) (((_) >> 59) & 0x0F)
    ...
    /**
     * [Bit 63] If IA32_EFER.NXE = 1, execute-disable (if 1, instruction fetches
     * are not allowed from the 2-MByte page controlled by this entry);
     * otherwise, reserved (must be 0).
     *
     * @see Vol3A[4.6(Access Rights)]
     */
...
#define PDE_64_WRITE(_) (((_) >> 1) & 0x01)
    ...
    /**
     * [Bit 2] User/supervisor; if 0, user-mode accesses are not allowed to the
     * 2-MByte region controlled by this entry.
     *
     * @see Vol3A[4.6(Access Rights)]
     */
    UINT64 Supervisor : 1;
...
#define PDE_64_RESTART(_) (((_) >> 11) & 0x01)
    ...
    /**
     * [Bits 47:12] Physical address of 4-KByte aligned page table referenced by
     * this entry.
     */
    UINT64 PageFrameNumber : 36;
...
#define PTE_64_RESTART(_) (((_) >> 11) & 0x01)
    ...
    /**
     * [Bits 47:12] Physical address of the 4-KByte page referenced by this
     * entry.
     */
    UINT64 PageFrameNumber : 36;
...
typedef enum {
  /**
   * If the INVPCID type is 0, the logical processor invalidates mappings-except
   * global translations-associated with the PCID specified in the INVPCID
   * descriptor and that would be used to translate the linear address specified
   * in the INVPCID descriptor.2 (The instruction may also invalidate global
   * translations, as well as mappings associated with other PCIDs and for other
   * linear addresses.)
   */
  InvpcidIndividualAddress = 0x00000000,

  /**
   * If the INVPCID type is 1, the logical processor invalidates all
   * mappings-except global translations-associated with the PCID specified in
   * the INVPCID descriptor. (The instruction may also invalidate global
  ...
  /**
   * If the INVPCID type is 3, the logical processor invalidates mappings-except
   * global translations- associated with all PCIDs. (The instruction may also
   * invalidate global translations.)
   */
...
#define INVPCID_DESCRIPTOR_LINEAR_ADDRESS_FLAG                                 \
 ...
 *
 * @see Vol3A[5.2(FIELDS AND FLAGS USED FOR SEGMENT-LEVEL AND PAGE-LEVEL
 * PROTECTION)]
...
typedef struct {
   ...
   * (granularity) flag:
   * - If the granularity flag is clear, the segment size can range from 1 byte
   * to 1 MByte, in byte increments.
   * - If the granularity flag is set, the segment size can range from 4 KBytes
   * to 4 GBytes, in 4-KByte increments. The processor uses the segment limit in
   * two different ways, depending on whether the segment is an expand-up or an
   * expand-down segment. For expand-up segments, the offset in a logical
   * address can range from 0 to the segment limit. Offsets greater than the
   * segment limit generate general-protection exceptions (\#GP, for all
   * segments other than SS) or stack-fault exceptions (\#SS for the SS
   * segment). For expand-down segments, the segment limit has the reverse
   * function; the offset can range from the segment limit plus 1 to FFFFFFFFH
   * or FFFFH, depending on the setting of the B flag. Offsets less than or
   * equal to the segment limit generate general-protection exceptions or
   * stack-fault exceptions. Decreasing the value in the segment limit field for
   * an expanddown segment allocates new memory at the bottom of the segment's
   * address space, rather than at the top. IA-32 architecture stacks always
   * grow downwards, making this mechanism convenient for expandable stacks.
   *
   * @see Vol3A[3.4.5.1(Code- and Data-Segment Descriptor Types)]
   */
...
#define SEGMENT__BASE_ADDRESS_MIDDLE(_) (((_) >> 0) & 0xFF)
       ...
       * @brief Type field
       *
       * [Bits 11:8] Indicates the segment or gate type and specifies the kinds
...
#define SEGMENT__TYPE(_) (((_) >> 8) & 0x0F)
       ...
       * @brief S (descriptor type) flag
       *
       * [Bit 12] Specifies whether the segment descriptor is for a system
...
#define SEGMENT__DESCRIPTOR_TYPE(_) (((_) >> 12) & 0x01)
       ...
       * @brief DPL (descriptor privilege level) field
       *
       * [Bits 14:13] Specifies the privilege level of the segment. The
       * privilege level can range from 0 to 3, with 0 being the most privileged
       * level. The DPL is used to control access to the segment. See
       * Section 5.5, "Privilege Levels", for a description of the relationship
...
#define SEGMENT__DESCRIPTOR_PRIVILEGE_LEVEL(_) (((_) >> 13) & 0x03)
       ...
       * @brief P (segment-present) flag
       *
       * [Bit 15] Indicates whether the segment is present in memory (set) or
       * not present (clear). If this flag is clear, the processor generates a
       ...
       * Memory management software can use this flag to control which segments
       * are actually loaded into physical memory at a given time. It offers a
       * control in addition to paging for managing virtual memory.
       */
      UINT32 Present : 1;
...
#define SEGMENT__SYSTEM(_) (((_) >> 20) & 0x01)
       ...
       * executed in 64-bit mode. A value of 0 indicates the instructions in
       * this code segment are executed in compatibility mode. If L-bit is set,
       * then D-bit must be cleared. When not in IA-32e mode or for non-code
...
#define SEGMENT__LONG_MODE(_) (((_) >> 21) & 0x01)
       ...
       * the default, and the prefix 67H can be used select an address size
       ...
       * pointer is used, which is stored in the 16- bit SP register. If the
       * stack segment is set up to be an expand-down data segment (described in
       * the next paragraph), the B flag also specifies the upper bound of the
       * stack segment.
       * - Expand-down data segment. The flag is called the B flag and it
       * specifies the upper bound of the segment. If the flag is set, the upper
       * bound is FFFFFFFFH (4 GBytes); if the flag is clear, the upper bound is
...
#define SEGMENT__DEFAULT_BIG(_) (((_) >> 22) & 0x01)
       ...
       * granularity flag is clear, the segment limit is interpreted in byte
       * units; when flag is set, the segment limit is interpreted in 4-KByte
       * units. (This flag does not affect the granularity of the base address;
       * it is always byte granular.) When the granularity flag is set, the
       * twelve least significant bits of an offset are not tested when checking
       * the offset against the segment limit. For example, when the granularity
...
#define SEGMENT__GRANULARITY(_) (((_) >> 23) & 0x01)
      ...
      /**
       * [Bits 31:24] Base address field (31:24); see description of $BASE_LOW
       * for more details.
       */
 ...
 * then determines whether the descriptor is for a data segment (clear) or a
 * code segment (set). For data segments, the three low-order bits of the type
 * field (bits 8, 9, and 10) are interpreted as accessed (A), write-enable (W),
 * and expansion-direction (E). See Table 3-1 for a description of the encoding
 * of the bits in the type field for code and data segments. Data segments can
 * be read-only or read/write segments, depending on the setting of the
 * write-enable bit.
 *
 ...
 * descriptor type is a system descriptor. The processor recognizes the
 * following types of system descriptors:
 ...
 * These descriptor types fall into two categories: system-segment descriptors
 * and gate descriptors. Systemsegment descriptors point to system segments (LDT
 * and TSS segments). Gate descriptors are in themselves "gates," which hold
 * pointers to procedure entry points in code segments (call, interrupt, and
 * trap gates) or which hold segment selectors for TSS's (task gates).
 *
...
/**
 * @brief A segment selector is a 16-bit identifier for a segment. It does not
 * point directly to the segment, but instead points to the segment descriptor
...
typedef union {
  struct {
    /**
     * [Bits 1:0] Specifies the privilege level of the selector. The privilege
...
#define SEGMENT_SELECTOR_REQUEST_PRIVILEGE_LEVEL(_) (((_) >> 0) & 0x03)
    ...
    /**
     * [Bit 2] Specifies the descriptor table to use: clearing this flag selects
     * the GDT; setting this flag selects the current LDT.
     */
    UINT16 Table : 1;
...
#define SEGMENT_SELECTOR_TABLE(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bits 15:3] Selects one of 8192 descriptors in the GDT or LDT. The
     * processor multiplies the index value by 8 (the number of bytes in a
     * segment descriptor) and adds the result to the base address of the GDT or
 ...
 *
 * Either:
 * -# Guest software caused an exception and the bit in the exception bitmap
 * associated with exception's vector was 1. This case includes executions of
 * BOUND that cause \#BR, executions of INT1 (they cause \#DB), executions of
 * INT3 (they cause
 * \#BP), executions of INTO that cause \#OF, and executions of UD0, UD1, and
 * UD2 (they cause \#UD).
 * -# An NMI was delivered to the logical processor and the "NMI exiting"
 ...
 * @brief Triple fault
 *
 * The logical processor encountered an exception while attempting to call the
 * double-fault handler and that exception did not itself cause a VM exit due to
 ...
 *
 * An SMI arrived immediately after retirement of an I/O instruction and caused
 * an SMM VM exit.
 *
 ...
 * @brief Other SMI
 *
 * An SMI arrived and caused an SMM VM exit but not immediately after retirement
 ...
 * by STI or by MOV SS; and the "interrupt-window exiting" VM-execution control
 * was 1.
 */
 ...
 *
 * At the beginning of an instruction, there was no virtual-NMI blocking; events
 * were not blocked by MOV SS; and the "NMI-window exiting" VM-execution control
 * was 1.
 */
 ...
 * @brief HLT
 *
 * Guest software attempted to execute HLT and the "HLT exiting" VM-execution
 * control was 1.
 */
 ...
 * @brief VMCALL
 *
 * VMCALL was executed either by guest software (causing an ordinary VM exit) or
 * by the executive monitor (causing an SMM VM exit).
 *
 * @see Vol3C[34.15.2(SMM VM Exits)]
 */
 ...
 * MOV CR and the VM-execution control fields indicate that a VM exit should
 * occur. This basic exit reason is not used for trap-like VM exits following
 * executions of the MOV to CR8 instruction when the "use TPR shadow"
 * VM-execution control is 1. Such VM exits instead use basic exit reason 43.
 *
 ...
 *
 * Guest software attempted a MOV to or from a debug register and the "MOV-DR
 ...
 * -# The "use I/O bitmaps" VM-execution control was 0 and the "unconditional
 * I/O exiting" VM-execution control was 1.
 * -# The "use I/O bitmaps" VM-execution control was 1 and a bit in the I/O
 ...
 * -# The value of RCX is neither in the range 00000000H - 00001FFFH nor in the
 * range C0000000H - C0001FFFH.
 * -# The value of RCX was in the range 00000000H - 00001FFFH and the nth bit in
 * read bitmap for low MSRs is 1, where n was the value of RCX.
 * -# The value of RCX is in the range C0000000H - C0001FFFH and the nth bit in
 ...
 *
 * Guest software attempted to execute MWAIT and the "MWAIT exiting"
 * VM-execution control was 1.
 */
 ...
 *
 * A VM entry occurred due to the 1-setting of the "monitor trap flag"
 * VM-execution control and injection of an MTF VM exit as part of VM entry.
 *
 * @see Vol3C[25.5.2(Monitor Trap Flag)]
 */
 ...
 *
 * Guest software attempted to execute MONITOR and the "MONITOR exiting"
 * VM-execution control was 1.
 */
 ...
 *
 * Either guest software attempted to execute PAUSE and the "PAUSE exiting"
 * VM-execution control was 1 or the "PAUSE-loop exiting" VM-execution control
 * was 1 and guest software executed a PAUSE loop with execution time exceeding
 * PLE_Window.
 *
 * @see Vol3C[25.1.3(Instructions That Cause VM Exits Conditionally)]
 */
 ...
 *
 * The logical processor determined that the value of bits 7:4 of the byte at
 * offset 080H on the virtual-APIC page was below that of the TPR threshold
 * VM-execution control field while the "use TPR shadow" VMexecution control was
 * 1 either as part of TPR virtualization or VM entry.
 *
 ...
 * @brief APIC access
 *
 * Guest software attempted to access memory at a physical address on the
 * APIC-access page and the "virtualize APIC accesses" VM-execution control
 * was 1.
 *
 * @see Vol3C[29.4(VIRTUALIZING MEMORY-MAPPED APIC ACCESSES)]
 */
 ...
 * a bit set in the EOIexit bitmap.
 */
 ...
 *
 * Guest software attempted to execute LGDT, LIDT, SGDT, or SIDT and the
 * "descriptor-table exiting" VM-execution control was 1.
 */
 ...
 *
 * Guest software attempted to execute LLDT, LTR, SLDT, or STR and the
 * "descriptor-table exiting" VM-execution control was 1.
 */
 ...
 * @brief EPT violation
 *
 * An attempt to access memory with a guest-physical address was disallowed by
 * the configuration of the EPT paging structures.
 */
 ...
 * @brief RDTSCP
 *
 * Guest software attempted to execute RDTSCP and the "enable RDTSCP" and "RDTSC
 * exiting" VM-execution controls were both 1.
 */
 ...
 * @brief WBINVD
 *
 * Guest software attempted to execute WBINVD and the "WBINVD exiting"
 * VM-execution control was 1.
 */
 ...
 * @brief APIC write
 *
 * Guest software completed a write to the virtual-APIC page that must be
 * virtualized by VMM software.
 *
 ...
 * @brief RDRAND
 *
 * Guest software attempted to execute RDRAND and the "RDRAND exiting"
 * VM-execution control was 1.
 */
 ...
 * @brief INVPCID
 *
 * Guest software attempted to execute INVPCID and the "enable INVPCID" and
 * "INVLPG exiting" VM-execution controls were both 1.
 */
 ...
 * @brief VMFUNC
 *
 * Guest software invoked a VM function with the VMFUNC instruction and the VM
 * function either was not enabled or generated a function-specific condition
 * causing a VM exit.
 */
 ...
 * @brief ENCLS
 *
 * Guest software attempted to execute ENCLS and "enable ENCLS exiting"
 * VM-execution control was 1 and either:
 ...
 * @brief RDSEED
 *
 * Guest software attempted to execute RDSEED and the "RDSEED exiting"
 * VM-execution control was 1.
 */
 ...
 *
 * The processor attempted to create a page-modification log entry and the value
 * of the PML index was not in the range 0-511.
 */
 ...
 * @brief XSAVES
 *
 * Guest software attempted to execute XSAVES, the "enable XSAVES/XRSTORS" was
 * 1, and a bit was set in the logical-AND of the following three values:
 * EDX:EAX, the IA32_XSS MSR, and the XSS-exiting bitmap.
 */
 ...
 * @brief XRSTORS
 *
 * Guest software attempted to execute XRSTORS, the "enable XSAVES/XRSTORS" was
 * 1, and a bit was set in the logical-AND of the following three values:
 * EDX:EAX, the IA32_XSS MSR, and the XSS-exiting bitmap.
 */
...
/**
 * VMCALL with incorrect MSEG revision identifier (when attempting to activate
...
typedef struct {
  /**
   * The 32-bit value that would have been saved into the VMCS as an exit reason
   * had a VM exit occurred instead of the virtualization exception. For EPT
   * violations, this value is 48 (00000030H).
   */
  UINT32 Reason;
  ...
  /**
   * The 64-bit value that would have been saved into the VMCS as an exit
   * qualification had a VM exit occurred instead of the virtualization
   * exception.
   */
  UINT64 Exit;

  /**
   * The 64-bit value that would have been saved into the VMCS as a guest-linear
   * address had a VM exit occurred instead of the virtualization exception.
   */
  UINT64 GuestLinearAddress;

  /**
   * The 64-bit value that would have been saved into the VMCS as a
   * guest-physical address had a VM exit occurred instead of the virtualization
   * exception.
   */
...
#define VMX_EXIT_QUALIFICATION_DEBUG_EXCEPTION_BREAKPOINT_CONDITION(_)         \
  (((_) >> 0) & 0x0F)
     ...
     * @brief BD
     *
     * [Bit 13] When set, this bit indicates that the cause of the debug
     * exception is "debug register access detected."
     */
    UINT64 DebugRegisterAccessDetected : 1;
...
#define VMX_EXIT_QUALIFICATION_DEBUG_EXCEPTION_DEBUG_REGISTER_ACCESS_DETECTED( \
  ...
  (((_) >> 13) & 0x01)
     ...
     * @brief BS
     *
     * [Bit 14] When set, this bit indicates that the cause of the debug
     * exception is either the execution of a single instruction (if RFLAGS.TF =
     * 1 and IA32_DEBUGCTL.BTF = 0) or a taken branch (if RFLAGS.TF =
     * DEBUGCTL.BTF = 1).
     */
    UINT64 SingleInstruction : 1;
...
typedef union {
  struct {
    /**
     * [Bits 15:0] Selector of task-state segment (TSS) to which the guest
     * attempted to switch.
     */
    UINT64 Selector : 16;
...
typedef union {
  struct {
    /**
     * [Bits 3:0] Number of control register (0 for CLTS and LMSW). Bit 3 is
     * always 0 on processors that do not support Intel 64 architecture as they
     * do not support CR8.
     */
...
#define VMX_EXIT_QUALIFICATION_GENREG_R15 0x0000000F
    ...
    /**
     * [Bits 31:16] For LMSW, the LMSW source data. For CLTS and MOV CR, cleared
     * to 0.
     */
...
/**
 * @brief Exit Qualification for APIC-Access VM Exits from Linear Accesses and
 * Guest-Physical Accesses
 */
typedef union {
  struct {
    /**
     * [Bits 11:0] - If the APIC-access VM exit is due to a linear access, the
     * offset of access within the APIC page.
...
#define VMX_EXIT_QUALIFICATION_TYPE_PHYSICAL_EVENT_DELIVERY 0x0000000A
    ...
    /**
     * Guest-physical access for an instruction fetch or during instruction
     * execution.
     */
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_WRITE_ACCESS(_) (((_) >> 1) & 0x01)
    ...
    /**
     * [Bit 2] Set if the access causing the EPT violation was an instruction
     * fetch.
     */
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_EXECUTE_ACCESS(_)                 \
  (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] The logical-AND of bit 0 in the EPT paging-structure entries used
     * to translate the guest-physical address of the access causing the EPT
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_EPT_READABLE(_) (((_) >> 3) & 0x01)
    ...
    /**
     * [Bit 4] The logical-AND of bit 1 in the EPT paging-structure entries used
     * to translate the guest-physical address of the access causing the EPT
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_EPT_WRITEABLE(_)                  \
  (((_) >> 4) & 0x01)
    ...
    /**
     * [Bit 5] The logical-AND of bit 2 in the EPT paging-structure entries used
     * to translate the guest-physical address of the access causing the EPT
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_EPT_EXECUTABLE(_)                 \
  (((_) >> 5) & 0x01)
    ...
    /**
     * [Bit 6] If the "mode-based execute control" VM-execution control is 0,
     * the value of this bit is undefined. If that control is 1, this bit is the
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_EPT_EXECUTABLE_FOR_USER_MODE(_)   \
  (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 7] Set if the guest linear-address field is valid. The guest
     * linear-address field is valid for all EPT violations except those
     * resulting from an attempt to load the guest PDPTEs as part of the
     * execution of the MOV CR instruction.
     */
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_CAUSED_BY_TRANSLATION(_)          \
  (((_) >> 8) & 0x01)
    ...
    /**
     * [Bit 9] This bit is 0 if the linear address is a supervisor-mode linear
     * address and 1 if it is a user-mode linear address. Otherwise, this bit is
     * undefined.
     *
     * @remarks If bit 7 is 1, bit 8 is 1, and the processor supports advanced
     * VM-exit information for EPT violations. (If CR0.PG = 0, the translation
     * of every linear address is a user-mode linear address and thus this bit
     * will be 1.)
     */
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_USER_MODE_LINEAR_ADDRESS(_)       \
  (((_) >> 9) & 0x01)
    ...
    /**
     * [Bit 10] This bit is 0 if paging translates the linear address to a
     * read-only page and 1 if it translates to a read/write page. Otherwise,
     * this bit is undefined
     *
     * @remarks If bit 7 is 1, bit 8 is 1, and the processor supports advanced
     * VM-exit information for EPT violations. (If CR0.PG = 0, every linear
     * address is read/write and thus this bit will be 1.)
     */
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_READABLE_WRITABLE_PAGE(_)         \
  (((_) >> 10) & 0x01)
    ...
    /**
     * [Bit 11] This bit is 0 if paging translates the linear address to an
     * executable page and 1 if it translates to an execute-disable page.
     * Otherwise, this bit is undefined.
     *
     * @remarks If bit 7 is 1, bit 8 is 1, and the processor supports advanced
     * VM-exit information for EPT violations. (If CR0.PG = 0, CR4.PAE = 0, or
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_NMI_UNBLOCKING(_)                 \
  (((_) >> 12) & 0x01)
    ...
    /**
     * [Bit 13] Set if the access causing the EPT violation was a shadow-stack
     * access.
     */
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_SHADOW_STACK_ACCESS(_)            \
  (((_) >> 13) & 0x01)

    /**
     * [Bit 14] If supervisor shadow-stack control is enabled (by setting bit 7
     * of EPTP), this bit is the same as bit 60 in the EPT paging-structure
     * entry that maps the page of the guest-physical address of the access
     * causing the EPT violation. Otherwise (or if translation of the
     * guest-physical address terminates before reaching an EPT paging-structure
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_SUPERVISOR_SHADOW_STACK(_)        \
  (((_) >> 14) & 0x01)
...
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_GUEST_PAGING_VERIFICATION_FLAG    \
  0x8000
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_GUEST_PAGING_VERIFICATION_MASK 0x01
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_GUEST_PAGING_VERIFICATION(_)      \
  (((_) >> 15) & 0x01)

    /**
     * [Bit 16] This bit is set if the access was asynchronous to instruction
     * execution not the result of event delivery. (The bit is set if the access
     * is related to trace output by Intel PT; see Section 25.5.4.) Otherwise,
     * this bit is cleared.
     */
    UINT64 AsynchronousToInstruction : 1;
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_ASYNCHRONOUS_TO_INSTRUCTION_BIT 16
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_ASYNCHRONOUS_TO_INSTRUCTION_FLAG  \
  0x10000
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_ASYNCHRONOUS_TO_INSTRUCTION_MASK  \
  0x01
#define VMX_EXIT_QUALIFICATION_EPT_VIOLATION_ASYNCHRONOUS_TO_INSTRUCTION(_)    \
  (((_) >> 16) & 0x01)
    UINT64 Reserved1 : 47;
  };
...
/**
 * @brief VM-Exit Instruction-Information Field as Used for INVEPT, INVPCID, and
 * INVVPID
 */
typedef union {
  struct {
     ...
     * 1: scale by 2
     * 2: scale by 4
     * 3: scale by 8 (used only on processors that support Intel 64
     * architecture) Undefined for instructions with no index register (bit 22
     * is set).
     */
    UINT64 Scaling : 2;
...
#define VMX_VMEXIT_INSTRUCTION_INFO_INVALIDATE_SEGMENT_REGISTER(_)             \
  (((_) >> 15) & 0x07)
    ...
    /**
     * [Bits 21:18] General-purpose register. Undefined for instructions with no
     * index register (bit 22 is set).
     */
    UINT64 GeneralPurposeRegister : 4;
...
/**
 * @brief VM-Exit Instruction-Information Field as Used for LIDT, LGDT, SIDT, or
 * SGDT
 */
typedef union {
  struct {
...
/**
 * @brief VM-Exit Instruction-Information Field as Used for LLDT, LTR, SLDT, and
 * STR
 */
typedef union {
  struct {
...
#define VMX_VMEXIT_INSTRUCTION_INFO_LDTR_TR_ACCESS_REG_1(_) (((_) >> 3) & 0x0F)
     ...
     * 1: 32-bit
     * 2: 64-bit (used only on processors that support Intel 64 architecture)
     * Other values not used. Undefined for register instructions (bit 10 is
     * set).
     */
    UINT64 AddressSize : 3;
...
#define VMX_VMEXIT_INSTRUCTION_INFO_LDTR_TR_ACCESS_SEGMENT_REGISTER(_)         \
  (((_) >> 15) & 0x07)
    ...
    /**
     * [Bits 21:18] General-purpose register. Undefined for register
     * instructions (bit 10 is set) and for memory instructions with no index
...
#define VMX_VMEXIT_INSTRUCTION_INFO_LDTR_TR_ACCESS_GENERAL_PURPOSE_REGISTER(_) \
  (((_) >> 18) & 0x0F)
    ...
    /**
     * [Bit 22] IndexReg invalid (0 = valid; 1 = invalid). Undefined for
     * register instructions (bit 10 is set).
     */
    UINT64 GeneralPurposeRegisterInvalid : 1;
...
#define VMX_VMEXIT_INSTRUCTION_INFO_LDTR_TR_ACCESS_GENERAL_PURPOSE_REGISTER_INVALID( \
  ...
  (((_) >> 22) & 0x01)
     ...
     * instructions (bit 10 is set) and for memory instructions with no base
...
/**
 * @brief VM-Exit Instruction-Information Field as Used for VMCLEAR, VMPTRLD,
 * VMPTRST, VMXON, XRSTORS, and XSAVES
 */
typedef union {
  struct {
    /**
...
typedef union {
  struct {
     ...
     * 2: scale by 4
     * 3: scale by 8 (used only on processors that support Intel 64
     * architecture) Undefined for register instructions (bit 10 is set) and for
     * memory instructions with no index register (bit 10 is clear and bit 22 is
     * set).
     */
    UINT64 Scaling : 2;
...
#define VMX_VMEXIT_INSTRUCTION_INFO_VMREAD_VMWRITE_GENERAL_PURPOSE_REGISTER_INVALID( \
  ...
  (((_) >> 22) & 0x01)
    ...
    /**
     * [Bits 26:23] BaseReg (encoded as Reg1 above). Undefined for register
     * instructions (bit 10 is set) and for memory instructions with no base
     * register (bit 10 is clear and bit 27 is set).
     */
    UINT64 BaseRegister : 4;
...
/**
 * @brief - The low 16 bits correspond to bits 23:8 of the upper 32 bits of a
 * 64-bit segment descriptor. While bits 19:16 of code-segment and data-segment
 * descriptors correspond to the upper 4 bits of the segment limit, the
 * corresponding bits (bits 11:8) are reserved in this VMCS field.
 *        - Bit 16 indicates an unusable segment. Attempts to use such a segment
 * fault except in 64-bit mode. In general, a segment register is unusable if it
 * has been loaded with a null selector.
 *        - Bits 31:17 are reserved
 *
 * @note There are a few exceptions to this statement. For example, a segment
 * with a non-null selector may be unusable following a task switch that fails
 * after its commit point. In contrast, the TR register is usable after
 * processor reset despite having a null selector
...
#define VMX_SEGMENT_ACCESS_RIGHTS_LONG_MODE(_) (((_) >> 13) & 0x01)
    ...
    /**
     * [Bit 14] D/B - Default operation size (0 = 16-bit segment; 1 = 32-bit
     * segment).
     */
    UINT32 DefaultBig : 1;
...
/**
 * @brief The IA-32 architecture includes features that permit certain events to
 * be blocked for a period of time. This field contains information about such
 * blocking
 *
...
typedef union {
  struct {
    /**
     * [Bit 0] Execution of STI with RFLAGS.IF = 0 blocks maskable interrupts on
     * the instruction boundary following its execution.1 Setting this bit
...
#define VMX_INTERRUPTIBILITY_STATE_BLOCKING_BY_STI(_) (((_) >> 0) & 0x01)
    ...
    /**
     * [Bit 1] Execution of a MOV to SS or a POP to SS blocks or suppresses
     * certain debug exceptions as well as interrupts (maskable and nonmaskable)
     * on the instruction boundary following its execution. Setting this bit
     * indicates that this blocking is in effect. This document uses the term
     * "blocking by MOV SS," but it applies equally to POP SS.
     *
     * @see Vol3A[6.8.3(Masking Exceptions and Interrupts When Switching
     * Stacks)]
     */
...
#define VMX_INTERRUPTIBILITY_STATE_BLOCKING_BY_MOV_SS(_) (((_) >> 1) & 0x01)
    ...
    /**
     * [Bit 2] System-management interrupts (SMIs) are disabled while the
     * processor is in system-management mode (SMM). Setting this bit indicates
     * that blocking of SMIs is in effect.
     *
...
#define VMX_INTERRUPTIBILITY_STATE_BLOCKING_BY_SMI(_) (((_) >> 2) & 0x01)
     ...
     * interrupt (SMI) blocks subsequent NMIs until the next execution of IRET.
     * Setting this bit indicates that blocking of NMIs is in effect. Clearing
     * this bit does not imply that NMIs are not (temporarily) blocked for other
     * reasons. If the "virtual NMIs" VM-execution control is 1, this bit does
     * not control the blocking of NMIs. Instead, it refers to "virtual-NMI
...
#define VMX_INTERRUPTIBILITY_STATE_BLOCKING_BY_NMI(_) (((_) >> 3) & 0x01)
    ...
    /**
     * [Bit 4] A VM exit saves this bit as 1 to indicate that the VM exit was
     * incident to enclave mode.
     */
...
typedef enum {
  ...
  /**
   * The logical processor is inactive because it incurred a triple fault1 or
   * some other serious error.
   */
  VmxShutdown = 0x00000002,

  /**
   * The logical processor is inactive because it is waiting for a startup-IPI
   * (SIPI).
   */
...
/**
 * @brief IA-32 processors may recognize one or more debug exceptions without
 * immediately delivering them. This field contains information about such
 * exceptions
 *
 * @see Vol3C[24.4.2(Guest Non-Register State)] (reference)
 */
typedef union {
  struct {
    /**
     * [Bit 0] When set, indicates that the corresponding breakpoint condition
     * was met. May be set even if the corresponding enabling bit in DR7 is not
     * set.
     */
    UINT64 B0 : 1;
...
#define VMX_PENDING_DEBUG_EXCEPTIONS_B0(_) (((_) >> 0) & 0x01)
    ...
    /**
     * [Bit 1] When set, indicates that the corresponding breakpoint condition
     * was met. May be set even if the corresponding enabling bit in DR7 is not
     * set.
     */
    UINT64 B1 : 1;
...
#define VMX_PENDING_DEBUG_EXCEPTIONS_B1(_) (((_) >> 1) & 0x01)
    ...
    /**
     * [Bit 2] When set, indicates that the corresponding breakpoint condition
     * was met. May be set even if the corresponding enabling bit in DR7 is not
     * set.
     */
    UINT64 B2 : 1;
...
#define VMX_PENDING_DEBUG_EXCEPTIONS_B2(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] When set, indicates that the corresponding breakpoint condition
     * was met. May be set even if the corresponding enabling bit in DR7 is not
     * set.
     */
    UINT64 B3 : 1;
...
#define VMX_PENDING_DEBUG_EXCEPTIONS_B3(_) (((_) >> 3) & 0x01)
    ...
    /**
     * [Bit 12] When set, this bit indicates that at least one data or I/O
...
#define VMX_PENDING_DEBUG_EXCEPTIONS_ENABLED_BREAKPOINT(_) (((_) >> 12) & 0x01)
    ...
    /**
     * [Bit 14] When set, this bit indicates that a debug exception would have
     * been triggered by single-step execution mode.
     */
    UINT64 Bs : 1;
 ...
 *
 * Exit reason (32 bits). This field encodes the reason for the VM exit and has
 * the structure.
 *
...
typedef union {
  struct {
    /**
     * [Bits 15:0] Provides basic information about the cause of the VM exit (if
     * bit 31 is clear) or of the VM-entry failure (if bit 31 is set).
     */
...
#define VMX_VMEXIT_REASON_RESERVED1(_) (((_) >> 17) & 0x3FF)
    ...
    /**
     * [Bit 27] A VM exit saves this bit as 1 to indicate that the VM exit was
     * incident to enclave mode.
     */
 ...
 *
 * The extended page-table mechanism (EPT) is a feature that can be used to
 * support the virtualization of physical memory. When EPT is in use, certain
 * addresses that would normally be treated as physical addresses (and used to
 * access memory) are instead treated as guest-physical addresses.
 * Guest-physical addresses are translated by traversing a set of EPT paging
 ...
 * EPT PML4 table, as well as other EPT configuration information.
 *
...
#define EPT_PAGE_WALK_LENGTH_4 0x00000003
    ...
    /**
     * [Bit 6] Setting this control to 1 enables accessed and dirty flags for
     * EPT.
     *
     * @see Vol3C[28.2.4(Accessed and Dirty Flags for EPT)]
     */
...
#define EPT_POINTER_ENABLE_ACCESS_AND_DIRTY_FLAGS(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 7] Setting this control to 1 enables enforcement of access rights
     * for supervisor shadow-stack pages.
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
...
#define EPT_POINTER_ENABLE_SUPERVISOR_SHADOW_STACK_PAGES(_) (((_) >> 7) & 0x01)
    ...
    /**
     * [Bits 47:12] Bits N-1:12 of the physical address of the 4-KByte aligned
     * EPT PML4 table.
     */
    UINT64 PageFrameNumber : 36;
...
/**
 * @brief Format of an EPT PML4 Entry (PML4E) that References an EPT
 * Page-Directory-Pointer Table
 *
 * A 4-KByte naturally aligned EPT PML4 table is located at the physical address
 * specified in bits 51:12 of the extended-page-table pointer (EPTP), a
 * VM-execution control field. An EPT PML4 table comprises 512 64-bit entries
 * (EPT PML4Es). An EPT PML4E is selected using the physical address defined as
 * follows:
...
#define EPT_PML4E_WRITE_ACCESS(_) (((_) >> 1) & 0x01)
    ...
    /**
     * [Bit 2] If the "mode-based execute control for EPT" VM-execution control
     * is 0, execute access; indicates whether instruction fetches are allowed
     * from the 512-GByte region controlled by this entry. If that control is 1,
     * execute access for supervisor-mode linear addresses; indicates whether
     * instruction fetches are allowed from supervisor-mode linear addresses in
...
#define EPT_PML4E_EXECUTE_ACCESS(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 8] If bit 6 of EPTP is 1, accessed flag for EPT; indicates whether
     * software has accessed the 512-GByte region controlled by this entry.
...
#define EPT_PML4E_ACCESSED(_) (((_) >> 8) & 0x01)
    ...
    /**
     * [Bit 10] Execute access for user-mode linear addresses. If the
     * "mode-based execute control for EPT" VM-execution control is 1, indicates
     * whether instruction fetches are allowed from user-mode linear addresses
     * in the 512-GByte region controlled by this entry. If that control is 0,
     * this bit is ignored.
     */
...
#define EPT_PML4E_USER_MODE_EXECUTE(_) (((_) >> 10) & 0x01)
    ...
    /**
     * [Bits 47:12] Physical address of 4-KByte aligned EPT
...
/**
 * @brief Format of an EPT Page-Directory-Pointer-Table Entry (PDPTE) that Maps
 * a 1-GByte Page
 */
typedef union {
  struct {
    /**
     * [Bit 0] Read access; indicates whether reads are allowed from the 1-GByte
     * page referenced by this entry.
     */
    UINT64 ReadAccess : 1;
...
#define EPT_PDPTE_1GB_WRITE_ACCESS(_) (((_) >> 1) & 0x01)
     ...
     * [Bit 2] If the "mode-based execute control for EPT" VM-execution control
     * is 0, execute access; indicates whether instruction fetches are allowed
     * from the 1-GByte page controlled by this entry. If that control is 1,
     * execute access for supervisor-mode linear addresses; indicates whether
     * instruction fetches are allowed from supervisor-mode linear addresses in
...
#define EPT_PDPTE_1GB_IGNORE_PAT(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 7] Must be 1 (otherwise, this entry references an EPT page
     * directory).
     */
    UINT64 LargePage : 1;
...
#define EPT_PDPTE_1GB_LARGE_PAGE(_) (((_) >> 7) & 0x01)
    ...
    /**
     * [Bit 8] If bit 6 of EPTP is 1, accessed flag for EPT; indicates whether
     * software has accessed the 1-GByte page referenced by this entry. Ignored
     * if bit 6 of EPTP is 0.
     *
     * @see Vol3C[28.2.4(Accessed and Dirty Flags for EPT)]
     */
    UINT64 Accessed : 1;
...
#define EPT_PDPTE_1GB_ACCESSED(_) (((_) >> 8) & 0x01)
    ...
    /**
     * [Bit 9] If bit 6 of EPTP is 1, dirty flag for EPT; indicates whether
     * software has written to the 1-GByte page referenced by this entry.
     * Ignored if bit 6 of EPTP is 0.
     *
     * @see Vol3C[28.2.4(Accessed and Dirty Flags for EPT)]
     */
...
#define EPT_PDPTE_1GB_DIRTY(_) (((_) >> 9) & 0x01)
     ...
     * "mode-based execute control for EPT" VM-execution control is 1, indicates
     * whether instruction fetches are allowed from user-mode linear addresses
     * in the 1-GByte page controlled by this entry. If that control is 0, this
     * bit is ignored.
     */
    UINT64 UserModeExecute : 1;
...
#define EPT_PDPTE_1GB_USER_MODE_EXECUTE(_) (((_) >> 10) & 0x01)
    ...
    /**
     * [Bits 47:30] Physical address of 4-KByte aligned EPT
     * page-directory-pointer table referenced by this entry.
     */
    UINT64 PageFrameNumber : 18;
...
#define EPT_PDPTE_1GB_PAGE_FRAME_NUMBER(_) (((_) >> 30) & 0x3FFFF)
    ...
    /**
     * [Bit 57] Verify guest paging. If the "guest-paging verification"
     * VM-execution control is 1, indicates limits on the guest paging
     * structures used to access the 1-GByte page controlled by this entry (see
     * Section 28.3.3.2). If that control is 0, this bit is ignored.
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
...
#define EPT_PDPTE_1GB_VERIFY_GUEST_PAGING(_) (((_) >> 57) & 0x01)
    ...
    /**
     * [Bit 58] Paging-write access. If the "EPT paging-write control"
     * VM-execution control is 1, indicates that guest paging may update the
     * 1-GByte page controlled by this entry (see Section 28.3.3.2). If that
...
#define EPT_PDPTE_1GB_PAGING_WRITE_ACCESS(_) (((_) >> 58) & 0x01)
    ...
    /**
     * [Bit 60] Supervisor shadow stack. If bit 7 of EPTP is 1, indicates
     * whether supervisor shadow stack accesses are allowed to guest-physical
     * addresses in the 1-GByte page mapped by this entry (see Section 28.3.3.2)
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
    UINT64 SupervisorShadowStack : 1;
...
#define EPT_PDPTE_1GB_SUPERVISOR_SHADOW_STACK(_) (((_) >> 60) & 0x01)
    ...
    /**
     * [Bit 63] Suppress \#VE. If the "EPT-violation \#VE" VM-execution control
     * is 1, EPT violations caused by accesses to this page are convertible to
     * virtualization exceptions only if this bit is 0. If "EPT-violation \#VE"
     * VMexecution control is 0, this bit is ignored.
     *
...
/**
 * @brief Format of an EPT Page-Directory-Pointer-Table Entry (PDPTE) that
...
#define EPT_PDPTE_WRITE_ACCESS(_) (((_) >> 1) & 0x01)
     ...
     * [Bit 2] If the "mode-based execute control for EPT" VM-execution control
     * is 0, execute access; indicates whether instruction fetches are allowed
     * from the 1-GByte region controlled by this entry. If that control is 1,
     * execute access for supervisor-mode linear addresses; indicates whether
     * instruction fetches are allowed from supervisor-mode linear addresses in
...
#define EPT_PDPTE_EXECUTE_ACCESS(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 8] If bit 6 of EPTP is 1, accessed flag for EPT; indicates whether
     * software has accessed the 1-GByte region controlled by this entry.
     * Ignored if bit 6 of EPTP is 0.
     *
     * @see Vol3C[28.2.4(Accessed and Dirty Flags for EPT)]
     */
...
#define EPT_PDPTE_ACCESSED(_) (((_) >> 8) & 0x01)
     ...
     * "mode-based execute control for EPT" VM-execution control is 1, indicates
     * whether instruction fetches are allowed from user-mode linear addresses
     * in the 1-GByte region controlled by this entry. If that control is 0,
     * this bit is ignored.
     */
    UINT64 UserModeExecute : 1;
...
typedef union {
  struct {
    /**
     * [Bit 0] Read access; indicates whether reads are allowed from the 2-MByte
     * page referenced by this entry.
     */
    UINT64 ReadAccess : 1;
...
#define EPT_PDE_2MB_WRITE_ACCESS(_) (((_) >> 1) & 0x01)
     ...
     * [Bit 2] If the "mode-based execute control for EPT" VM-execution control
     * is 0, execute access; indicates whether instruction fetches are allowed
     * from the 2-MByte page controlled by this entry. If that control is 1,
     * execute access for supervisor-mode linear addresses; indicates whether
     * instruction fetches are allowed from supervisor-mode linear addresses in
...
#define EPT_PDE_2MB_LARGE_PAGE(_) (((_) >> 7) & 0x01)
    ...
    /**
     * [Bit 8] If bit 6 of EPTP is 1, accessed flag for EPT; indicates whether
     * software has accessed the 2-MByte page referenced by this entry. Ignored
     * if bit 6 of EPTP is 0.
     *
     * @see Vol3C[28.2.4(Accessed and Dirty Flags for EPT)]
     */
...
#define EPT_PDE_2MB_ACCESSED(_) (((_) >> 8) & 0x01)
    ...
    /**
     * [Bit 9] If bit 6 of EPTP is 1, dirty flag for EPT; indicates whether
     * software has written to the 2-MByte page referenced by this entry.
     * Ignored if bit 6 of EPTP is 0.
     *
     * @see Vol3C[28.2.4(Accessed and Dirty Flags for EPT)]
     */
...
#define EPT_PDE_2MB_DIRTY(_) (((_) >> 9) & 0x01)
     ...
     * "mode-based execute control for EPT" VM-execution control is 1, indicates
     * whether instruction fetches are allowed from user-mode linear addresses
     * in the 2-MByte page controlled by this entry. If that control is 0, this
     * bit is ignored.
     */
    UINT64 UserModeExecute : 1;
...
#define EPT_PDE_2MB_USER_MODE_EXECUTE(_) (((_) >> 10) & 0x01)
    ...
    /**
     * [Bits 47:21] Physical address of 4-KByte aligned EPT
     * page-directory-pointer table referenced by this entry.
     */
    UINT64 PageFrameNumber : 27;
...
#define EPT_PDE_2MB_PAGE_FRAME_NUMBER(_) (((_) >> 21) & 0x7FFFFFF)
     ...
     * [Bit 57] Verify guest paging. If the "guest-paging verification"
     * VM-execution control is 1, indicates limits on the guest paging
     * structures used to access the 2-MByte page controlled by this entry (see
     * Section 28.3.3.2). If that control is 0, this bit is ignored.
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
...
#define EPT_PDE_2MB_VERIFY_GUEST_PAGING(_) (((_) >> 57) & 0x01)
     ...
     * [Bit 58] Paging-write access. If the "EPT paging-write control"
     * VM-execution control is 1, indicates that guest paging may update the
     * 2-MByte page controlled by this entry (see Section 28.3.3.2). If that
     * control is 0, this bit is ignored
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
...
#define EPT_PDE_2MB_PAGING_WRITE_ACCESS(_) (((_) >> 58) & 0x01)
     ...
     * [Bit 60] Supervisor shadow stack. If bit 7 of EPTP is 1, indicates
     * whether supervisor shadow stack accesses are allowed to guest-physical
     * addresses in the 2-MByte page mapped by this entry (see Section 28.3.3.2)
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
    UINT64 SupervisorShadowStack : 1;
...
/**
 * @brief Format of an EPT Page-Directory Entry (PDE) that References an EPT
 * Page Table
 */
typedef union {
  struct {
...
#define EPT_PDE_WRITE_ACCESS(_) (((_) >> 1) & 0x01)
     ...
     * [Bit 2] If the "mode-based execute control for EPT" VM-execution control
     * is 0, execute access; indicates whether instruction fetches are allowed
     * from the 2-MByte region controlled by this entry. If that control is 1,
     * execute access for supervisor-mode linear addresses; indicates whether
     * instruction fetches are allowed from supervisor-mode linear addresses in
...
#define EPT_PDE_EXECUTE_ACCESS(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 8] If bit 6 of EPTP is 1, accessed flag for EPT; indicates whether
     * software has accessed the 2-MByte region controlled by this entry.
     * Ignored if bit 6 of EPTP is 0.
     *
     * @see Vol3C[28.2.4(Accessed and Dirty Flags for EPT)]
     */
...
#define EPT_PDE_ACCESSED(_) (((_) >> 8) & 0x01)
     ...
     * "mode-based execute control for EPT" VM-execution control is 1, indicates
     * whether instruction fetches are allowed from user-mode linear addresses
     * in the 2-MByte region controlled by this entry. If that control is 0,
     * this bit is ignored.
     */
    UINT64 UserModeExecute : 1;
...
#define EPT_PDE_USER_MODE_EXECUTE(_) (((_) >> 10) & 0x01)
    ...
    /**
     * [Bits 47:12] Physical address of 4-KByte aligned EPT page table
     * referenced by this entry.
     */
    UINT64 PageFrameNumber : 36;
...
typedef union {
  struct {
    /**
     * [Bit 0] Read access; indicates whether reads are allowed from the 4-KByte
     * page referenced by this entry.
     */
    UINT64 ReadAccess : 1;
...
#define EPT_PTE_WRITE_ACCESS(_) (((_) >> 1) & 0x01)
     ...
     * [Bit 2] If the "mode-based execute control for EPT" VM-execution control
     * is 0, execute access; indicates whether instruction fetches are allowed
     * from the 4-KByte page controlled by this entry. If that control is 1,
     * execute access for supervisor-mode linear addresses; indicates whether
     * instruction fetches are allowed from supervisor-mode linear addresses in
...
#define EPT_PTE_IGNORE_PAT(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 8] If bit 6 of EPTP is 1, accessed flag for EPT; indicates whether
     * software has accessed the 4-KByte page referenced by this entry. Ignored
     * if bit 6 of EPTP is 0.
     *
     * @see Vol3C[28.2.4(Accessed and Dirty Flags for EPT)]
     */
...
#define EPT_PTE_ACCESSED(_) (((_) >> 8) & 0x01)
    ...
    /**
     * [Bit 9] If bit 6 of EPTP is 1, dirty flag for EPT; indicates whether
     * software has written to the 4-KByte page referenced by this entry.
     * Ignored if bit 6 of EPTP is 0.
     *
     * @see Vol3C[28.2.4(Accessed and Dirty Flags for EPT)]
     */
...
#define EPT_PTE_DIRTY(_) (((_) >> 9) & 0x01)
     ...
     * "mode-based execute control for EPT" VM-execution control is 1, indicates
     * whether instruction fetches are allowed from user-mode linear addresses
     * in the 4-KByte page controlled by this entry. If that control is 0, this
     * bit is ignored.
     */
    UINT64 UserModeExecute : 1;
...
#define EPT_PTE_PAGE_FRAME_NUMBER(_) (((_) >> 12) & 0xFFFFFFFFF)
     ...
     * [Bit 57] Verify guest paging. If the "guest-paging verification"
     * VM-execution control is 1, indicates limits on the guest paging
     * structures used to access the 4-KByte page controlled by this entry (see
     * Section 28.3.3.2). If that control is 0, this bit is ignored.
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
...
#define EPT_PTE_VERIFY_GUEST_PAGING(_) (((_) >> 57) & 0x01)
     ...
     * [Bit 58] Paging-write access. If the "EPT paging-write control"
     * VM-execution control is 1, indicates that guest paging may update the
     * 4-KByte page controlled by this entry (see Section 28.3.3.2). If that
     * control is 0, this bit is ignored
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
...
#define EPT_PTE_PAGING_WRITE_ACCESS(_) (((_) >> 58) & 0x01)
     ...
     * [Bit 60] Supervisor shadow stack. If bit 7 of EPTP is 1, indicates
     * whether supervisor shadow stack accesses are allowed to guest-physical
     * addresses in the 4-KByte page mapped by this entry (see Section 28.3.3.2)
     *
     * @see Vol3C[28.3.3.2(EPT Violations)]
     */
    UINT64 SupervisorShadowStack : 1;
...
#define EPT_PTE_SUPERVISOR_SHADOW_STACK(_) (((_) >> 60) & 0x01)
    ...
    /**
     * [Bit 61] Sub-page write permissions. If the "sub-page write permissions
     * for EPT" VM-execution control is 1, writes to individual 128-byte regions
     * of the 4-KByte page referenced by this entry may be allowed even if the
     * page would normally not be writable (see Section 28.3.4). If "sub-page
     * write permissions for EPT" VM-execution control is 0, this bit is
     * ignored.
     *
...
typedef enum {
  /**
   * If the INVEPT type is 1, the logical processor invalidates all
  ...
  /**
   * If the INVEPT type is 2, the logical processor invalidates guest-physical
   * mappings and combined mappings associated with all EP4TAs (and, for
   * combined mappings, for all VPIDs and PCIDs).
   */
...
typedef enum {
  /**
   * If the INVVPID type is 0, the logical processor invalidates linear mappings
   * and combined mappings associated with the VPID specified in the INVVPID
   * descriptor and that would be used to translate the linear address specified
   * in of the INVVPID descriptor. Linear mappings and combined mappings for
   * that VPID and linear address are invalidated for all PCIDs and, for
   * combined mappings, all EP4TAs. (The instruction may also invalidate
   * mappings associated with other VPIDs and for other linear addresses).
   */
  InvvpidIndividualAddress = 0x00000000,

  /**
   * If the INVVPID type is 1, the logical processor invalidates all linear
   * mappings and combined mappings associated with the VPID specified in the
   * INVVPID descriptor. Linear mappings and combined mappings for that VPID are
   * invalidated for all PCIDs and, for combined mappings, all EP4TAs. (The
   * instruction may also invalidate mappings associated with other VPIDs).
   */
  InvvpidSingleContext = 0x00000001,

  /**
   * If the INVVPID type is 2, the logical processor invalidates linear mappings
   * and combined mappings associated with all VPIDs except VPID 0000H and with
   * all PCIDs. (The instruction may also invalidate linear mappings with VPID
   * 0000H.) Combined mappings are invalidated for all EP4TAs.
   */
  InvvpidAllContext = 0x00000002,

  /**
   * If the INVVPID type is 3, the logical processor invalidates linear mappings
   * and combined mappings associated with the VPID specified in the INVVPID
   * descriptor. Linear mappings and combined mappings for that VPID are
   * invalidated for all PCIDs and, for combined mappings, all EP4TAs. The
   * logical processor is not required to invalidate information that was used
   * for global translations (although it may do so). (The instruction may also
 ...
 *
 * The hypervisor-managed linear-address translation pointer (HLAT pointer or
 * HLATP) is used by HLAT paging to locate and access the first paging structure
...
typedef union {
  struct {
    ...
    /**
     * [Bit 3] Page-level write-through; indirectly determines the memory type
     * used to access the first HLAT paging structure during linear-address
     * translation.
     */
    UINT64 PageLevelWriteThrough : 1;
...
#define HLAT_POINTER_PAGE_LEVEL_CACHE_DISABLE(_) (((_) >> 4) & 0x01)
    ...
    /**
     * [Bits 47:12] Guest-physical address (4KB-aligned) of the first HLAT
 ...
 *
 * A logical processor uses virtual-machine control data structures (VMCSs)
 * while it is in VMX operation. These manage transitions into and out of VMX
 * non-root operation (VM entries and VM exits) as well as processor behavior in
 * VMX non-root operation. This structure is manipulated by the new instructions
 * VMCLEAR, VMPTRLD, VMREAD, and VMWRITE. A VMCS region comprises up to
 * 4-KBytes. The exact size is implementation specific and can be determined by
 * consulting the VMX capability MSR IA32_VMX_BASIC.
 *
...
typedef struct {
  struct {
     ...
     *
     * [Bits 30:0] Processors that maintain VMCS data in different formats (see
     * below) use different VMCS revision identifiers. These identifiers enable
     * software to avoid using a VMCS region formatted for one processor on a
     * processor that uses a different format. Software should write the VMCS
     * revision identifier to the VMCS region before using that region for a
     * VMCS. The VMCS revision identifier is never written by the processor;
     * VMPTRLD fails if its operand references a VMCS region whose VMCS revision
     * identifier differs from that used by the processor. Software can discover
     * the VMCS revision identifier that a processor uses by reading the VMX
     ...
     *
     * [Bit 31] Software should clear or set the shadow-VMCS indicator depending
     * on whether the VMCS is to be an ordinary VMCS or a shadow VMCS. VMPTRLD
     * fails if the shadow-VMCS indicator is set and the processor does not
     * support the 1-setting of the "VMCS shadowing" VM-execution control.
     * Software can discover support for this setting by reading the VMX
     * capability MSR IA32_VMX_PROCBASED_CTLS2.
     *
     * @see Vol3C[24.10(VMCS TYPES ORDINARY AND SHADOW)]
     */
   ...
   *
   * The contents of these bits do not control processor operation in any way. A
   * logical processor writes a non-zero value into these bits if a VMX abort
   ...
   *
   * These parts of the VMCS control VMX non-root operation and the VMX
   * transitions. The format of these data is implementation-specific. To ensure
   * proper behavior in VMX operation, software should maintain the VMCS region
   * and related structures in writeback cacheable memory. Future
   * implementations may allow or require a different memory type. Software
 ...
 *
 * Before executing VMXON, software allocates a region of memory that the
 * logical processor uses to support VMX operation. This region is called the
 * VMXON region. A VMXON region comprises up to 4-KBytes. The exact size is
 * implementation specific and can be determined by consulting the VMX
 * capability MSR IA32_VMX_BASIC.
 *
...
typedef struct {
  struct {
     ...
     * @brief VMCS revision identifier
     *
     * [Bits 30:0] Before executing VMXON, software should write the VMCS
     * revision identifier to the VMXON region. (Specifically, it should write
     * the 31-bit VMCS revision identifier to bits 30:0 of the first 4 bytes of
   ...
   *
   * The format of these data is implementation-specific. To ensure proper
   * behavior in VMX operation, software should not access or modify the VMXON
   * region of a logical processor between execution of VMXON and VMXOFF on that
   * logical processor. Doing otherwise may lead to unpredictable behavior.
   *
   * @see Vol3C[24.11.4(Software Access to Related Structures)]
   * @see Vol3D[A.1(BASIC VMX INFORMATION)]
   */
 ...
 * VMREAD and VMWRITE. This enumerates all fields in the VMCS and their
 * encodings. Fields are grouped by width (16-bit, 32-bit, etc.) and type
 * (guest-state, host-state, etc.).
 *
...
typedef union {
  struct {
    /**
     * [Bit 0] Access type (0 = full; 1 = high); must be full for 16-bit,
 ...
 *
 * @remarks This field exists only on processors that support the 1-setting of
 * the "enable VPID" VM-execution control.
 */
 ...
 *
 * @remarks This field exists only on processors that support the 1-setting of
 * the "process posted interrupts" VM-execution control.
 */
 ...
 *
 * @remarks This field exists only on processors that support the 1-setting of
 * the "enable HLAT" VM-execution control.
 */
 ...
 *
 * @remarks This field exists only on processors that support the 1-setting of
 * the "virtual-interrupt delivery" VM-execution control.
 */
...
typedef enum {
  ...
  /**
   * Other event. This type is used for injection of events that are not
...
/**
 * @brief VM entry can be configured to conclude by delivering an event through
 * the IDT (after all guest state and MSRs have been loaded). This process is
 * called event injection and is controlled by these VM-entry control fields
 *
...
typedef union {
  struct {
     ...
     *
     * [Bits 7:0] Determines which entry in the IDT is used or which other event
     * is injected.
     */
    UINT32 Vector : 8;
...
#define VMENTRY_INTERRUPT_INFORMATION_INTERRUPTION_TYPE(_) (((_) >> 8) & 0x07)
     ...
     *
     * [Bit 11] Determines whether delivery pushes an error code on the guest
     * stack.
     */
...
#define VMENTRY_INTERRUPT_INFORMATION_DELIVER_ERROR_CODE(_) (((_) >> 11) & 0x01)
     ...
     * @brief Valid
     *
     * [Bit 31] VM entry injects an event if and only if the valid bit is 1. The
     * valid bit in this field is cleared on every VM exit.
     */
    UINT32 Valid : 1;
 ...
 * called event injection and is controlled by these VM-entry control fields
 *
 * @see Vol3A[24.9.2(Information for VM Exits Due to Vectored Events)]
 * (reference)
 */
typedef union {
  struct {
 ...
 * correct APIC operation, this address space must be mapped to an area of
 * memory that has been designated as strong uncacheable (UC).
 *
 * @remarks Registers are 32 bits, 64 bits, or 256 bits in width; all are
 * aligned on 128-bit boundaries. All 32-bit registers should be accessed using
 * 128-bit aligned 32-bit loads or stores. Some processors may support loads and
 * stores of less than 32 bits to some of the APIC registers. This is model
 * specific behavior and is not guaranteed to work on all processors. Any
 * FP/MMX/SSE access to an APIC register, or any access that touches bytes 4
 * through 15 of an APIC register may cause undefined behavior and must not be
 * executed. This undefined behavior could include hangs, incorrect results or
 *          unexpected exceptions, including machine checks, and may vary
 * between implementations. Wider registers (64-bit or 256-bit) must be accessed
 * using multiple 32-bit loads or stores, with all accesses being 128-bit
 * aligned.
...
typedef union {
  struct {
     ...
     * 2 - FAR-RET/IRET: Indicates the \#CP was caused by a FAR RET or IRET
     * instruction. 3 - ENDBRANCH: indicates the \#CP was due to missing
     * ENDBRANCH at target of an indirect call or jump instruction. 4 -
     * RSTORSSP: Indicates the \#CP was caused by a shadow-stack-restore token
     * check failure in the RSTORSSP instruction. 5 - SETSSBSY: Indicates \#CP
     * was caused by a supervisor shadow stack token check failure in the
...
#define CONTROL_PROTECTION_EXCEPTION_CPEC(_) (((_) >> 0) & 0x7FFF)
    ...
    /**
     * [Bit 15] If set to 1, indicates the \#CP occurred during enclave
     * execution.
     */
    UINT32 Encl : 1;
...
/**
 * @brief Exceptions that can occur when the instruction is executed in
 * protected mode. Each exception is given a mnemonic that consists of a pound
 * sign (\#) followed by two letters and an optional error code in parentheses.
 * For example, \#GP(0) denotes a general protection exception with an error
 * code of 0
 *
...
typedef enum {
   ...
   * Error Code: No.
   *
   * @note Processors after the Intel386 processor do not generate this
   * exception.
   */
   ...
   * Source: Control flow transfer attempt violated the control flow enforcement
   * technology constraints. Error Code: Yes.
   */
  ControlProtection = 0x00000015,
} EXCEPTION_VECTOR;

/**
 * @brief When an exception condition is related to a specific segment selector
 * or IDT vector, the processor pushes an error code onto the stack of the
 * exception handler (whether it is a procedure or task). The error code
 * resembles a segment selector; however, instead of a TI flag and RPL field,
 * the error code contains 3 different flags
 *
...
typedef union {
  struct {
     ...
     * exception. The bit is cleared if the exception occurred during delivery
...
#define EXCEPTION_ERROR_CODE_DESCRIPTOR_LOCATION(_) (((_) >> 1) & 0x01)
     ...
     * gate descriptor in the LDT; when clear, it indicates that the index
...
#define EXCEPTION_ERROR_CODE_GDT_LDT(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bits 15:3] The segment selector index field provides an index into the
     * IDT, GDT, or current LDT to the segment or gate selector being referenced
     * by the error code. In some cases the error code is null (all bits are
     * clear except possibly EXT). A null error code indicates that the error
     * was not caused by a reference to a specific segment or that a null
     * segment selector was referenced in an operation.
     *
     * @note The format of the error code is different for page-fault exceptions
     * (#PF).
     */
    UINT32 Index : 13;
...
typedef union {
  struct {
    /**
     * [Bit 0] This flag is 0 if there is no translation for the linear address
     * because the P flag was 0 in one of the pagingstructure entries used to
...
#define PAGE_FAULT_EXCEPTION_PRESENT(_) (((_) >> 0) & 0x01)
    ...
    /**
     * [Bit 1] If the access causing the page-fault exception was a write, this
     * flag is 1; otherwise, it is 0. This flag describes the access causing the
...
#define PAGE_FAULT_EXCEPTION_WRITE(_) (((_) >> 1) & 0x01)
    ...
    /**
     * [Bit 2] If a user-mode access caused the page-fault exception, this flag
     * is 1; it is 0 if a supervisor-mode access did so. This flag describes the
...
#define PAGE_FAULT_EXCEPTION_USER_MODE_ACCESS(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] This flag is 1 if there is no translation for the linear address
     * because a reserved bit was set in one of the pagingstructure entries used
     * to translate that address. (Because reserved bits are not checked in a
     * paging-structure entry whose P flag is 0, bit 3 of the error code can be
     * set only if bit 0 is also set). Bits reserved in the paging-structure
     * entries are reserved for future functionality. Software developers should
     * be aware that such bits may be used in the future and that a
     * paging-structure entry that causes a page-fault exception on one
...
#define PAGE_FAULT_EXCEPTION_RESERVED_BIT_VIOLATION(_) (((_) >> 3) & 0x01)
    ...
    /**
     * [Bit 4] This flag is 1 if (1) the access causing the page-fault exception
     * was an instruction fetch; and (2) either (a) CR4.SMEP = 1; or (b) both
     * (i) CR4.PAE = 1 (either PAE paging or 4-level paging is in use); and (ii)
     * IA32_EFER.NXE = 1. Otherwise, the flag is 0. This flag describes the
     * access causing the page-fault exception, not the access rights specified
     * by paging.
     */
    UINT32 Execute : 1;
...
#define PAGE_FAULT_EXCEPTION_EXECUTE(_) (((_) >> 4) & 0x01)
    ...
    /**
     * [Bit 5] This flag is 1 if (1) IA32_EFER.LMA = CR4.PKE = 1; (2) the access
     ...
     * register is such that either (a) ADi = 1; or (b) the following all hold:
     * (i) WDi = 1; (ii) the access is a write access; and (iii) either CR0.WP =
...
#define PAGE_FAULT_EXCEPTION_PROTECTION_KEY_VIOLATION(_) (((_) >> 5) & 0x01)
    ...
    /**
     * [Bit 6] If the access causing the page-fault exception was a shadow-stack
     * access (including shadow-stack accesses in enclave mode), this flag is 1;
...
#define PAGE_FAULT_EXCEPTION_SHADOW_STACK(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 7] This flag is 1 if there is no translation for the linear address
     * using HLAT paging because, in one of the paging- structure entries used
     * to translate that address, either the P flag was 0 or a reserved bit was
     * set. An error code will set this flag only if it clears bit o or sets
     * bit 3. This flag will not be set by a page fault resulting from a
     * violation of access rights, nor for one encountered during ordinary
     * paging, including the case in which there has been a restart of HLAT
     * paging.
     *
     * @see Vol3A[4.5.1(Ordinary Paging and HLAT Paging)]
     */
...
#define PAGE_FAULT_EXCEPTION_HLAT(_) (((_) >> 7) & 0x01)
    ...
    /**
     * [Bit 15] This flag is 1 if the exception is unrelated to paging and
     * resulted from violation of SGX-specific access-control requirements.
     * Because such a violation can occur only if there is no ordinary page
     * fault, this flag is set only if the P flag (bit 0) is 1 and the RSVD flag
 ...
 *           Memory caching type
 *
 * The processor allows any area of system memory to be cached in the L1, L2,
 * and L3 caches. In individual pages or regions of system memory, it allows the
 * type of caching (also called memory type) to be specified.
 *
 ...
 * system bus and are executed in program order without reordering. No
 * speculative memory accesses, pagetable walks, or prefetches of speculated
 * branch targets are made. This type of cache-control is useful for
 * memory-mapped I/O devices. When used with normal RAM, it greatly reduces
 ...
 * Speculative reads are allowed. Writes may be delayed and combined in the
 * write combining buffer (WC buffer) to reduce memory accesses. If the WC
 * buffer is partially filled, the writes may be delayed until the next
 ...
 * or a LOCK instruction execution. This type of cache-control is appropriate
 * for video frame buffers, where the order of writes is unimportant as long as
 * the writes update memory so they can be seen on the graphics display. This
 * memory type is available in the Pentium Pro and Pentium II processors by
 * programming the MTRRs; or in processor families starting from the Pentium III
 * processors by programming the MTRRs or by selecting it through the PAT.
 *
 ...
 * allowed. All writes are written to a cache line (when possible) and through
 * to system memory. When writing through to memory, invalid cache lines are
 ...
 * buffers or when there are devices on the system bus that access system
 * memory, but do not perform snooping of memory accesses. It enforces coherency
 ...
 * on all processors on the bus to be invalidated. Speculative reads are
 * allowed. This memory type is available in processor families starting from
 ...
 * Writes and reads to and from system memory are cached. Reads come from cache
 * lines on cache hits; read misses cause cache fills. Speculative reads are
 * allowed. Write misses cause cache line fills (in processor families starting
 * with the P6 family processors), and writes are performed entirely in the
 * cache, when possible. Write combining is allowed. The write-back memory type
 * reduces bus traffic by eliminating many unnecessary writes to system memory.
 ...
 *
 * Has same characteristics as the strong uncacheable (UC) memory type, except
 * that this memory type can be overridden by programming the MTRRs for the WC
 * memory type. This memory type is available in processor families starting
 * from the Pentium III processors and can only be selected through the PAT.
 */
 ...
 */
/**
 * @brief The Root Table Address Register points to a table of root-entries,
...
#define VTD_Lower64_PRESENT(_) (((_) >> 0) & 0x01)
      ...
      /**
       * [Bits 63:12] Pointer to Context-table for this bus. The Context-table
       * is 4KB in size and size aligned. Hardware treats bits 63:HAW as
       * reserved (0), where HAW is the host address width of the platform.
       */
...
/**
 * @brief Context-entries support translation of requests-without-PASID.
 * Context-entries are referenced through root-entries
 *
...
#define VTD_Lower64_PRESENT(_) (((_) >> 0) & 0x01)
       ...
       * * 0: Qualified non-recoverable faults are recorded/reported for
       * requests processed through this context-entry.
       * * 1: Qualified non-recoverable faults are not recorded/reported for
       * requests processed through this context-entry. This field is evaluated
       * by hardware irrespective of the setting of the present (P) field.
       */
...
#define VTD_Lower64_FAULT_PROCESSING_DISABLE(_) (((_) >> 1) & 0x01)
       ...
       * * 10b: Untranslated requests are processed as pass-through. SLPTPTR
       * field is ignored by hardware. Translated and Translation Requests are
       * blocked. This encoding is treated by hardware as reserved for hardware
       * implementations not supporting Pass Through (PT=0 in Extended
       * Capability Register).
       * * 11b: Reserved.
       */
...
#define VTD_Lower64_TRANSLATION_TYPE(_) (((_) >> 2) & 0x03)
       ...
       * the host address width of the platform. This field is ignored by
       * hardware when Translation-Type (TT) field is 10b (pass-through).
       */
      UINT64 SecondLevelPageTranslationPointer : 52;
#define VTD_Lower64_SECOND_LEVEL_PAGE_TRANSLATION_POINTER_BIT 12
#define VTD_Lower64_SECOND_LEVEL_PAGE_TRANSLATION_POINTER_FLAG                 \
  0xFFFFFFFFFFFFF000
#define VTD_Lower64_SECOND_LEVEL_PAGE_TRANSLATION_POINTER_MASK 0xFFFFFFFFFFFFF
#define VTD_Lower64_SECOND_LEVEL_PAGE_TRANSLATION_POINTER(_)                   \
  (((_) >> 12) & 0xFFFFFFFFFFFFF)
    };
  ...
  union {
    struct {
      /**
       * [Bits 2:0] When the Translation-type (TT) field is 00b or 01b, this
       * field indicates the adjusted guest address- width (AGAW) to be used by
       * hardware for the second-level page-table walk. The following encodings
       ...
       * through this context-entry and accessing addresses above 2X-1 (where X
       * is the AGAW value indicated by this field) are blocked and treated as
       * translation faults.
       */
...
#define VTD_Upper64_IGNORED(_) (((_) >> 3) & 0x0F)
       ...
       * implementation supporting 8-bit domain-ids, bits 87:80 of this field
       * are treated as reserved. Context-entries programmed with the same
       * domain identifier must always reference same address translation
       * (SLPTPTR field). Context-entries referencing same address translation
       * are recommended to be programmed with same domain id for hardware
       * efficiency. When Caching Mode (CM) field in Capability Register is
       * reported as Set, the domain-id value of zero is architecturally
...
/**
 * Register to report the implementation version. Backward compatibility for the
 * architecture is maintained with new revision numbers, allowing software to
 * load remapping hardware drivers written for prior versions.
 *
 * @remarks VER_REG
...
#define VTD_CAPABILITY_NUMBER_OF_DOMAINS_SUPPORTED(_) (((_) >> 0) & 0x07)
     ...
     *
     * [Bit 3]
     * * 0: Indicates advanced fault logging is not supported. Only primary
     * fault logging is supported.
...
#define VTD_CAPABILITY_PROTECTED_HIGH_MEMORY_REGION(_) (((_) >> 6) & 0x01)
     ...
     *
     * [Bit 7] This field applies to all DMA and Interrupt remap tables except
     * FLtables. Hardware will not cache faulting FL-only translations in IOTLB
     ...
     * that result in decreasing the effective permissions or partial permission
     ...
     * updates to "not-present" or erroneous entries) require explicit
     * invalidation.
     */
...
#define VTD_CAPABILITY_CACHING_MODE(_) (((_) >> 7) & 0x01)
     ...
     *
     * [Bits 12:8] This 5-bit field indicates the supported adjusted guest
     * address widths (which in turn represents the levels of page-table walks
     * for the 4KB base page size) supported by the hardware implementation. A
     * value of 1 in any of these bits indicates the corresponding adjusted
     ...
     * this field. Hardware implementations reporting second-level translation
...
#define VTD_CAPABILITY_SUPPORTED_ADJUSTED_GUEST_ADDRESS_WIDTHS(_)              \
  (((_) >> 8) & 0x1F)
    UINT64 Reserved1 : 3;
     ...
     *
     * [Bits 21:16] This field indicates the maximum guest physical address
     * width supported by second-level translation in remapping hardware. The
     * Maximum Guest Address Width (MGAW) is computed as (N+1), where N is the
     * valued reported in this field. For example, a hardware implementation
     * supporting 48-bit MGAW reports a value of 47 (101111b) in this field. If
     * the value in this field is X, untranslated DMA requests with addresses
     * above 2(X+1)-1 that are subjected to second-level translation are blocked
     * by hardware. Device-TLB translation requests to addresses above 2(X+1)-1
     * that are subjected to second-level translation from allowed devices
     * return a null Translation-Completion Data with R=W=0. Guest
     * addressability for a given DMA request is limited to the minimum of the
     * value reported through this field and the adjusted guest address width of
...
#define VTD_CAPABILITY_MAXIMUM_GUEST_ADDRESS_WIDTH(_) (((_) >> 16) & 0x3F)
     ...
     *
     * [Bit 22]
     * * 0: Indicates the remapping hardware unit blocks (and treats as fault)
     * zero length DMA read requests to write-only pages.
     * * 1: Indicates the remapping hardware unit supports zero length DMA read
     * requests to write-only pages. DMA remapping hardware implementations are
...
#define VTD_CAPABILITY_ZERO_LENGTH_READ(_) (((_) >> 22) & 0x01)
     ...
     *
     * [Bit 23] This field must be reported as 0 to ensure backward
...
#define VTD_CAPABILITY_DEPRECATED(_) (((_) >> 23) & 0x01)
     ...
     * register relative to the register base address of this remapping hardware
     * unit. If the register base address is X, and the value reported in this
...
#define VTD_CAPABILITY_FAULT_RECORDING_REGISTER_OFFSET(_) (((_) >> 24) & 0x3FF)
     ...
     *
     * [Bits 37:34] This field indicates the large page sizes supported by
     * hardware. A value of 1 in any of these bits indicates the corresponding
     * large page size is supported. The large-page sizes corresponding to
     * various bit positions within this field are:
     * * 0: 21-bit offset to page frame (2MB)
     * * 1: 30-bit offset to page frame (1GB)
     * * 2: Reserved
     * * 3: Reserved
     * Hardware implementations supporting a specific large-page size must
     * support all smaller large-page sizes. i.e., only valid values for this
     * field are 0000b, 0001b, 0011b.
     */
...
#define VTD_CAPABILITY_SECOND_LEVEL_LARGE_PAGE_SUPPORT(_) (((_) >> 34) & 0x0F)
     ...
     *
     * [Bit 39]
     * * 0: Hardware supports only global and domain-selective invalidates for
     * IOTLB.
     * * 1: Hardware supports page-selective, domain-selective, and global
     * invalidates for IOTLB. Hardware implementations reporting this field as
     * Set are recommended to support a Maximum Address Mask Value (MAMV) value
     * of at least 9 (or 18 if supporting 1GB pages with second level
     * translation). This field is applicable only for IOTLB invalidations for
     * second-level translation. Irrespective of value reported in this field,
     * implementations supporting SMTS must support page/address selective IOTLB
...
#define VTD_CAPABILITY_PAGE_SELECTIVE_INVALIDATION(_) (((_) >> 39) & 0x01)
     ...
     *
     * [Bits 47:40] Number of fault recording registers is computed as N+1,
     * where N is the value reported in this field. Implementations must support
     * at least one fault recording register (NFR = 0) for each remapping
     * hardware unit in the platform. The maximum number of fault recording
...
#define VTD_CAPABILITY_NUMBER_OF_FAULT_RECORDING_REGISTERS(_)                  \
  (((_) >> 40) & 0xFF)

    /**
     * @brief Maximum Address Mask Value <b>(RO)</b>
     *
     * [Bits 53:48] The value in this field indicates the maximum supported
     * value for the Address Mask (AM) field in the Invalidation Address
     * register (IVA_REG), and IOTLB Invalidation Descriptor (iotlb_inv_dsc)
     * used for invalidations of second-level translation. This field is valid
     * when the PSI field in Capability register is reported as Set. Independent
     * of value reported in this field, implementations supporting SMTS must
     * support address-selective PASID-based IOTLB invalidations
     * (p_iotlb_inv_dsc) with any defined address mask.
     */
...
#define VTD_CAPABILITY_MAXIMUM_ADDRESS_MASK_VALUE(_) (((_) >> 48) & 0x3F)
     ...
     *
     * [Bit 54]
     * * 0: Hardware does not support draining of write requests on IOTLB
     * Invalidation.
...
#define VTD_CAPABILITY_WRITE_DRAINING(_) (((_) >> 54) & 0x01)
     ...
     *
     * [Bit 55]
     * * 0: Hardware does not support draining of read requests on IOTLB
     * Invalidation.
...
#define VTD_CAPABILITY_READ_DRAINING(_) (((_) >> 55) & 0x01)
     ...
     *
     * [Bit 56] A value of 1 in this field indicates 1-GByte page size is
     * supported for first-level translation. Hardware implementation reporting
     * First-level Translation Support (FLTS) as Clear also report this field as
     * Clear.
     */
...
#define VTD_CAPABILITY_FIRST_LEVEL_1GBYTE_PAGE_SUPPORT(_) (((_) >> 56) & 0x01)
     ...
     * Hardware implementation reporting Interrupt Remapping support (IR) field
     * in Extended Capability Register as Clear also report this field as Clear.
     */
...
#define VTD_CAPABILITY_POSTED_INTERRUPTS_SUPPORT(_) (((_) >> 59) & 0x01)
     ...
     * Hardware implementation reporting First-level Translation Support (FLTS)
...
#define VTD_CAPABILITY_FIRST_LEVEL_5LEVEL_PAGING_SUPPORT(_) (((_) >> 60) & 0x01)
     ...
     *
     * [Bit 62]
     * * 0: Hardware does not invalidate all Interrupt remapping hardware
     * translation caches as part of SIRTP flow.
     * * 1: Hardware invalidates all Interrupt remapping hardware translation
...
#define VTD_CAPABILITY_ENHANCED_SET_INTERRUPT_REMAP_TABLE_POINTER_SUPPORT_FLAG \
  0x4000000000000000
#define VTD_CAPABILITY_ENHANCED_SET_INTERRUPT_REMAP_TABLE_POINTER_SUPPORT_MASK \
  0x01
#define VTD_CAPABILITY_ENHANCED_SET_INTERRUPT_REMAP_TABLE_POINTER_SUPPORT(_)   \
  (((_) >> 62) & 0x01)
     ...
     *
     * [Bit 63]
     * * 0: Hardware does not invalidate all DMA remapping hardware translation
     * caches as part of SRTP flow.
     * * 1: Hardware invalidates all DMA remapping hardware translation caches
...
#define VTD_CAPABILITY_ENHANCED_SET_ROOT_TABLE_POINTER_SUPPORT_FLAG            \
  0x8000000000000000
#define VTD_CAPABILITY_ENHANCED_SET_ROOT_TABLE_POINTER_SUPPORT_MASK 0x01
#define VTD_CAPABILITY_ENHANCED_SET_ROOT_TABLE_POINTER_SUPPORT(_)              \
  (((_) >> 63) & 0x01)
  };
...
typedef union {
  struct {
     ...
     *
     * [Bit 0] This field indicates if hardware access to the root,
     * scalable-mode root, context, scalable-mode-context, scalable-mode
     * PASIDdirectory, scalable-mode PASID-table, and interrupt-remap tables,
     * and legacy-mode second-level paging structures are coherent (snooped) or
     * not.
...
#define VTD_EXTENDED_CAPABILITY_QUEUED_INVALIDATION_SUPPORT(_)                 \
  (((_) >> 1) & 0x01)
     ...
     * Hardware implementation reporting Queued Invalidation support (QI) field
     * as Clear also report this field as Clear.
     */
...
#define VTD_EXTENDED_CAPABILITY_INTERRUPT_REMAPPING_SUPPORT(_)                 \
  (((_) >> 3) & 0x01)
     ...
     *
     * [Bit 4]
     * * 0: On Intel(R) 64 platforms, hardware supports only 8-bit APIC-IDs
     * (xAPIC Mode).
     * * 1: On Intel(R) 64 platforms, hardware supports 32-bit APICIDs (x2APIC
     * mode). Hardware implementation reporting Interrupt Remapping support (IR)
...
#define VTD_EXTENDED_CAPABILITY_EXTENDED_INTERRUPT_MODE(_) (((_) >> 4) & 0x01)
     ...
     *
     * [Bit 5] This field must be reported as 0 to ensure backward compatibility
     * with older software.
     */
...
#define VTD_EXTENDED_CAPABILITY_PASS_THROUGH(_) (((_) >> 6) & 0x01)
     ...
     * entries. Implementations are recommended to support Snoop Control to
     * support software usages that require Snoop Control for assignment of
     * devices behind a remapping hardware unit.
     */
...
#define VTD_EXTENDED_CAPABILITY_SNOOP_CONTROL(_) (((_) >> 7) & 0x01)
     ...
     *
     * [Bits 17:8] This field specifies the offset to the IOTLB registers
     * relative to the register base address of this remapping hardware unit. If
     * the register base address is X, and the value reported in this field is
     * Y, the address for the IOTLB registers is calculated as X+(16*Y).
     */
...
#define VTD_EXTENDED_CAPABILITY_IOTLB_REGISTER_OFFSET(_) (((_) >> 8) & 0x3FF)
     ...
     *
     * [Bits 23:20] The value in this field indicates the maximum supported
     * value for the Interrupt Mask (IM) field in the Interrupt Entry Cache
     * Invalidation Descriptor (iec_inv_dsc). This field is unused and is
     * reported as 0 if Interrupt Remapping support (IR) field is Clear.
     */
...
#define VTD_EXTENDED_CAPABILITY_MAXIMUM_HANDLE_MASK_VALUE(_)                   \
  (((_) >> 20) & 0x0F)

    /**
     * @brief Deprecated2 <b>(RO)</b>
     *
     * [Bit 24] In prior versions of this specification this bit was used to
     * enumerate "Extended mode address translation" which is now deprecated.
     * This field must be reported as 0 to ensure backward compatibility with
     * any software that enables extended mode address translation.
     */
...
#define VTD_EXTENDED_CAPABILITY_DEPRECATED2(_) (((_) >> 24) & 0x01)
     ...
     * * 1: Hardware supports Memory Type in first-level translation and
     * Extended Memory type in second-level translation. Hardware
     * implementations reporting Scalable Mode Translation Support (SMTS) field
     * as Clear also report this field as Clear. Remapping hardware units with,
     * one or more devices that operate in processor coherency domain, under its
     * scope must report this field as Set.
     */
...
#define VTD_EXTENDED_CAPABILITY_MEMORY_TYPE_SUPPORT(_) (((_) >> 25) & 0x01)
     ...
     * (SMTS) field as Clear or First-level Translation Support (FLTS) field as
     * Clear or Second-level Translation Support (SLTS) field as Clear also
...
#define VTD_EXTENDED_CAPABILITY_NESTED_TRANSLATION_SUPPORT(_)                  \
  (((_) >> 26) & 0x01)
    UINT64 Reserved2 : 1;
     ...
     *
     * [Bit 28] This field must be reported as 0 to ensure backward
     * compatibility with older software.
     */
...
#define VTD_EXTENDED_CAPABILITY_DEPRECATED3(_) (((_) >> 28) & 0x01)
     ...
     * Hardware implementation reporting Device-TLB support (DT) field as Clear
     * or Scalable Mode Translation Support (SMTS) field as Clear also report
     * this field as Clear.
     */
...
#define VTD_EXTENDED_CAPABILITY_PAGE_REQUEST_SUPPORT(_) (((_) >> 29) & 0x01)
     ...
     *
     * [Bit 30]
     * * 0: Hardware does not support requests-with-PASID seeking execute
     * permission.
     * * 1: Hardware supports requests-with-PASID seeking execute permission.
     * Hardware implementations reporting Process Address Space ID support
     * (PASID) field as Clear must report this field as Clear.
     */
...
#define VTD_EXTENDED_CAPABILITY_EXECUTE_REQUEST_SUPPORT(_) (((_) >> 30) & 0x01)
     ...
     * translation-requests. Hardware implementations reporting Device-TLB
     * support (DT) field as Clear also report this field as Clear.
     */
...
#define VTD_EXTENDED_CAPABILITY_NO_WRITE_FLAG_SUPPORT(_) (((_) >> 33) & 0x01)
     ...
     *
     * [Bit 34]
     * * 0: Hardware does not support the extended-accessed (EA) bit in
     * first-level paging-structure entries.
     * * 1: Hardware supports the extended-accessed (EA) bit in first-level
     * paging-structure entries. Hardware implementations reporting
     * Scalable-Mode Page-walk Coherency Support (SWPWCS) as Clear also report
     * this field as Clear.
     */
...
#define VTD_EXTENDED_CAPABILITY_EXTENDED_ACCESSED_FLAG_SUPPORT(_)              \
  (((_) >> 34) & 0x01)
     ...
     * hardware for requests-with-PASID. A value of N in this field indicates
     * hardware supports PASID field of N+1 bits (For example, value of 7 in
     * this field, indicates 8-bit PASIDs are supported). Requests-with-PASID
     * with PASID value beyond the limit specified by this field are treated as
     * error by the remapping hardware. This field is unused and reported as 0
     * if Scalable Mode Translation Support (SMTS) field is Clear.
     */
...
#define VTD_EXTENDED_CAPABILITY_PASID_SIZE_SUPPORTED(_) (((_) >> 35) & 0x1F)
     ...
     *
     * [Bit 40]
     * * 0: Hardware does not support requests tagged with Process Address Space
     * IDs.
     * * 1: Hardware supports requests tagged with Process Address Space IDs.
     * Hardware implementations reporting Scalable Mode Translation Support
     * (SMTS) field as Clear also report this field as Clear.
     */
    UINT64 ProcessAddressSpaceIdSupport : 1;
#define VTD_EXTENDED_CAPABILITY_PROCESS_ADDRESS_SPACE_ID_SUPPORT_BIT 40
#define VTD_EXTENDED_CAPABILITY_PROCESS_ADDRESS_SPACE_ID_SUPPORT_FLAG          \
  0x10000000000
#define VTD_EXTENDED_CAPABILITY_PROCESS_ADDRESS_SPACE_ID_SUPPORT_MASK 0x01
#define VTD_EXTENDED_CAPABILITY_PROCESS_ADDRESS_SPACE_ID_SUPPORT(_)            \
  (((_) >> 40) & 0x01)
     ...
     * Hardware implementations reporting Device-TLB support (DT) as Clear also
     * report this field as Clear.
     */
    UINT64 DeviceTlbInvalidationThrottle : 1;
#define VTD_EXTENDED_CAPABILITY_DEVICE_TLB_INVALIDATION_THROTTLE_BIT 41
#define VTD_EXTENDED_CAPABILITY_DEVICE_TLB_INVALIDATION_THROTTLE_FLAG          \
  0x20000000000
#define VTD_EXTENDED_CAPABILITY_DEVICE_TLB_INVALIDATION_THROTTLE_MASK 0x01
#define VTD_EXTENDED_CAPABILITY_DEVICE_TLB_INVALIDATION_THROTTLE(_)            \
  (((_) >> 41) & 0x01)
     ...
     *
     * [Bit 42]
     * * 0: Hardware does not support Page-request Drain (PD) flag in
     * inv_wait_dsc.
...
#define VTD_EXTENDED_CAPABILITY_PAGE_REQUEST_DRAIN_SUPPORT(_)                  \
  (((_) >> 42) & 0x01)
     ...
     * context-table and PASID-table structures. Hardware implementation
     * reporting Queued Invalidation (QI) field as Clear also report this field
     * as Clear. Hardware implementation reporting First-Level Translation
     * Support (FLTS), Second-level Translation Support (SLTS) and Pass-through
     * Support (PT) as Clear also report this field as Clear.
     */
    UINT64 ScalableModeTranslationSupport : 1;
#define VTD_EXTENDED_CAPABILITY_SCALABLE_MODE_TRANSLATION_SUPPORT_BIT 43
#define VTD_EXTENDED_CAPABILITY_SCALABLE_MODE_TRANSLATION_SUPPORT_FLAG         \
  0x80000000000
#define VTD_EXTENDED_CAPABILITY_SCALABLE_MODE_TRANSLATION_SUPPORT_MASK 0x01
#define VTD_EXTENDED_CAPABILITY_SCALABLE_MODE_TRANSLATION_SUPPORT(_)           \
  (((_) >> 43) & 0x01)
     ...
     *
     * [Bit 44]
     * * 0: Hardware does not support command submission to virtual-DMA
     * Remapping hardware.
     * * 1: Hardware does support command submission to virtual- DMA Remapping
     * hardware. Hardware implementations of this architecture report a value of
     * 0 in this field. Software implementations (emulation) of this
     * architecture may report VCS=1. Software managing remapping hardware
     * should be written to handle both values of VCS.
     */
...
#define VTD_EXTENDED_CAPABILITY_VIRTUAL_COMMAND_SUPPORT(_) (((_) >> 44) & 0x01)
     ...
     *
     * [Bit 45]
     * * 0: Hardware does not support Accessed/Dirty bits in Second- Level
     * translation.
     * * 1: Hardware supports Accessed/Dirty bits in Second-Level translation.
     * Hardware implementations reporting Scalable-Mode Page-walk Coherency
     * Support (SMPWCS) as Clear also report this field as Clear.
     */
    UINT64 SecondLevelAccessedDirtySupport : 1;
#define VTD_EXTENDED_CAPABILITY_SECOND_LEVEL_ACCESSED_DIRTY_SUPPORT_BIT 45
#define VTD_EXTENDED_CAPABILITY_SECOND_LEVEL_ACCESSED_DIRTY_SUPPORT_FLAG       \
  0x200000000000
#define VTD_EXTENDED_CAPABILITY_SECOND_LEVEL_ACCESSED_DIRTY_SUPPORT_MASK 0x01
#define VTD_EXTENDED_CAPABILITY_SECOND_LEVEL_ACCESSED_DIRTY_SUPPORT(_)         \
  (((_) >> 45) & 0x01)
     ...
     *
     * [Bit 46]
     * * 0: Hardware does not support PASID Granular Translation Type of
     * second-level (PGTT=010b) in scalable-mode PASIDTable entry.
...
#define VTD_EXTENDED_CAPABILITY_SECOND_LEVEL_TRANSLATION_SUPPORT_FLAG          \
  0x400000000000
#define VTD_EXTENDED_CAPABILITY_SECOND_LEVEL_TRANSLATION_SUPPORT_MASK 0x01
#define VTD_EXTENDED_CAPABILITY_SECOND_LEVEL_TRANSLATION_SUPPORT(_)            \
  (((_) >> 46) & 0x01)
     ...
     * [Bit 47]
     * * 0: Hardware does not support PASID Granular Translation Type of
     * first-level (PGTT=001b) in scalable-mode PASIDTable entry.
     * * 1: Hardware supports PASID Granular Translation Type of first-level
     * (PGTT=001b) in scalable-mode PASID-Table entry. Hardware implementations
     * reporting Scalable Mode Translation Support (SMTS) as Clear also report
     * this field as Clear
     */
    UINT64 FirstLevelTranslationSupport : 1;
#define VTD_EXTENDED_CAPABILITY_FIRST_LEVEL_TRANSLATION_SUPPORT_BIT 47
#define VTD_EXTENDED_CAPABILITY_FIRST_LEVEL_TRANSLATION_SUPPORT_FLAG           \
  0x800000000000
#define VTD_EXTENDED_CAPABILITY_FIRST_LEVEL_TRANSLATION_SUPPORT_MASK 0x01
#define VTD_EXTENDED_CAPABILITY_FIRST_LEVEL_TRANSLATION_SUPPORT(_)             \
  (((_) >> 47) & 0x01)
     ...
     *
     * [Bit 48]
     * * 0: Hardware access to paging structures accessed through PASID-table
     * entry are not snooped.
     * * 1: Hardware access to paging structures accessed through PASID-table
     * entry are snooped if PWSNP field in PASID-table entry is Set.
     * Paging-structures accessed through PASID-table entry are not snooped if
     * PWSNP field in PASID-table entry is Clear. Hardware implementations
     * reporting Scalable Mode Translation Support (SMTS) as Clear also report
     * this field as Clear.
     */
    UINT64 ScalableModePageWalkCoherency : 1;
#define VTD_EXTENDED_CAPABILITY_SCALABLE_MODE_PAGE_WALK_COHERENCY_BIT 48
#define VTD_EXTENDED_CAPABILITY_SCALABLE_MODE_PAGE_WALK_COHERENCY_FLAG         \
  0x1000000000000
#define VTD_EXTENDED_CAPABILITY_SCALABLE_MODE_PAGE_WALK_COHERENCY_MASK 0x01
#define VTD_EXTENDED_CAPABILITY_SCALABLE_MODE_PAGE_WALK_COHERENCY(_)           \
  (((_) >> 48) & 0x01)
     ...
     * context-entry. Hardware implementations reporting Scalable Mode
     * Translation Support (SMTS) as Clear also report this field as Clear.
     */
...
#define VTD_EXTENDED_CAPABILITY_ABORT_DMA_MODE_SUPPORT(_) (((_) >> 52) & 0x01)
     ...
     * context-entry. Hardware implementations reporting Supervisor Request
     * Support (SRS) as Clear also report this field as Clear.
     */
...
/**
 * Register to control remapping hardware. If multiple control fields in this
 * register need to be modified, software must serialize the modifications
 * through multiple writes to this register. For example, to update a bit field
 * in this register at offset X with value of Y, software must follow below
 * steps:
 * 1. Tmp = Read GSTS_REG
 * 2. Status = (Tmp & 96FFFFFFh) // Reset the one-shot bits
 * 3. if (Y) {Command = (Status | (1 << X))} else {Command = (Status & ~(1 <<
 * X))}
...
typedef union {
  struct {
     ...
     *
     * [Bit 23] This field is valid only for Intel(R) 64 implementations
     * supporting interrupt remapping. Software writes to this field to enable
     * or disable Compatibility Format interrupts on Intel(R) 64 platforms. The
     * value in this field is effective only when interrupt-remapping is enabled
     * and Extended Interrupt Mode (x2APIC mode) is not enabled.
     * * 0: Block Compatibility format interrupts.
     * * 1: Process Compatibility format interrupts as pass-through (bypass
     * interrupt remapping). Hardware reports the status of updating this field
     * through the CFIS field in the Global Status register. The value returned
...
#define VTD_GLOBAL_COMMAND_COMPATIBILITY_FORMAT_INTERRUPT(_)                   \
  (((_) >> 23) & 0x01)

    /**
     * @brief Set Interrupt Remap Table Pointer <b>(WO)</b>
     *
     * [Bit 24] This field is valid only for implementations supporting
     * interrupt remapping. Software sets this field to set/update the interrupt
     * remapping table pointer used by hardware. The interrupt remapping table
     * pointer is specified through the Interrupt Remapping Table Address
     * (IRTA_REG) register. Hardware reports the status of the 'Set Interrupt
     * Remap Table Pointer' operation through the IRTPS field in the Global
     * Status register.
     ...
     * hardware through the IRE field. Clearing this bit has no effect. The
     * value returned on a read of this field is undefined.
     */
...
#define VTD_GLOBAL_COMMAND_SET_INTERRUPT_REMAP_TABLE_POINTER(_)                \
  (((_) >> 24) & 0x01)

    /**
     * @brief Interrupt Remapping Enable <b>(WO)</b>
     *
     * [Bit 25] This field is valid only for implementations supporting
     * interrupt remapping.
     ...
     * through the IRES field in the Global Status register. There may be active
     * interrupt requests in the platform when software updates this field.
     * Hardware must enable or disable interrupt-remapping logic only at
     * deterministic transaction boundaries, so that any in-flight interrupts
     * are either subject to remapping or not at all. For implementations
     * reporting the Enhanced Set Interrupt Remap Table Pointer Support
     * (ESIRTPS) field as Set, hardware performs global invalidation on all
     * Interrupt remapping caches as part of Interrupt Remapping Disable
     * operation. Hardware implementations must drain any in-flight interrupts
     * requests queued in the Root-Complex before completing the
     * interrupt-remapping enable command and reflecting the status of the
     * command through the IRES field in the Global Status register. The value
...
#define VTD_GLOBAL_COMMAND_INTERRUPT_REMAPPING_ENABLE(_) (((_) >> 25) & 0x01)
     ...
     * through QIES field in the Global Status register. The value returned on a
...
#define VTD_GLOBAL_COMMAND_QUEUED_INVALIDATION_ENABLE(_) (((_) >> 26) & 0x01)
     ...
     *
     * [Bit 27] This bit is valid only for implementations requiring write
     * buffer flushing. Software sets this field to request that hardware flush
     * the Root-Complex internal write buffers. This is done to ensure any
     * updates to the memory resident remapping structures are not held in any
     * internal write posting buffers. Hardware reports the status of the write
     * buffer flushing operation through the WBFS field in the Global Status
     * register. Clearing this bit has no effect. The value returned on a read
...
#define VTD_GLOBAL_COMMAND_WRITE_BUFFER_FLUSH(_) (((_) >> 27) & 0x01)
     ...
     *
     * [Bit 28] This field is valid only for implementations supporting advanced
     * fault logging. Software writes to this field to request hardware to
     ...
     * must be set in hardware (through the SFL field) before enabling advanced
     * fault logging. Hardware reports the status of the advanced fault logging
     * enable operation through the AFLS field in the Global Status register.
...
#define VTD_GLOBAL_COMMAND_ENABLE_ADVANCED_FAULT_LOGGING(_) (((_) >> 28) & 0x01)
     ...
     *
     * [Bit 29] This field is valid only for implementations supporting advanced
     * fault logging. Software sets this field to request hardware to set/update
     * the fault-log pointer used by hardware. The fault-log pointer is
     * specified through Advanced Fault Log register. Hardware reports the
     * status of the 'Set Fault Log' operation through the FLS field in the
     * Global Status register. The fault log pointer must be set before enabling
     * advanced fault logging (through EAFL field). Once advanced fault logging
     * is enabled, the fault log pointer may be updated through this field while
     * DMA remapping is active. Clearing this bit has no effect. The value
...
#define VTD_GLOBAL_COMMAND_SET_FAULT_LOG(_) (((_) >> 29) & 0x01)
     ...
     *
     * [Bit 30] Software sets this field to set/update the root-table pointer
     * (and translation table mode) used by hardware. The root-table pointer
     * (and translation table mode) is specified through the Root Table Address
     * (RTADDR_REG) register. Hardware reports the status of the 'Set Root Table
     * Pointer' operation through the RTPS field in the Global Status register.
     * The 'Set Root Table Pointer' operation must be performed before enabling
     * or re-enabling (after disabling) DMA remapping through the TE field. For
     * details on invalidation that software may have to perform after the
...
#define VTD_GLOBAL_COMMAND_SET_ROOT_TABLE_POINTER(_) (((_) >> 30) & 0x01)
     ...
     *
     * [Bit 31] Software writes to this field to request hardware to
     ...
     * the TES field in the Global Status register. There may be active DMA
     * requests in the platform when software updates this field. Hardware must
     * enable or disable remapping logic only at deterministic transaction
     * boundaries, so that any in-flight transaction is either subject to
     ...
     * through the TES field in the Global Status register. For implementations
     * reporting Scalable Mode Translation Support (SMTS) field as Set, hardware
     * performs global invalidation on all DMA remapping translation caches as
     * part of Translation Disable operation. The value returned on a read of
...
typedef union {
  struct {
     ...
     *
     * [Bit 23] This field indicates the status of Compatibility format
     * interrupts on Intel(R) 64 implementations supporting interrupt-remapping.
     * The value reported in this field is applicable only when
     * interrupt-remapping is enabled and extended interrupt mode (x2APIC mode)
     * is not enabled.
...
#define VTD_GLOBAL_STATUS_COMPATIBILITY_FORMAT_INTERRUPT_STATUS(_)             \
  (((_) >> 23) & 0x01)

    /**
     * @brief Interrupt Remapping Table Pointer Status <b>(RO)</b>
     *
     * [Bit 24] This field indicates the status of the interrupt remapping table
     * pointer in hardware. This field is cleared by hardware when software sets
     * the SIRTP field in the Global Command register. This field is Set by
     * hardware when hardware completes the 'Set Interrupt Remap Table Pointer'
     * operation using the value provided in the Interrupt Remapping Table
     * Address register.
     */
    UINT32 InterruptRemappingTablePointerStatus : 1;
#define VTD_GLOBAL_STATUS_INTERRUPT_REMAPPING_TABLE_POINTER_STATUS_BIT 24
#define VTD_GLOBAL_STATUS_INTERRUPT_REMAPPING_TABLE_POINTER_STATUS_FLAG        \
  0x1000000
#define VTD_GLOBAL_STATUS_INTERRUPT_REMAPPING_TABLE_POINTER_STATUS_MASK 0x01
#define VTD_GLOBAL_STATUS_INTERRUPT_REMAPPING_TABLE_POINTER_STATUS(_)          \
  (((_) >> 24) & 0x01)
...
#define VTD_GLOBAL_STATUS_INTERRUPT_REMAPPING_ENABLE_STATUS(_)                 \
  (((_) >> 25) & 0x01)
...
#define VTD_GLOBAL_STATUS_QUEUED_INVALIDATION_ENABLE_STATUS(_)                 \
  (((_) >> 26) & 0x01)

    /**
     * @brief Write Buffer Flush Status <b>(RO)</b>
     *
     * [Bit 27] This field is valid only for implementations requiring write
     * buffer flushing. This field indicates the status of the write buffer
     * flush command. It is
...
#define VTD_GLOBAL_STATUS_FAULT_LOG_STATUS(_) (((_) >> 29) & 0x01)
     ...
     *
     * [Bit 30] This field indicates the status of the root-table pointer in
     * hardware. This field is cleared by hardware when software sets the SRTP
     * field in the Global Command register. This field is set by hardware when
     * hardware completes the 'Set Root Table Pointer' operation using the value
...
/**
 * Register providing the base address of root-table and the translation table
 * mode. Software programs the desired values in this register but these values
 * take effect only after software executes Set Root Table Pointer command
...
typedef union {
  struct {
     ...
     *
     * [Bits 11:10] This field specifies the translation mode used for DMA
     * remapping.
     * * 00: legacy mode - uses root tables and context tables.
     * * 01: scalable mode - uses scalable-mode root tables and scalable mode
     * context tables.
     * * 10: reserved - in prior version of this specification, this encoding
     * was used to enable extended mode which is no longer supported.
     * * 11: abort-dma mode.
     * For implementations reporting Enhanced SRTP Support (ESRTPS) field as
     * Clear in the Capability register, software must not modify this field
     * while DMA remapping is active (TES=1 in Global Status register). The
     * value of this field takes effect only after software executes Set Root
...
#define VTD_ROOT_TABLE_ADDRESS_TRANSLATION_TABLE_MODE(_) (((_) >> 10) & 0x03)
     ...
     * root-table in system memory. Hardware may ignore and not implement bits
     * 63:HAW, where HAW is the host address width. The value of this field
     * takes effect only after software executes Set Root Table Pointer command.
     */
...
#define VTD_ROOT_TABLE_ADDRESS_ROOT_TABLE_ADDRESS(_)                           \
  (((_) >> 12) & 0xFFFFFFFFFFFFF)
  };
 ...
 * CCMD_REG with the ICC field Set causes the hardware to perform the
...
typedef union {
  struct {
     ...
     *
     * [Bits 15:0] Indicates the id of the domain whose context-entries need to
     * be selectively invalidated. This field must be programmed by software for
     * both domain selective and device-selective invalidation requests. The
     * Capability register reports the domain-id width supported by hardware.
     * Software must ensure that the value written to this field is within this
     * limit. Hardware ignores (and may not implement) bits 15:N, where N is the
...
#define VTD_CONTEXT_COMMAND_DOMAIN_ID(_) (((_) >> 0) & 0xFFFF)
     ...
     * context-entry needs to be selectively invalidated.This field along with
     * the FM field must be programmed by software for device-selective
     * invalidation requests. The value returned on a read of this field is
     * undefined.
     */
    UINT64 SourceId : 16;
...
#define VTD_CONTEXT_COMMAND_SOURCE_ID(_) (((_) >> 16) & 0xFFFF)
     ...
     *
     * [Bits 33:32] Software may use the Function Mask to perform
     * device-selective invalidations on behalf of devices supporting PCI
     * Express Phantom Functions. This field specifies which bits of the
     * function number portion (least significant three bits) of the SID field
     * to mask when performing device selective invalidations.The following
     ...
     * SID and FM fields must have the domain-id specified in the DID field. The
     * value returned on a read of this field is undefined.
     */
...
#define VTD_CONTEXT_COMMAND_FUNCTION_MASK(_) (((_) >> 32) & 0x03)
     ...
     *
     * [Bits 60:59] Hardware reports the granularity at which an invalidation
     * request was processed through the CAIG field at the time of reporting
     * invalidation completion (by clearing the ICC field). The following are
     * the encodings for this field:
     ...
     * programmed to legacy mode (RTADDR_REG.TTM!=00b). On hardware
     * implementations with Major Version 6 or higher (VER_REG), all
     * invalidation requests through this register are treated as incorrect
     * invalidation requests. Software should use the Queued Invalidation
...
#define VTD_CONTEXT_COMMAND_CONTEXT_ACTUAL_INVALIDATION_GRANULARITY_FLAG       \
  0x1800000000000000
#define VTD_CONTEXT_COMMAND_CONTEXT_ACTUAL_INVALIDATION_GRANULARITY_MASK 0x03
#define VTD_CONTEXT_COMMAND_CONTEXT_ACTUAL_INVALIDATION_GRANULARITY(_)         \
  (((_) >> 59) & 0x03)

    /**
     * @brief Context Invalidation Request Granularity <b>(RW)</b>
     *
     * [Bits 62:61] Software provides the requested invalidation granularity
     * through this field when setting the ICC field:
     * * 00: Reserved.
     ...
     * be specified through the SID and FM fields, and the domain-id [that was
     * programmed in the context-entry for these device(s)] must be provided in
     * the DID field. Hardware implementations may process an invalidation
     * request by performing invalidation at a coarser granularity than
     * requested. Hardware indicates completion of the invalidation request by
     * clearing the ICC field. At this time, hardware also indicates the
     * granularity at which the actual invalidation was performed through the
     * CAIG field.
     */
    UINT64 ContextInvalidationRequestGranularity : 2;
#define VTD_CONTEXT_COMMAND_CONTEXT_INVALIDATION_REQUEST_GRANULARITY_BIT 61
#define VTD_CONTEXT_COMMAND_CONTEXT_INVALIDATION_REQUEST_GRANULARITY_FLAG      \
  0x6000000000000000
#define VTD_CONTEXT_COMMAND_CONTEXT_INVALIDATION_REQUEST_GRANULARITY_MASK 0x03
#define VTD_CONTEXT_COMMAND_CONTEXT_INVALIDATION_REQUEST_GRANULARITY(_)        \
  (((_) >> 61) & 0x03)

    /**
     * @brief Invalidate Context-Cache <b>(RW)</b>
     *
     * [Bit 63] Software requests invalidation of context-cache by setting this
     * field. Software must also set the requested invalidation granularity by
     ...
     * update this register when this field is Set. Hardware clears the ICC
     * field to indicate the invalidation request is complete.Hardware also
     * indicates the granularity at which the invalidation operation was
     * performed through the CAIG field. Software must submit a context-cache
     * invalidation request through this field only when there are no
     * invalidation requests pending at this remapping hardware unit. Since
     * information from the context-cache may be used by hardware to tag IOTLB
     * entries, software must perform domain-selective (or global) invalidation
     * of IOTLB after the context-cache invalidation has completed. Hardware
     * implementations reporting a write-buffer flushing requirement (RWBF=1 in
     * the Capability register) must implicitly perform a write buffer flush
     * before invalidating the context-cache. When Translation Table Mode field
     * in Root Table Address register is not setup as legacy mode
     * (RTADDR_REG.TTM!=00b), hardware will ignore the value provided by
     * software in this register, treat it as an incorrect invalidation request,
     * and report a value of 00b in CAIG field.
     */
...
/**
 * Register to provide the DMA address whose corresponding IOTLB entry needs to
 * be invalidated through the corresponding IOTLB Invalidate register. This
 * register is a write-only register. A value returned on a read of this
...
typedef union {
  struct {
     ...
     *
     * [Bits 5:0] The value in this field specifies the number of low order bits
     * of the ADDR field that must be masked for the invalidation operation.
     * This field enables software to request invalidation of contiguous
     * mappings for size-aligned regions. When invalidating mappings for
     * large-pages, software must specify the appropriate mask value. For
     * example, when invalidating mapping for a 2MB page, software must specify
     * an address mask value of at least 9. Hardware implementations report the
     * maximum supported address mask value through the Capability register. A
     * value returned on a read of this field is undefined.
     */
...
#define VTD_INVALIDATE_ADDRESS_ADDRESS_MASK(_) (((_) >> 0) & 0x3F)
     ...
     *
     * [Bit 6] The field provides hints to hardware about preserving or flushing
     * the nonleaf (context-entry) entries that may be cached in hardware:
     * * 0: Software may have modified both leaf and non-leaf second-level
     * paging-structure entries corresponding to mappings specified in the ADDR
     * and AM fields. On a page-selective-within-domain invalidation request,
     * hardware must invalidate the cached entries associated with the mappings
     * specified by DID, ADDR and AM fields, in both IOTLB and paging-structure
     * caches.
     ...
     * page-selective-within-domain invalidation request, hardware may preserve
     * the cached second-level mappings in paging-structurecaches. A value
     * returned on a read of this field is undefined.
     */
...
#define VTD_INVALIDATE_ADDRESS_INVALIDATION_HINT(_) (((_) >> 6) & 0x01)
     ...
     *
     * [Bits 63:12] Software provides the second-level-input-address that needs
     * to be page selectively invalidated. To make a
     * page-selective-within-domain invalidation request to hardware, software
     * must first write the appropriate fields in this register, and then issue
     * the page-selective-within-domain invalidate command through the
     * IOTLB_REG. Hardware ignores bits 63:N, where N is the maximum guest
     * address width (MGAW) supported. A value returned on a read of this field
     * is undefined.
     */
...
/**
 * Register to invalidate IOTLB. The act of writing the upper byte of the
 * IOTLB_REG with the IVT field Set causes the hardware to perform the IOTLB
 * invalidation.
 *
 * @remarks IOTLB_REG
...
typedef union {
  struct {
     ...
     * @brief Domain-ID <b>(RW)</b>
     *
     * [Bits 47:32] Indicates the ID of the domain whose IOTLB entries need to
     * be selectively invalidated. This field must be programmed by software for
     * domainselective and page-selective invalidation requests. The Capability
     * register reports the domain-id width supported by hardware. Software must
     * ensure that the value written to this field is within this limit.
...
#define VTD_IOTLB_INVALIDATE_DOMAIN_ID(_) (((_) >> 32) & 0xFFFF)
     ...
     *
     * [Bit 48] This field is ignored by hardware if the DWD field is reported
     * as Clear in the Capability register. When the DWD field is reported as
     * Set in the Capability register, the following encodings are supported for
     * this field:
...
#define VTD_IOTLB_INVALIDATE_DRAIN_WRITES(_) (((_) >> 48) & 0x01)
     ...
     *
     * [Bit 49] This field is ignored by hardware if the DRD field is reported
     * as Clear in the Capability register. When the DRD field is reported as
     * Set in the Capability register, the following encodings are supported for
     * this field:
     * * 0: Hardware may complete the IOTLB invalidation without draining DMA
     * read requests.
...
#define VTD_IOTLB_INVALIDATE_DRAIN_READS(_) (((_) >> 49) & 0x01)
     ...
     *
     * [Bits 58:57] Hardware reports the granularity at which an invalidation
     * request was processed through this field when reporting invalidation
     * completion (by clearing the IVT field). The following are the encodings
     * for this field.
     * * 00: Error. This indicates hardware detected an incorrect invalidation
     * request and ignored the request, e.g., register based invalidation when
     * Translation Table Mode (TTM) in Root Table Address Register is not
     * programmed to legacy mode (RTADDR_REG.TTM!=00b), detected an unsupported
     * address mask value in Invalidate Address register for page-selective
...
#define VTD_IOTLB_INVALIDATE_IOTLB_ACTUAL_INVALIDATION_GRANULARITY_FLAG        \
  0x600000000000000
#define VTD_IOTLB_INVALIDATE_IOTLB_ACTUAL_INVALIDATION_GRANULARITY_MASK 0x03
#define VTD_IOTLB_INVALIDATE_IOTLB_ACTUAL_INVALIDATION_GRANULARITY(_)          \
  (((_) >> 57) & 0x03)
    UINT64 Reserved3 : 1;
     ...
     *
     * [Bits 61:60] When requesting hardware to invalidate the IOTLB (by setting
     * the IVT field), software writes the requested invalidation granularity
     ...
     * indicates completion of the invalidation request by clearing the IVT
     * field. At that time, the granularity at which actual invalidation was
     * performed is reported through the IAIG field.
     */
    UINT64 IotlbInvalidationRequestGranularity : 2;
#define VTD_IOTLB_INVALIDATE_IOTLB_INVALIDATION_REQUEST_GRANULARITY_BIT 60
#define VTD_IOTLB_INVALIDATE_IOTLB_INVALIDATION_REQUEST_GRANULARITY_FLAG       \
  0x3000000000000000
#define VTD_IOTLB_INVALIDATE_IOTLB_INVALIDATION_REQUEST_GRANULARITY_MASK 0x03
#define VTD_IOTLB_INVALIDATE_IOTLB_INVALIDATION_REQUEST_GRANULARITY(_)         \
  (((_) >> 60) & 0x03)
    UINT64 Reserved4 : 1;
     ...
     *
     * [Bit 63] Software requests IOTLB invalidation by setting this field.
     * Software must also set the requested invalidation granularity by
     * programming the IIRG field. Hardware clears the IVT field to indicate the
     * invalidation request is complete. Hardware also indicates the granularity
     * at which the invalidation operation was performed through the IAIG field.
     * Software must not submit another invalidation request through this
     * register while the IVT field is Set, nor update the associated Invalidate
     * Address register. Software must not submit IOTLB invalidation requests
     * when there is a context-cache invalidation request pending at this
     ...
     * flushing before invalidating the IOTLB. Refer to Section 6.8 for write
     * buffer flushing requirements. When Translation Table Mode field in Root
     * Table Address registers is not setup as legacy mode
     * (RTADDR_REG.TTM!=00b), hardware will ignore the value provided by
     * software in this register, treat it as an incorrect invalidation request,
     * and report a value of 00b in IAIG field.
     */
...
#define XCR0_X87(_) (((_) >> 0) & 0x01)
    ...
    /**
     * [Bit 1] If 1, the XSAVE feature set can be used to manage MXCSR and the
     * XMM registers (XMM0-XMM15 in 64-bit mode; otherwise XMM0-XMM7).
     */
    UINT64 Sse : 1;
...
#define XCR0_SSE(_) (((_) >> 1) & 0x01)
    ...
    /**
     * [Bit 2] If 1, AVX instructions can be executed and the XSAVE feature set
...
#define XCR0_AVX(_) (((_) >> 2) & 0x01)
    ...
    /**
     * [Bit 3] If 1, MPX instructions can be executed and the XSAVE feature set
     * can be used to manage the bounds registers BND0-BND3.
     */
    UINT64 Bndreg : 1;
...
#define XCR0_BNDREG(_) (((_) >> 3) & 0x01)
    ...
    /**
     * [Bit 4] If 1, MPX instructions can be executed and the XSAVE feature set
...
#define XCR0_BNDCSR(_) (((_) >> 4) & 0x01)
    ...
    /**
     * [Bit 5] If 1, AVX-512 instructions can be executed and the XSAVE feature
...
#define XCR0_OPMASK(_) (((_) >> 5) & 0x01)
    ...
    /**
     * [Bit 6] If 1, AVX-512 instructions can be executed and the XSAVE feature
...
#define XCR0_ZMM_HI256(_) (((_) >> 6) & 0x01)
    ...
    /**
     * [Bit 7] If 1, AVX-512 instructions can be executed and the XSAVE feature
     * set can be used to manage the upper ZMM registers (ZMM16-ZMM31, oonly in
     * 64-bit mode).
     */
    UINT64 ZmmHi16 : 1;
...
#define XCR0_ZMM_HI16(_) (((_) >> 7) & 0x01)
    ...
    /**
     * [Bit 9] If 1, the XSAVE feature set can be used to manage the PKRU
     * register.
     */
    UINT64 Pkru : 1;
